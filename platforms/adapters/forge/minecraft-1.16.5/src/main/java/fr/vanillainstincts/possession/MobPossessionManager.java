...
public final class MobPossessionManager {
    ...
    public static void onEnderPearlTeleport(
        ...
        Session session = player == null ? null : BY_PLAYER.get(player.getUUID());
        if (session == null || !valid(player, session.mob)) return;
        if (!(event.getPearlEntity().level instanceof ServerWorld)) {
            return;
        } ServerWorld targetLevel = (ServerWorld) (event.getPearlEntity().level);
        MobEntity mob = session.mob;
    ...
    private static MobEntity teleportMob1193(MobEntity mob, ServerWorld targetLevel,
        ...
        if (mob.level != targetLevel) {
            Entity changed = mob.changeDimension(targetLevel);
    ...
    private static void steerCreativeFlight(MobEntity mob, float forward,
        ...
        Vector3d forwardVector = Vector3d.directionFromRotation(0.0F, mob.yRot);
        Vector3d right = new Vector3d(-forwardVector.z, 0.0D, forwardVector.x);
        Vector3d desired = forwardVector.scale(forward)
    ...
    private static void steerThreeDimensional(MobEntity mob, float forward,
        ...
        Vector3d horizontalRight = new Vector3d(-look.z, 0.0D, look.x);
        if (horizontalRight.lengthSqr() > 1.0E-6D) {
            horizontalRight = horizontalRight.normalize();
        }
        Vector3d desired = look.scale(forward)
    ...
    private static void steerGround(ServerPlayerEntity player, Session session,
        ...
        Vector3d forwardVector = Vector3d.directionFromRotation(0.0F, mob.yRot);
        Vector3d right = new Vector3d(-forwardVector.z, 0.0D, forwardVector.x);
        Vector3d desired = forwardVector.scale(forward).add(right.scale(-strafe));
        Vector3d current = mob.getDeltaMovement();
        if (desired.lengthSqr() > 1.0E-5D) {
            desired = desired.normalize();
            ...
            if (sneak && mob.isOnGround()) {
                Vector3d safe = keepSneakingOnLedge(mob, dx, dz);
                dx = safe.x;
                dz = safe.z;
            }
        ...
        if (!sneak && mob.isOnGround() && ((jump && !session.jumpHeld)
                || (autoHop && moving))) {
            ...
            Vector3d velocity = mob.getDeltaMovement();
            mob.setDeltaMovement(velocity.x, Math.max(velocity.y, 0.20D),
                    velocity.z);
        }
    }

    private static Vector3d keepSneakingOnLedge(MobEntity mob, double dx, double dz) {
    ...
    private static void applyMovementExhaustion(ServerPlayerEntity player,
        ...
        Vector3d current = session.mob.position();
        Vector3d previous = session.lastFoodPosition;
        ...
        if (session.mob.isInWaterOrBubble()) {
            exhaustion = (float) (Math.sqrt(dx * dx + dy * dy + dz * dz)
                    * 0.01D);
        } else if (session.lastFoodSprint && session.mob.isOnGround()) {
    ...
    private static double movementSpeed(MobEntity mob) {
        ...
        return MathHelper.clamp(mob.getAttributeValue(Attributes.MOVEMENT_SPEED),
                0.02D, 1.50D);
    }
    ...
    private static void steerDragon(EnderDragonEntity dragon, float forward,
                                    float strafe, boolean ascend,
                                    boolean descend, boolean sprint) {
        Vector3d look = dragon.getLookAngle().normalize();
        ...
        Vector3d wanted = dragon.position().add(desired.scale(lead));
    ...
    private static void syncControllerBody(ServerPlayerEntity player, MobEntity mob) {
        ...
        player.setOnGround(mob.isOnGround());
    }
    ...
    private static boolean teleportEndermanToAim(EndermanEntity enderman) {
        ...
        Vector3d origin = enderman.position();
        ...
        for (int drop = 0; drop <= 6; drop++) {
            ...
            Vector3d target = new Vector3d(feet.getX() + 0.5D, feet.getY(),
                    feet.getZ() + 0.5D);
            if (target.distanceToSqr(origin)
                    > ENDERMAN_TELEPORT_RANGE * ENDERMAN_TELEPORT_RANGE) {
                continue;
            }
            Vector3d delta = target.subtract(origin);
    ...
    private static final class Session {
        ...
        private Vector3d lastFoodPosition;
