...
public final class GolemConstructionController {
    ...
    public static boolean maintain(EntityLiving mob, EntityLivingBase target,
        ...
        if (anchorDistance > GolemRules.CONSTRUCTION_COLUMN_LOCK_DISTANCE_SQR) {
            Vec3d center = columnCenter(anchor, mob.posY);
            if (combat.constructionBlocksPlaced() == 0) {
                ...
                mob.getNavigator().tryMoveToXYZ(center.x, center.y, center.z, speed);
            ...
            double speed = MobMovementPolicy.navigationSpeed(mob,
                    ...
                    capabilities.approachSpeed, gameTime);
            Minecraft112Compat.moveHelper(mob, center.x, center.y,
                    center.z, speed);
            MobMovementPolicy.applyNavigationSprintFlag(mob,
    ...
    private static boolean tryBreakFatalPerchSupport(
        ...
        if (supports.isEmpty()) {
            return !target.onGround && Minecraft112Compat.motion(target).y < 0.0D;
        }
        ...
        for (int y = maxY; y >= minY; y--) {
            for (int x = minX; x <= maxX; x++) {
                for (int z = minZ; z <= maxZ; z++) {
                    ...
                    AxisAlignedBB shape = level.getBlockState(pos).getCollisionBoundingBox(level, pos);
                    if (shape == null) {
                        continue;
                    }
    ...
    private static Vec3d jumpImpulse(EntityLiving mob, EntityLivingBase target,
                                    Capabilities capabilities) {
        Vec3d direction = target.getPositionVector().subtract(mob.getPositionVector())
                ;
    ...
    private static boolean canPlaceScaffold(WorldServer level, EntityLiving mob,
        ...
        return level.isBlockLoaded(position) && level.isBlockLoaded(support)
                ...
                && !level.getBlockState(position).getMaterial().isLiquid()
                && level.getTileEntity(position) == null
    ...
    private static void beginPhysicalPillarJump(EntityLiving mob,
                                                ...
                                                long gameTime) {
        mob.getNavigator().clearPath();
        Vec3d movement = Minecraft112Compat.motion(mob);
        Minecraft112Compat.setMotion(mob, movement.x * 0.08D,
    ...
    private static void steerTowardColumn(EntityLiving mob, BlockPos placement) {
        Vec3d center = new Vec3d(placement.getX() + 0.5D, mob.posY,
                placement.getZ() + 0.5D);
        Vec3d correction = Minecraft112Compat.multiply(
        ...
        Vec3d current = Minecraft112Compat.motion(mob);
        Minecraft112Compat.setMotion(mob, current.x * 0.35D + correction.x,
    ...
    private static void dampHorizontalMotion(EntityLiving mob, double factor) {
        Vec3d movement = Minecraft112Compat.motion(mob);
        Minecraft112Compat.setMotion(mob, movement.x * factor, movement.y,
                movement.z * factor);
        mob.velocityChanged = true;
    }
