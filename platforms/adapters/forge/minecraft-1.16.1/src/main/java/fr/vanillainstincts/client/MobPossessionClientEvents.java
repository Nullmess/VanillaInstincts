...
public final class MobPossessionClientEvents {
    ...
    private static void blit(MatrixStack poseStack, ResourceLocation texture,
        ...
        bind(texture);
        AbstractGui.blit(poseStack, x, y, u, v, width, height, 256, 256);
    }
    ...
    private static void predictLocalMovement(MobEntity mob, int flags, float yaw,
        ...
        if (creativeControl) {
            Vector3d facing = Vector3d.directionFromRotation(0.0F, yaw);
            Vector3d right = new Vector3d(-facing.z, 0.0D, facing.x);
            Vector3d desired = facing.scale(forward)
            ...
            if (desired.lengthSqr() < 1.0E-5D) {
                ...
                Vector3d predicted = desired.normalize()
        ...
        double baseSpeed = mob.getAttribute(Attributes.MOVEMENT_SPEED) == null
                ? 0.10D : MathHelper.clamp(mob.getAttributeValue(Attributes.MOVEMENT_SPEED),
                0.02D, 1.50D);
        ...
        Vector3d predicted;
        if (free3d) {
            Vector3d look = mob.getLookAngle();
            ...
            Vector3d facing = Vector3d.directionFromRotation(0.0F, yaw);
            Vector3d right = new Vector3d(-facing.z, 0.0D, facing.x);
            Vector3d desired = facing.scale(forward).add(right.scale(-strafe));
            ...
            if (sneak && mob.isOnGround()) {
                Vector3d safe = predictSneakEdge(mob, dx, dz);
                dx = safe.x;
                dz = safe.z;
            }
            double dy = 0.0D;
            ...
            predicted = new Vector3d(dx, dy, dz);
        }
    ...
    private static void applyPredictedMove(MobEntity mob, Vector3d movement,
    ...
    private static Vector3d predictSneakEdge(MobEntity mob, double dx, double dz) {
        ...
        return new Vector3d(safeX, 0.0D, safeZ);
    }
