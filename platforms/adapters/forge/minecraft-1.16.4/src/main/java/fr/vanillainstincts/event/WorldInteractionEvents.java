...
public final class WorldInteractionEvents {
    ...
    public static void onRightClickBlock(
        ...
        if (!(event.getWorld() instanceof ServerWorld)
            ...
            return;
        } ServerWorld level = (ServerWorld) (event.getWorld());ServerPlayerEntity player = (ServerPlayerEntity) (event.getEntity());
    ...
    public static void onRightClickItem(
        ...
        if (!(event.getWorld() instanceof ServerWorld)
                || !(event.getEntity() instanceof ServerPlayerEntity)
                || !FeatureGate.enabled(FeatureFlag.PERCEPTION_MEMORY, ((ServerWorld) (event.getWorld())))) {
            return;
        } ServerWorld level = (ServerWorld) (event.getWorld());ServerPlayerEntity player = (ServerPlayerEntity) (event.getEntity());
        MobStimulusSystem.emit(level, StimulusType.ITEM_USE,
    ...
    public static void onCropBroken(BlockEvent.BreakEvent event) {
        ...
        if (!(event.getWorld() instanceof ServerWorld)
            ...
            return;
        } ServerWorld level = (ServerWorld) (event.getWorld());ServerPlayerEntity player = (ServerPlayerEntity) (event.getPlayer());
        if (FeatureGate.enabled(FeatureFlag.PERCEPTION_MEMORY, level)) {
        ...
        VillagerEntity farmer = FarmerController.onPlayerCropBroken(level,
