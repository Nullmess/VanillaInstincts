...
public final class MobPossessionClientEvents {
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        minecraft.player.setDeltaMovement(Vec3d.ZERO);
    ...
    public static void onInteraction(
        ...
        if (event.isAttack() || event.isUseItem()) {
            ...
            if (event.isAttack()) {
                Minecraft minecraft = Minecraft.getInstance();
                EntityLiving mob = possessedMob(minecraft);
                if (mob != null) mob.swingArm(EnumHand.MAIN_HAND);
    ...
    public static void onRenderLevelLast(RenderWorldLastEvent event) {
        ...
        if (current != null && current.getType() == RayTraceResult.Type.ENTITY) {
            return;
        }
        RayTraceResult target = mob.pick(5.0D, 1.0F, false);
        if (!(target instanceof BlockRayTraceResult)
                || target.getType() != RayTraceResult.Type.BLOCK) {
            return;
        ...
        Vec3d camera = minecraft.gameRenderer.getMainCamera().getPosition();
    ...
    private static float displayedAbsorption(EntityLiving mob) {
        ...
        net.minecraft.potion.PotionEffect effect = mob.getEffect(MobEffects.ABSORPTION);
        if (effect != null) {
    ...
    private static void predictLocalMovement(EntityLiving mob, int flags, float yaw,
        ...
        if (creativeControl) {
            Vec3d facing = Vec3d.directionFromRotation(0.0F, yaw);
            Vec3d right = new Vec3d(-facing.zCoord, 0.0D, facing.xCoord);
            Vec3d desired = facing.scale(forward)
            ...
            if (desired.lengthSqr() < 1.0E-5D) {
                ...
                Vec3d predicted = desired.normalize()
        ...
        Vec3d predicted;
        if (free3d) {
            Vec3d look = mob.getLookVec();
            Vec3d right = new Vec3d(-look.zCoord, 0.0D, look.xCoord);
            ...
            Vec3d facing = Vec3d.directionFromRotation(0.0F, yaw);
            Vec3d right = new Vec3d(-facing.zCoord, 0.0D, facing.xCoord);
            Vec3d desired = facing.scale(forward).add(right.scale(-strafe));
            ...
            if (sneak && mob.onGround) {
                Vec3d safe = predictSneakEdge(mob, dx, dz);
                dx = safe.xCoord;
                dz = safe.zCoord;
            }
            double dy = 0.0D;
    ...
    private static void applyPredictedMove(EntityLiving mob, Vec3d movement,
    ...
    private static Vec3d predictSneakEdge(EntityLiving mob, double dx, double dz) {
