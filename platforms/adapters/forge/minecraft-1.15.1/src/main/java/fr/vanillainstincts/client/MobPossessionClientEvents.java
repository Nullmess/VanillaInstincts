...
public final class MobPossessionClientEvents {
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        MobEntity mob = possessedMob(minecraft);
        if (mob == null) return;
    ...
    private static void restoreCreativePredictionGravity(Minecraft minecraft) {
        ...
        if (previous instanceof MobEntity) { MobEntity mob = (MobEntity) (previous); 
    ...
    public static void onInteraction(
        ...
        if (event.isAttack() || event.isUseItem()) {
            ...
            if (event.isAttack()) {
                Minecraft minecraft = Minecraft.getInstance();
                MobEntity mob = possessedMob(minecraft);
                if (mob != null) mob.swing(Hand.MAIN_HAND);
    ...
    public static void onRenderLivingPre(RenderLivingEvent.Pre<?, ?> event) {
        if (!(event.getEntity() instanceof MobEntity)) return; MobEntity mob = (MobEntity) (event.getEntity());
        if (!MobPossessionClientState.active()) return;
        ...
        if (hideHands) {
            mob.setItemSlot(EquipmentSlotType.MAINHAND, ItemStack.EMPTY);
            mob.setItemSlot(EquipmentSlotType.OFFHAND, ItemStack.EMPTY);
        }
        if (hideArmor) {
            mob.setItemSlot(EquipmentSlotType.HEAD, ItemStack.EMPTY);
            mob.setItemSlot(EquipmentSlotType.CHEST, ItemStack.EMPTY);
            mob.setItemSlot(EquipmentSlotType.LEGS, ItemStack.EMPTY);
            mob.setItemSlot(EquipmentSlotType.FEET, ItemStack.EMPTY);
        }
    }
    ...
    public static void onRenderLevelLast(RenderWorldLastEvent event) {
        ...
        net.minecraft.block.BlockState state = minecraft.level.getBlockState(pos);
        if (state.isAir()
    ...
    private static void renderMobHotbar(MatrixStack poseStack,
                                        Minecraft minecraft, MobEntity mob) {
    ...
    private static float displayedAbsorption(MobEntity mob) {
        float amount = Math.max(0.0F, mob.getAbsorptionAmount());
        net.minecraft.potion.EffectInstance effect = mob.getEffect(Effects.ABSORPTION);
        if (effect != null) {
    ...
    private static void renderAir(MatrixStack poseStack, MobEntity mob,
                                  int right, int y) {
    ...
    private static void applyFirstPersonSwing(MatrixStack pose, MobEntity mob) {
    ...
    private static void predictLocalMovement(MobEntity mob, int flags, float yaw,
    ...
    private static void applyPredictedMove(MobEntity mob, Vec3d movement,
    ...
    private static Vec3d predictSneakEdge(MobEntity mob, double dx, double dz) {
    ...
    private static MobEntity possessedMob(Minecraft minecraft) {
    ...
    private static class VisualEquipmentSnapshot {
        ...
        private static VisualEquipmentSnapshot capture(MobEntity mob) {
            return new VisualEquipmentSnapshot(
                    mob.getItemBySlot(EquipmentSlotType.MAINHAND).copy(),
                    mob.getItemBySlot(EquipmentSlotType.OFFHAND).copy(),
                    mob.getItemBySlot(EquipmentSlotType.HEAD).copy(),
                    mob.getItemBySlot(EquipmentSlotType.CHEST).copy(),
                    mob.getItemBySlot(EquipmentSlotType.LEGS).copy(),
                    mob.getItemBySlot(EquipmentSlotType.FEET).copy());
        }

        private void restore(MobEntity mob) {
