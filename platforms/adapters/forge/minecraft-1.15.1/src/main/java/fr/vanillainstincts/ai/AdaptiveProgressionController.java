...
public final class AdaptiveProgressionController {
    ...
    public static void handleProjectileImpact(ProjectileImpactEvent event) {
        ...
        if (!(projectile.level instanceof ServerWorld)
                || !projectile.getPersistentData().getBoolean(PEARL_PROJECTILE)
                || !projectile.getPersistentData().hasUUID(PEARL_PROJECTILE_OWNER)) {
            return;
        }
        ServerWorld level = (ServerWorld) projectile.level;
        Entity owner = level.getEntity(projectile.getPersistentData().getUUID(PEARL_PROJECTILE_OWNER));
        if (!(owner instanceof ZombieEntity) || !owner.isAlive()
                || !isPearlHunter((ZombieEntity) owner)) return;
        ZombieEntity zombie = (ZombieEntity) owner;
    ...
    private static void castHook(ZombieEntity zombie, ServerPlayerEntity target,
        ...
        Vec3d origin = zombie.getEyePosition(1.0F).add(
                zombie.getLookAngle().scale(0.35D));
        Vec3d aim = target.getBoundingBox().getCenter().add(
    ...
    private static void tickActiveHook(ZombieEntity zombie, ServerPlayerEntity target,
        ...
        Vec3d pull = zombie.getBoundingBox().getCenter()
    ...
    private static void renderHookLine(ZombieEntity zombie, ServerWorld level,
        ...
        Vec3d start = zombie.position().add(0.0D, 1.35D, 0.0D);
    ...
    private static void tickPearlHunter(ZombieEntity zombie, ServerWorld level,
        ...
        pearl.getPersistentData().putUUID(PEARL_PROJECTILE_OWNER, zombie.getUUID());
    ...
    private static void emitPearlParticles(ServerWorld level, Vec3d point) {
