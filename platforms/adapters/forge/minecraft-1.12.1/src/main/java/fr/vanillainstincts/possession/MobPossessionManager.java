...
public final class MobPossessionManager {
    ...
    public static void tick(MinecraftServer server) {
        ...
        while (iterator.hasNext()) {
            ...
            EntityPlayerMP player = server.getPlayerList().getPlayer(entry.getKey());
            Session session = entry.getValue();
            EntityLiving mob = session.mob;
            if (player != null && mob != null && mob.isEntityAlive()
                    && !fr.vanillainstincts.compat.Minecraft112Compat.removed(mob) && player.getLevel() != mob.world) {
    ...
    public static void onEnderPearlTeleport(
        ...
        if (event == null || !(event.getEntityLiving() instanceof EntityPlayerMP)) {
            return;
        }
    ...
    public static void onMobJoin(EntityLiving mob) {
        if (mob == null || mob.world.isRemote) return;
        if (!mob.getEntityData().hasUniqueId(POSSESSION_CONTROLLER_KEY)) return;
        UUID playerId = mob.getEntityData().getUniqueId(POSSESSION_CONTROLLER_KEY);
        Session session = BY_PLAYER.get(playerId);
        if (session == null) {
    ...
    private static void completeDimensionTransfer(EntityPlayerMP player,
                                                  Session session) {
        EntityLiving mob = session.mob;
        if (!(mob.world instanceof WorldServer)) return; WorldServer targetLevel = (WorldServer) (mob.world);
        session.dimensionTransferPending = false;
        session.dimensionTransferReady = false;
        session.dimensionTransferGraceTicks = 0;
        mob.getEntityData().setUniqueId(POSSESSION_CONTROLLER_KEY,
                player.getUniqueID());
    ...
    public static void clearLevel(WorldServer level) {
        ...
        while (iterator.hasNext()) {
            Map.Entry<UUID, Session> entry = iterator.next();
            Session session = entry.getValue();
            if (session.mob.world != level) continue;
    ...
    private static boolean start(EntityPlayerMP player, EntityLiving mob) {
        ...
        if (mob == null || player.getCamera() != mob || !mob.isEntityAlive()
                || fr.vanillainstincts.compat.Minecraft112Compat.removed(mob) || mob.world != player.getLevel()) {
            message(player, "message.vanillainstincts.possession.camera_required");
            return false;
        }
    ...
    private static void restoreMob(Session session) {
        ...
        if (session.bowDrawing && mob instanceof AbstractSkeleton) { AbstractSkeleton skeleton = (AbstractSkeleton) (mob); 
        ...
        if (session.previousTarget != null && session.previousTarget.isEntityAlive()
                && session.previousTarget.world == mob.world) {
    ...
    private static void steerGround(EntityPlayerMP player, Session session,
        ...
        if (!sneak && mob.onGround && ((jump && !session.jumpHeld)
            ...
            Vec3d velocity = mob.getMotion();
            mob.setDeltaMovement(velocity.x, Math.max(velocity.y, 0.20D),
                    velocity.z);
        }
    }

    private static Vec3d keepSneakingOnLedge(EntityLiving mob, double dx, double dz) {
    ...
    private static void applyMovementExhaustion(EntityPlayerMP player,
        ...
        Vec3d current = session.mob.getPositionVector();
    ...
    private static void tickPossessionFood(EntityPlayerMP player, Session session,
                                           boolean creativeControl) {
        if (creativeControl || !(session.mob.world instanceof WorldServer)) {
            session.foodTickTimer = 0;
            return;
        } WorldServer level = (WorldServer) (session.mob.world);
    ...
    private static void steerDragon(EntityDragon dragon, float forward,
                                    float strafe, boolean ascend,
                                    boolean descend, boolean sprint) {
        Vec3d look = dragon.getLookVec().normalize();
        ...
        Vec3d wanted = dragon.getPositionVector().add(desired.scale(lead));
    ...
    private static void handleContinuousActions(EntityPlayerMP player,
        ...
        boolean emptyMainHand = player.getHeldItemMainhand().isEmpty();
    ...
    private static void resetMining(EntityPlayerMP player, Session session) {
        if (session.breakPos != null && session.mob.world instanceof WorldServer) { WorldServer level = (WorldServer) (session.mob.world); 
    ...
    private static void pickupNearbyItems(EntityPlayerMP player, Session session) {
        EntityLiving mob = session.mob;
        if (!(mob.world instanceof WorldServer)) return; WorldServer level = (WorldServer) (mob.world);
    ...
    private static boolean playerLikeEntityUse(EntityPlayerMP player, Session session) {
        ...
        EnumActionResult result = withPlayerProxy(player, mob, () ->
                player.interactOn(entityTarget, EnumHand.MAIN_HAND));
        if (result == EnumActionResult.PASS) {
            result = withPlayerProxy(player, mob, () ->
                    player.interactOn(entityTarget, EnumHand.OFF_HAND));
        }
        if (result == EnumActionResult.PASS) return false;
        PossessionPlayerInventory.syncEquipment(player, mob);
        player.inventoryMenu.broadcastChanges();
        return true;
    }
    ...
    private static void beginSkeletonBow(Session session,
                                         EnumHand hand) {
        if (!(session.mob instanceof AbstractSkeleton)) return; AbstractSkeleton skeleton = (AbstractSkeleton) (session.mob);
        session.bowDrawing = true;
        session.bowHand = hand;
        session.bowStartedAt = skeleton.world.getTotalWorldTime();
    ...
    private static void cancelSkeletonBow(Session session) {
        ...
        if (session.mob instanceof AbstractSkeleton) { AbstractSkeleton skeleton = (AbstractSkeleton) (session.mob); 
    ...
    private static void releaseSkeletonBow(EntityPlayerMP player,
                                           Session session) {
        if (!(session.mob instanceof AbstractSkeleton)
                || !session.bowDrawing || session.bowHand == null) {
            cancelSkeletonBow(session);
            return;
        } AbstractSkeleton skeleton = (AbstractSkeleton) (session.mob);
    ...
    private static boolean teleportEndermanToAim(EntityEnderman enderman) {
        ...
        WorldServer level = (WorldServer) enderman.world;
        Vec3d origin = enderman.getPositionVector();
        ...
        for (int drop = 0; drop <= 6; drop++) {
            ...
            if (!level.getBlockState(feet.down()).getMaterial().blocksMotion()) continue;
    ...
    private static final class Session {
        ...
        private Session(EntityLiving mob, boolean previousNoAi,
            ...
            this.lastFoodPosition = mob.getPositionVector();
        }
    }
