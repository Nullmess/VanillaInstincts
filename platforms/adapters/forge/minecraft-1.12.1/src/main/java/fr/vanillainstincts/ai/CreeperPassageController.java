...
public final class CreeperPassageController {
    ...
    public static Optional<BlockPos> findPassageObstacle(EntityCreeper creeper,
        ...
        if (creeper == null || destination == null
                || !(creeper.world instanceof WorldServer)) {
            return Optional.empty();
        }
        WorldServer level = (WorldServer) creeper.world;
    ...
    private static boolean isOpenPassage(IBlockState state) {
        ...
        if (block instanceof BlockDoor && state.getPropertyKeys().contains(BlockDoor.OPEN)) {
            return state.getValue(BlockDoor.OPEN);
        }
        if (block instanceof BlockFenceGate && state.getPropertyKeys().contains(BlockFenceGate.OPEN)) {
            return state.getValue(BlockFenceGate.OPEN);
        }
        if (block instanceof BlockTrapDoor && state.getPropertyKeys().contains(BlockTrapDoor.OPEN)) {
