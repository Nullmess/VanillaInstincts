...
public final class VillageInteractionEvents {
    ...
    public static void onTradeWithVillager(EntityPlayer player,
        ...
        if (abstractVillager instanceof EntityVillager
                ...
                ((WorldServer) (player.world)))) { EntityVillager villager = (EntityVillager) (abstractVillager); WorldServer level = (WorldServer) (player.world); 
            VillagerGrandMasterController.recordMasterTrade(villager, level);
        }
        if (FeatureGate.enabled(FeatureFlag.SOCIAL_TRADING, player.world)) {
            VillagerWalletController.recordPlayerTrade(abstractVillager, offer);
            if (player.world instanceof WorldServer
                    && !offer.getResult().getItem().equals(net.minecraft.init.Items.EMERALD)) { WorldServer level = (WorldServer) (player.world); 
