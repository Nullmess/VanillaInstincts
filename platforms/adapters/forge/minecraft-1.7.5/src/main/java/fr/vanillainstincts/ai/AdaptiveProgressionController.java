...
public final class AdaptiveProgressionController {
    ...
    public static void tickHookItem(EntityItem hook, WorldServer level,
        ...
        if (!fr.vanillainstincts.compat.Minecraft18NbtCompat.hasUniqueId(data, ANGLER_HOOK_OWNER)) {
            hook.setDead();
            return;
        }
        Entity owner = fr.vanillainstincts.compat.Minecraft17Compat.getEntityFromUuid(level, fr.vanillainstincts.compat.Minecraft18NbtCompat.getUniqueId(data, ANGLER_HOOK_OWNER));
    ...
    private static void castHook(EntityZombie zombie, EntityPlayerMP target,
        ...
        Vec3 origin = fr.vanillainstincts.compat.Minecraft17Compat.eyes(zombie, 1.0F).add(
                fr.vanillainstincts.compat.Minecraft112Compat.scale(zombie.getLookVec(), 0.35D));
        Vec3 aim = fr.vanillainstincts.compat.Minecraft110Compat.boxCenter(fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(target)).add(
                fr.vanillainstincts.compat.Minecraft112Compat.scale(Minecraft112Compat.motion(target), 4.0D));
        ...
        Minecraft112Compat.setMotion(hook, fr.vanillainstincts.compat.Minecraft112Compat.scale(direction.normalize(), 
                AdaptiveProgressionRules.ZOMBIE_ANGLER_HOOK_SPEED));
        hook.getEntityData().setBoolean(ANGLER_HOOK, true);
        fr.vanillainstincts.compat.Minecraft18NbtCompat.setUniqueId(hook.getEntityData(), ANGLER_HOOK_OWNER,
                zombie.getUniqueID());
        if (!level.spawnEntityInWorld(hook)) return;

        fr.vanillainstincts.compat.Minecraft18NbtCompat.setUniqueId(zombie.getEntityData(), ANGLER_HOOK_ID, hook.getUniqueID());
        zombie.getEntityData().setLong(ANGLER_HOOK_EXPIRES,
                gameTime
                        + AdaptiveProgressionRules
                        .ZOMBIE_ANGLER_HOOK_LIFETIME_TICKS);
        zombie.swingItem();
        fr.vanillainstincts.compat.Minecraft18SoundCompat.play(level, entityBlockPos(zombie), "random.splash", 0.8F, 1.25F);
    }
    ...
    private static void tickActiveHook(EntityZombie zombie, EntityPlayerMP target,
        ...
        Vec3 pull = fr.vanillainstincts.compat.Minecraft110Compat.boxCenter(fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(zombie))
                .subtract(fr.vanillainstincts.compat.Minecraft110Compat.boxCenter(fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(target)));
        if (Minecraft112Compat.lengthSq(pull) > 1.0E-6D) {
            pull = fr.vanillainstincts.compat.Minecraft112Compat.scale(pull.normalize(), 
    ...
    private static void retractHook(EntityZombie zombie, EntityItem hook,
        ...
        clearHook(zombie);
        zombie.swingItem();
        fr.vanillainstincts.compat.Minecraft18SoundCompat.play(zombie, "random.pop", 0.8F, 0.75F);
    ...
    private static void renderHookLine(EntityZombie zombie, WorldServer level,
        ...
        Vec3 start = Minecraft112Compat.add(fr.vanillainstincts.compat.Minecraft17Compat.position(zombie), 0.0D, 1.35D, 0.0D);
    ...
    private static void tickPearlHunter(EntityZombie zombie, WorldServer level,
        ...
        Vec3 origin = fr.vanillainstincts.compat.Minecraft17Compat.eyes(zombie, 1.0F).add(
                fr.vanillainstincts.compat.Minecraft112Compat.scale(zombie.getLookVec(), 0.35D));
        Vec3 aim = fr.vanillainstincts.compat.Minecraft110Compat.boxCenter(fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(target)).add(
                fr.vanillainstincts.compat.Minecraft112Compat.scale(Minecraft112Compat.motion(target), 5.0D));
        Vec3 direction = aim.subtract(origin);
        if (Minecraft112Compat.lengthSq(direction) < 1.0E-6D) return;

        EntitySnowball pearl = new EntitySnowball(level, zombie);
        pearl.getEntityData().setBoolean(PEARL_PROJECTILE, true);
        fr.vanillainstincts.compat.Minecraft18NbtCompat.setUniqueId(pearl.getEntityData(), PEARL_PROJECTILE_OWNER, zombie.getUniqueID());
    ...
    private static EntityPlayerMP nearestPlayerWithAdvancement(
        ...
        return Minecraft112Compat.players(level).stream()
    ...
    private static boolean holdsFishingRod(EntityZombie zombie) {
        ItemStack held = zombie == null ? null : zombie.getHeldItem();
    ...
    private static EntityPlayerMP progressionTarget(EntityZombie zombie,
                                                  WorldServer level) {
        NBTTagCompound data = zombie.getEntityData();
        if (!fr.vanillainstincts.compat.Minecraft18NbtCompat.hasUniqueId(data, TARGET)) return null;
        Entity entity = fr.vanillainstincts.compat.Minecraft17Compat.getEntityFromUuid(level, fr.vanillainstincts.compat.Minecraft18NbtCompat.getUniqueId(data, TARGET));
    ...
    private static int countLocal(WorldServer level, EntityZombie zombie,
        ...
        return fr.vanillainstincts.compat.Minecraft112Compat.entitiesWithinAABB(level, EntityZombie.class,
                fr.vanillainstincts.compat.Minecraft112Compat.expandBox(fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(zombie), radius),
    ...
    private static EntityItem hookEntity(EntityZombie zombie, WorldServer level) {
        NBTTagCompound data = zombie.getEntityData();
        if (!fr.vanillainstincts.compat.Minecraft18NbtCompat.hasUniqueId(data, ANGLER_HOOK_ID)) return null;
        Entity entity = fr.vanillainstincts.compat.Minecraft17Compat.getEntityFromUuid(level, fr.vanillainstincts.compat.Minecraft18NbtCompat.getUniqueId(data, ANGLER_HOOK_ID));
    ...
    private static void ensureAnglerEquipment(EntityZombie zombie) {
        if (!holdsFishingRod(zombie)) {
            zombie.setCurrentItemOrArmor(0,
                    new ItemStack(Items.fishing_rod));
        }
    }

    private static void ensurePearlEquipment(EntityZombie zombie) {
        if (!zombie.getHeldItem().getItem().equals(Items.ender_pearl)) {
            zombie.setCurrentItemOrArmor(0,
                    new ItemStack(Items.ender_pearl));
        }
        if (!fr.vanillainstincts.compat.Minecraft110ItemStackCompat.isEmpty(null)) {
            
        }
    }

    private static void emitPearlParticles(WorldServer level, Vec3 point) {
