...
public final class MobPossessionClientEvents {
    // Minecraft 1.18 uses the legacy GUI sprite sheets.
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        if (!MobPossessionClientState.active()) {
            ...
            if (lastEntityId != -1) {
                minecraft.player.noPhysics = minecraft.fr.vanillainstincts.compat.Minecraft17Compat.isSpectator(player);
            }
            lastEntityId = -1;
            return;
        }
        ...
        EntityLiving mob = possessedMob(minecraft);
        if (mob == null) return;
        updateCreativePredictionGravity(minecraft, mob);
        if (minecraft.getCameraEntity() != mob) {
            minecraft.setCameraEntity(mob);
        }
        if (lastEntityId != mob.getEntityId()) {
            minecraft.player.yRot = mob.yRot;
            minecraft.player.xRot = mob.xRot;
            selectedSlot = 0;
            minecraft.player.inventory.selected = selectedSlot;
            lastEntityId = mob.getEntityId();
        }

        float yaw = minecraft.player.yRot;
        float pitch = minecraft.player.xRot;
        mob.yRot = yaw;
        mob.xRot = pitch;
        mob.setYHeadRot(yaw);
        mob.setYBodyRot(yaw);
        ...
        minecraft.player.setDeltaMovement(new Vec3(0.0D, 0.0D, 0.0D));
        minecraft.player.onGround = mob.onGround;
    ...
    private static void updateCreativePredictionGravity(Minecraft minecraft,
        ...
        if (minecraft.player != null && minecraft.player.isCreative()) {
            if (creativeFlightEntityId != mob.getEntityId()) {
                restoreCreativePredictionGravity(minecraft);
                creativeFlightEntityId = mob.getEntityId();
    ...
    private static void restoreCreativePredictionGravity(Minecraft minecraft) {
        if (creativeFlightEntityId == -1 || minecraft.world == null) return;
        Entity previous = minecraft.world.getEntity(creativeFlightEntityId);
        if (previous instanceof EntityLiving) { EntityLiving mob = (EntityLiving) (previous); 
    ...
    public static void onMovementInputUpdate(InputUpdateEvent event) {
        ...
        Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.player == null || event.getPlayer() != minecraft.player) return;
    ...
    public static void onInteraction(
        ...
        if (event.isAttack() || event.isUseItem()) {
            ...
            // Keep the local EntityLivingBase swing state for immediate vanilla-like
            // first-person feedback, while cancelling spectator interaction.
            if (event.isAttack()) {
                Minecraft minecraft = Minecraft.getInstance();
                EntityLiving mob = possessedMob(minecraft);
                if (mob != null) mob.swingItem();
    ...
    public static void onRenderLivingPre(RenderLivingEvent.Pre<?, ?> event) {
        if (!(event.getEntity() instanceof EntityLiving)) return; EntityLiving mob = (EntityLiving) (event.getEntity());
        if (!MobPossessionClientState.active()) return;

        Minecraft minecraft = Minecraft.getInstance();
        if (mob.getEntityId() == MobPossessionClientState.entityId()
                && minecraft.getCameraEntity() == mob
                && minecraft.options.thirdPersonView == 0) {
        ...
        VISUAL_EQUIPMENT_BACKUPS.put(mob.getEntityId(), snapshot);
        if (hideHands) {
            mob.setItemSlot(EntityEquipmentSlot.MAINHAND, null);
            mob.setItemSlot(EntityEquipmentSlot.OFFHAND, null);
        }
        if (hideArmor) {
            mob.setItemSlot(EntityEquipmentSlot.HEAD, null);
            mob.setItemSlot(EntityEquipmentSlot.CHEST, null);
            mob.setItemSlot(EntityEquipmentSlot.LEGS, null);
            mob.setItemSlot(EntityEquipmentSlot.FEET, null);
        }
    }
    ...
    public static void onRenderLivingPost(RenderLivingEvent.Post<?, ?> event) {
        ...
        VisualEquipmentSnapshot snapshot =
                VISUAL_EQUIPMENT_BACKUPS.remove(mob.getEntityId());
     ...
     */
    @SubscribeEvent
    public static void onRenderLevelLast(RenderWorldLastEvent event) {
        if (!MobPossessionClientState.active()) {
            return;
        }
        Minecraft minecraft = Minecraft.getInstance();
        ...
        MovingObjectPosition current = minecraft.hitResult;
        if (current != null && current.getType() == MovingObjectPosition.MovingObjectType.ENTITY) {
            return;
        }
        MovingObjectPosition target = mob.pick(5.0D, 1.0F, false);
        if (!(target instanceof BlockRayTraceResult)
                || target.getType() != MovingObjectPosition.MovingObjectType.BLOCK) {
            return;
        } BlockRayTraceResult blockHit = (BlockRayTraceResult) (target);

        fr.vanillainstincts.compat.BlockPos pos = blockHit.getBlockPos();
        fr.vanillainstincts.compat.LegacyBlockState state = minecraft.world.getBlockState(pos);
        if (state.isAir()
                || !minecraft.world.getWorldBorder().isWithinBounds(pos)) {
            return;
        }
        ...
        net.minecraft.client.renderer.IRenderTypeBuffer.Impl buffers = renderer.vanillaInstincts$getRenderBuffers()
                .bufferSource();
        IVertexBuilder lines = buffers.getBuffer(RenderType.lines());
        MatrixStack outlinePose = event.getMatrixStack();
        renderer.vanillaInstincts$renderHitOutline(outlinePose, lines,
                mob, camera.xCoord, camera.yCoord, camera.zCoord, pos, state);
    ...
    public static void renderPossessionHudFromMixin(MatrixStack poseStack) {
        if (!MobPossessionClientState.active()) return;
        Minecraft minecraft = Minecraft.getInstance();
        EntityLiving mob = possessedMob(minecraft);
        if (mob == null) return;
    ...
    private static void blit(MatrixStack poseStack, ResourceLocation texture,
                             int x, int y, int u, int v, int width, int height) {
        bind(texture);
        AbstractGui.blit(x, y, u, v, width, height, 256, 256);
    }

    private static void renderPossessionCrosshair(MatrixStack poseStack,
                                                   Minecraft minecraft) {
    ...
    private static void renderMobHotbar(MatrixStack poseStack,
                                        Minecraft minecraft, EntityLiving mob) {
        ...
        for (int slot = 0; slot < 9; slot++) {
            ItemStack stack = minecraft.player.inventory.getItem(slot);
        ...
        ItemStack offHand = minecraft.player.getHeldItem();
        if (!fr.vanillainstincts.compat.Minecraft110ItemStackCompat.isEmpty(offHand)) {
    ...
    private static void renderMobVitals(MatrixStack poseStack,
                                        Minecraft minecraft, EntityLiving mob) {
        int center = guiWidth(minecraft) / 2;
        ...
        minecraft.font.drawShadow(hearts,
    ...
    private static void renderHearts(MatrixStack poseStack,
                                     Minecraft minecraft, EntityLiving mob,
                                     int x, int y) {
    ...
    private static float displayedAbsorption(EntityLiving mob) {
        float amount = Math.max(0.0F, mob.getAbsorptionAmount());
        net.minecraft.potion.PotionEffect effect = mob.getEffect(net.minecraft.potion.Potion.absorption);
        if (effect != null) {
    ...
    private static void drawAbsorptionHeart(MatrixStack poseStack, int x,
    ...
    private static void drawHeart(MatrixStack poseStack, int x, int y,
                                  float remaining) {
        blit(poseStack, GUI_ICONS, x, y, 16, 0, 9, 9);
        if (remaining >= 2.0F) {
    ...
    private static void renderArmor(MatrixStack poseStack, int armor,
                                    int x, int y) {
        if (armor <= 0) return;
    ...
    private static void renderFood(MatrixStack poseStack, Minecraft minecraft,
                                   int right, int y) {
        if (minecraft.player == null) return;
    ...
    private static void renderAir(MatrixStack poseStack, EntityLiving mob,
                                  int right, int y) {
    ...
    private static void renderFirstPersonMobArm(MatrixStack poseStack,
                                                 Minecraft minecraft,
                                                 EntityLiving mob) {
        net.minecraft.client.renderer.entity.EntityRenderer renderer = minecraft.getEntityRenderDispatcher().getRenderer(mob);
        if (!(renderer instanceof LivingRenderer<?, ?>)
                || !(((LivingRenderer<?, ?>) (renderer)).getModel() instanceof BipedModel<?>)) {
            return;
        } LivingRenderer<?, ?> livingRenderer = (LivingRenderer<?, ?>) (renderer);BipedModel<?> model = (BipedModel<?>) (livingRenderer.getModel());
        ...
        fr.vanillainstincts.compat.Minecraft112Compat.scale(poseStack, 3.4F, -3.4F, 3.4F);
        IRenderTypeBuffer.Impl buffers = minecraft.renderBuffers().bufferSource();
        IVertexBuilder consumer = buffers.getBuffer(RenderType.entityCutoutNoCull(texture));
    ...
    private static final class ComponentName {
        private static void drawCenteredShadow(MatrixStack poseStack,
                                               Minecraft minecraft,
                                               net.minecraft.util.text.ITextComponent text,
                                               int center, int y, int color) {
            float x = center - minecraft.font.width(text.getString()) / 2.0F;
            minecraft.font.drawShadow(text.getString(), x, y, color);
        }
    }

    private static void applyFirstPersonSwing(MatrixStack pose, EntityLiving mob) {
        float swing = MathHelper.clamp(mob.getAttackAnim(1.0F), 0.0F, 1.0F);
        ...
        float arc = MathHelper.sin(root * (float) Math.PI);
        float follow = MathHelper.sin(swing * (float) Math.PI);
    ...
    private static void predictLocalMovement(EntityLiving mob, int flags, float yaw,
        ...
        if (creativeControl) {
            ...
            Vec3 right = new Vec3(-facing.zCoord, 0.0D, facing.xCoord);
            Vec3 desired = fr.vanillainstincts.compat.Minecraft112Compat.scale(facing, forward)
            ...
            if (desired.lengthSqr() < 1.0E-5D) {
                ...
                Vec3 predicted = fr.vanillainstincts.compat.Minecraft112Compat.scale(desired.normalize(), sprint ? 0.45D : 0.25D);
        ...
        double baseSpeed = mob.getAttribute(SharedMonsterAttributes.MOVEMENT_SPEED) == null
                ? 0.10D : MathHelper.clamp(mob.getAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).getValue(),
                0.02D, 1.50D);
        ...
        if (free3d) {
            ...
            predicted = fr.vanillainstincts.compat.Minecraft112Compat.scale(look, forward)
                    .add(fr.vanillainstincts.compat.Minecraft112Compat.scale(right, -strafe))
                    .add(0.0D, jump ? 1.0D : descend ? -1.0D : 0.0D, 0.0D);
            if (predicted.lengthSqr() > 1.0E-5D) {
                predicted = fr.vanillainstincts.compat.Minecraft112Compat.scale(predicted.normalize(), speed);
            } else {
                return;
            }
            ...
            Vec3 facing = Vec3.directionFromRotation(0.0F, yaw);
            Vec3 right = new Vec3(-facing.zCoord, 0.0D, facing.xCoord);
            Vec3 desired = fr.vanillainstincts.compat.Minecraft112Compat.scale(facing, forward).add(fr.vanillainstincts.compat.Minecraft112Compat.scale(right, -strafe));
            ...
            if (jump && mob.onGround) {
                ...
                dy = Math.max(0.0D, mob.getMotion().y) + 0.20D;
            }
    ...
    private static void applyPredictedMove(EntityLiving mob, Vec3 movement,
        ...
        mob.move(net.minecraft.entity.MoverType.PLAYER, movement);
    }

    private static Vec3 predictSneakEdge(EntityLiving mob, double dx, double dz) {
        AxisAlignedBB box = mob.getBoundingBox();
        double safeX = dx;
        double safeZ = dz;
        if (mob.fr.vanillainstincts.compat.Minecraft112Compat.noCollision(world, mob, box.move(dx, -0.55D, 0.0D))) safeX = 0.0D;
        if (mob.fr.vanillainstincts.compat.Minecraft112Compat.noCollision(world, mob, box.move(0.0D, -0.55D, dz))) safeZ = 0.0D;
    ...
    private static EntityLiving possessedMob(Minecraft minecraft) {
        if (minecraft.world == null || !MobPossessionClientState.active()) {
            return null;
        }
        Entity entity = minecraft.world.getEntity(
    ...
    private static class VisualEquipmentSnapshot {
        ...
        public VisualEquipmentSnapshot(ItemStack mainHand, ItemStack offHand, ItemStack head, ItemStack chest, ItemStack legs, ItemStack feet) {
        ...
        public ItemStack mainHand() { return this.mainHand; }

        public ItemStack offHand() { return this.offHand; }

        public ItemStack head() { return this.head; }

        public ItemStack chest() { return this.chest; }

        public ItemStack legs() { return this.legs; }

        public ItemStack feet() { return this.feet; }

        @Override
        public boolean equals(Object other) {
            if (this == other) return true;
            if (!(other instanceof VisualEquipmentSnapshot)) return false;
            VisualEquipmentSnapshot that = (VisualEquipmentSnapshot) other;
            return java.util.Objects.equals(this.mainHand, that.mainHand) && java.util.Objects.equals(this.offHand, that.offHand) && java.util.Objects.equals(this.head, that.head) && java.util.Objects.equals(this.chest, that.chest) && java.util.Objects.equals(this.legs, that.legs) && java.util.Objects.equals(this.feet, that.feet);
        }

        @Override
        public int hashCode() { return java.util.Objects.hash(this.mainHand, this.offHand, this.head, this.chest, this.legs, this.feet); }

        @Override
        public String toString() {
            return "VisualEquipmentSnapshot[" + "mainHand=" + this.mainHand + ", " + "offHand=" + this.offHand + ", " + "head=" + this.head + ", " + "chest=" + this.chest + ", " + "legs=" + this.legs + ", " + "feet=" + this.feet + "]";
        }

        private static VisualEquipmentSnapshot capture(EntityLiving mob) {
            return new VisualEquipmentSnapshot(
                    mob.getItemBySlot(EntityEquipmentSlot.MAINHAND).copy(),
                    mob.getItemBySlot(EntityEquipmentSlot.OFFHAND).copy(),
                    mob.getItemBySlot(EntityEquipmentSlot.HEAD).copy(),
                    mob.getItemBySlot(EntityEquipmentSlot.CHEST).copy(),
                    mob.getItemBySlot(EntityEquipmentSlot.LEGS).copy(),
                    mob.getItemBySlot(EntityEquipmentSlot.FEET).copy());
        }

        private void restore(EntityLiving mob) {
