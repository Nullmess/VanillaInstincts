...
public final class GolemConstructionController {
     ...
     */
    public static boolean isCommittedPlan(MobEntity mob, LivingEntity target,
     ...
     */
    public static boolean maintain(MobEntity mob, LivingEntity target,
                                   GolemConstructionState combat,
        ...
        if (anchorDistance > GolemRules.CONSTRUCTION_COLUMN_LOCK_DISTANCE_SQR) {
            Vector3d center = columnCenter(anchor, mob.getY());
        ...
        if (!obstructions.isEmpty()) {
            ...
            if (hasUnbreakable && mob instanceof IronGolemEntity
                    && combat.constructionBlocksPlaced() > 0
                    && combat.constructionObstructionDuration(gameTime)
    ...
    public static boolean contribute(MobEntity mob, LivingEntity target,
                                     GolemConstructionState combat,
                                     MobDecisionPlan plan,
                                     ServerWorld level, long gameTime) {
        Optional<Capabilities> optional = capabilities(mob);
        if (!optional.isPresent() || target == null || !target.isAlive()
        ...
        if (activeSession) {
            ...
            if (anchor.isPresent()) {
                Vector3d destination = columnCenter(anchor.get(), placementY(mob));
                double anchorDistance = horizontalDistanceSqr(mob.position(),
                        destination);
                if (combat.constructionBlocksPlaced() == 0
                        && anchorDistance
        ...
        if (!anchor.isPresent()) {
            ...
            anchor = findConstructionAnchor(level, mob, reference,
                    capabilities, mob instanceof IronGolemEntity);
            if (!anchor.isPresent()) {
                combat.setBuildCooldown(gameTime,
                        GolemRules.CONSTRUCTION_FAILED_RETRY_TICKS);
                return true;
    ...
    public static boolean canUseEmergencyConstruction(MobEntity mob) {
        return capabilities(mob).isPresent();
    }

    public static int initialBudget(MobEntity mob) {
        return capabilities(mob).map(value -> value.blockBudget).orElse(0);
    }

    public static int maximumRise(MobEntity mob) {
        return capabilities(mob).map(value -> value.maxRise).orElse(0);
    }

    public static int plannedGoalY(MobEntity mob, LivingEntity target) {
        return capabilities(mob).map(value -> desiredGoalY(target, value))
                .orElse(target.blockPosition().getY());
    }

    public static BlockState scaffoldFor(MobEntity mob, ServerWorld level) {
        return Blocks.COBBLESTONE.defaultBlockState();
    }

    public static boolean canJumpAttack(MobEntity mob, LivingEntity target) {
        return capabilities(mob).map(value ->
                canJumpAttack(mob, target, value)).orElse(false);
    }

    public static boolean placeVirtualPillarBlock(ServerWorld level, MobEntity mob,
                                                   GolemConstructionState combat,
                                                   LivingEntity target,
                                                   long gameTime) {
    ...
    public static Optional<BlockPos> blockingAscentBlock(ServerWorld level,
                                                          MobEntity mob,
                                                          BlockPos anchor) {
        BlockPos placement = new BlockPos(anchor.getX(), placementY(mob),
                anchor.getZ());
    ...
    public static int blockingAscentBlockCount(ServerWorld level, MobEntity mob,
                                                BlockPos anchor) {
        BlockPos placement = new BlockPos(anchor.getX(), placementY(mob),
                anchor.getZ());
        return capabilities(mob).map(value ->
    ...
    public static double ascentClearanceWidth(MobEntity mob) {
    ...
    public static int constructionAnchorOffset(MobEntity mob) {
        return capabilities(mob).map(value -> value.anchorOffset).orElse(0);
    }

    private static void offerJumpProbe(MobEntity mob, LivingEntity target,
                                       GolemConstructionState combat,
                                       MobDecisionPlan plan,
                                       Capabilities capabilities,
                                       long gameTime) {
        Vector3d impulse = jumpImpulse(mob, target, capabilities);
    ...
    private static void refreshPersistentPlan(MobEntity mob, LivingEntity target,
                                              GolemConstructionState combat,
                                              Capabilities capabilities,
                                              long gameTime) {
        combat.initializeConstructionPlan(target.blockPosition(),
        ...
        double drift = horizontalDistanceSqr(Vector3d.atBottomCenterOf(origin.get()),
                target.position());
    ...
    private static boolean handleSustainedTargetDrift(
            MobEntity mob, LivingEntity target, GolemConstructionState combat,
        ...
        double anchorDistance = combat.constructionAnchor()
                .map(anchor -> horizontalDistanceSqr(
                        Vector3d.atBottomCenterOf(anchor), target.position()))
                .orElse(0.0D);
        if (combat.constructionBlocksPlaced() == 0
                && combat.constructionReplans()
                < GolemRules.CONSTRUCTION_MAX_REPLANS) {
    ...
    private static int cappedGoalY(MobEntity mob, GolemConstructionState combat,
                                   LivingEntity target,
                                   Capabilities capabilities) {
    ...
    private static int desiredGoalY(LivingEntity target,
                                    Capabilities capabilities) {
        int targetFeetY = new BlockPos(target.getX(),
    ...
    private static boolean canResumeMeleeFromCurrentLevel(
            MobEntity mob, LivingEntity target, Capabilities capabilities) {
        double horizontal = horizontalDistanceSqr(mob.position(),
                target.position());
        double allowed = capabilities.completionHorizontalReach
                + (mob.getBbWidth() + target.getBbWidth()) * 0.5D;
    ...
    private static boolean shouldUseFatalFallDemolition(
            LivingEntity target, GolemConstructionState combat, MobEntity mob) {
        if (!(target instanceof PlayerEntity) || combat == null || mob == null
                || ((PlayerEntity) (target)).isCreative() || ((PlayerEntity) (target)).isSpectator()
                || ((PlayerEntity) (target)).abilities.flying
                || ((PlayerEntity) (target)).hasEffect(Effects.SLOW_FALLING)) {
            return false;
    ...
    private static boolean tryBreakFatalPerchSupport(
            ServerWorld level, MobEntity mob, LivingEntity target,
            GolemConstructionState combat, Capabilities capabilities,
            long gameTime) {
        if (!mob.isOnGround() || !combat.obstacleReady(gameTime)) {
            return false;
        }
        List<BlockPos> supports = supportingPerchBlocks(level, target);
        if (supports.isEmpty()) {
            return !target.isOnGround() && target.getDeltaMovement().y < 0.0D;
        }
    ...
    private static List<BlockPos> supportingPerchBlocks(
            ServerWorld level, LivingEntity target) {
        AxisAlignedBB body = target.getBoundingBox();
        ...
        for (int y = maxY; y >= minY; y--) {
            for (int x = minX; x <= maxX; x++) {
                for (int z = minZ; z <= maxZ; z++) {
                    ...
                    AxisAlignedBB collision = shape.bounds().move(x, y, z);
    ...
    private static boolean canBreakFatalPerchBlock(
            ServerWorld level, MobEntity mob, BlockPos position,
            Capabilities capabilities) {
    ...
    private static boolean canJumpAttack(MobEntity mob, LivingEntity target,
                                         Capabilities capabilities) {
        double vertical = target.getY() - mob.getY();
        ...
        return vertical >= 0.75D
                ...
                && mob.isOnGround() && mob.canSee(target);
    }

    private static Vector3d jumpImpulse(MobEntity mob, LivingEntity target,
                                    Capabilities capabilities) {
        Vector3d direction = target.position().subtract(mob.position())
                .multiply(1.0D, 0.0D, 1.0D);
        if (direction.lengthSqr() > 1.0E-8D) {
            direction = direction.normalize().scale(
                    GolemRules.CONSTRUCTION_JUMP_HORIZONTAL);
        }
        return new Vector3d(direction.x, capabilities.jumpVelocity, direction.z);
    }

    private static Optional<BlockPos> findConstructionAnchor(
            ServerWorld level, MobEntity mob, BlockPos targetReference,
        ...
        int maximumOffset = mob instanceof IronGolemEntity
        ...
        candidates.sort(Comparator.comparingDouble(position -> {
            double targetDistance = horizontalDistanceSqr(
                    Vector3d.atBottomCenterOf(position),
                    Vector3d.atBottomCenterOf(targetReference));
        ...
        BlockPos local = new BlockPos(mob.getX(), y, mob.getZ());
    ...
    private static Optional<BlockPos> firstUsableAnchor(
            ServerWorld level, MobEntity mob, List<BlockPos> candidates,
    ...
    private static boolean placeVirtualBlock(ServerWorld level, MobEntity mob,
                                             GolemConstructionState combat,
                                             BlockPos position,
                                             BlockState scaffold,
                                             LivingEntity target,
    ...
    private static boolean canPlaceScaffold(ServerWorld level, MobEntity mob,
                                            BlockPos position,
                                            BlockState scaffold) {
    ...
    private static List<BlockPos> findBlockingAscentBlocks(
            ServerWorld level, MobEntity mob, BlockPos anchor,
        ...
        for (int y = minY; y <= maxY; y++) {
            for (int x = minX; x <= maxX; x++) {
                for (int z = minZ; z <= maxZ; z++) {
                    ...
                    AxisAlignedBB collision = shape.bounds().move(pos.getX(),
                            pos.getY(), pos.getZ());
    ...
    private static boolean canBreakAscentBlock(ServerWorld level, MobEntity mob,
                                                BlockPos position,
                                                Capabilities capabilities) {
        if (!canModifyWorld(level, mob) || !level.isLoaded(position)
                || !level.isInWorldBounds(position)
    ...
    private static boolean breakAscentBlock(ServerWorld level, MobEntity mob,
                                             BlockPos position) {
        BlockState state = level.getBlockState(position);
        if (state.isAir()) {
            return false;
    ...
    private static int breakAscentBlocks(ServerWorld level, MobEntity mob,
    ...
    private static boolean canModifyWorld(ServerWorld level, MobEntity mob) {
     ...
     */
    public static boolean requiresPhysicalMaintenance(MobEntity mob,
                                                       LivingEntity target,
                                                       ServerWorld level) {
    ...
    private static boolean maintainLogicalDescent(MobEntity mob,
                                                   GolemConstructionState combat,
                                                   ServerWorld level,
                                                   long gameTime) {
        if (!canModifyWorld(level, mob)) {
        ...
        if (!mob.isOnGround() || !combat.buildReady(gameTime)) {
            return true;
        }
    ...
    private static BlockPos supportingBlock(MobEntity mob) {
        return new BlockPos(mob.getX(),
                mob.getBoundingBox().minY - 0.05D, mob.getZ());
    }

    private static void lookAtConstructionBlock(MobEntity mob, BlockPos block) {
        mob.getLookControl().setLookAt(block.getX() + 0.5D,
                block.getY() + 0.35D, block.getZ() + 0.5D,
                35.0F, 45.0F);
    }

    private static void beginPhysicalPillarJump(MobEntity mob,
                                                GolemConstructionState combat,
                                                BlockPos placement,
                                                double jumpVelocity,
                                                long gameTime) {
        mob.getNavigation().stop();
        Vector3d movement = mob.getDeltaMovement();
    ...
    private static void steerTowardColumn(MobEntity mob, BlockPos placement) {
        Vector3d center = new Vector3d(placement.getX() + 0.5D, mob.getY(),
                placement.getZ() + 0.5D);
        Vector3d correction = center.subtract(mob.position())
                .multiply(1.0D, 0.0D, 1.0D);
        if (correction.lengthSqr() > 1.0E-8D) {
            correction = correction.normalize().scale(
                    GolemRules.CONSTRUCTION_AIR_CENTERING_STRENGTH);
        }
        Vector3d current = mob.getDeltaMovement();
        mob.setDeltaMovement(current.x * 0.35D + correction.x,
                current.y, current.z * 0.35D + correction.z);
        mob.hasImpulse = true;
    }

    private static void dampHorizontalMotion(MobEntity mob, double factor) {
        Vector3d movement = mob.getDeltaMovement();
    ...
    private static int placementY(MobEntity mob) {
        return new BlockPos(mob.getX(),
                mob.getBoundingBox().minY + 0.05D, mob.getZ()).getY();
    }

    private static BlockPos placementUnderFeet(MobEntity mob) {
        return new BlockPos(mob.getX(),
                mob.getBoundingBox().minY + 0.05D, mob.getZ());
    }

    private static Vector3d columnCenter(BlockPos anchor, double y) {
        return new Vector3d(anchor.getX() + 0.5D, y,
                anchor.getZ() + 0.5D);
    }

    private static Optional<Capabilities> capabilities(MobEntity mob) {
        if (!(mob instanceof IronGolemEntity)) {
            return Optional.empty();
        }
    ...
    private static double horizontalDistanceSqr(Vector3d first, Vector3d second) {
