...
public final class GolemConstructionController {
    ...
    public static boolean maintain(EntityLiving mob, EntityLivingBase target,
        ...
        if (pending.isPresent()) {
            ...
            if (fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(mob).minY >= blockTop + 0.015D) {
        ...
        double anchorDistance = horizontalDistanceSqr(fr.vanillainstincts.compat.Minecraft17Compat.position(mob),
        ...
        BlockPos placement = new BlockPos(anchor.getX(), placementY(mob),
                anchor.getZ());
        LegacyBlockState scaffold = scaffoldFor(mob, level);
    ...
    public static boolean contribute(EntityLiving mob, EntityLivingBase target,
        ...
        double horizontal = horizontalDistanceSqr(fr.vanillainstincts.compat.Minecraft17Compat.position(mob),
        ...
        combat.sampleMovement(fr.vanillainstincts.compat.Minecraft17Compat.position(mob), gameTime,
    ...
    public static LegacyBlockState scaffoldFor(EntityLiving mob, WorldServer level) {
        return fr.vanillainstincts.compat.Minecraft17Compat.defaultState(Blocks.cobblestone);
    }
    ...
    private static boolean handleSustainedTargetDrift(
        ...
        double anchorDistance = combat.constructionAnchor()
                .map(anchor -> horizontalDistanceSqr(
                        Minecraft115VectorCompat.atBottomCenterOf(anchor), fr.vanillainstincts.compat.Minecraft17Compat.position(target)))
                .orElse(0.0D);
        if (combat.constructionBlocksPlaced() == 0
                && combat.constructionReplans()
                < GolemRules.CONSTRUCTION_MAX_REPLANS) {
    ...
    private static int desiredGoalY(EntityLivingBase target,
        ...
        int targetFeetY = new BlockPos(target.posX,
                fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(target).minY + 0.05D, target.posZ).getY();
    ...
    private static boolean hasReachedPlannedHeight(
        ...
        if (combat.constructionGoalY() <= 0
                || fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(mob).minY
        ...
        return horizontal <= allowed * allowed
                && fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(target).minY
                <= fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(mob).minY
    ...
    private static boolean canResumeMeleeFromCurrentLevel(
        ...
        return horizontal <= allowed * allowed
                && fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(target).minY
                <= fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(mob).minY + 0.90D
    ...
    private static boolean shouldUseFatalFallDemolition(
        ...
        if (!(target instanceof EntityPlayer) || combat == null || mob == null
                || ((EntityPlayer) (target)).capabilities.isCreativeMode || fr.vanillainstincts.compat.Minecraft17Compat.isSpectator((EntityPlayer) (target))
        ...
        double baseY = combat.constructionStartY() == 0.0D
                ? mob.posY : combat.constructionStartY();
        double dropHeight = fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(target).minY - baseY;
    ...
    private static List<BlockPos> supportingPerchBlocks(
        ...
        AxisAlignedBB body = fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(target);
        double supportTop = body.minY + 0.04D;
        double supportBottom = body.minY
                - GolemRules.CONSTRUCTION_GOLEM_FATAL_SUPPORT_DEPTH;
        AxisAlignedBB supportProbe = AxisAlignedBB.getBoundingBox(body.minX + 0.02D, supportBottom,
        ...
        for (int y = maxY; y >= minY; y--) {
            for (int x = minX; x <= maxX; x++) {
                for (int z = minZ; z <= maxZ; z++) {
                    ...
                    AxisAlignedBB shape = fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, pos).getCollisionBoundingBox(level, pos);
                    if (shape == null) {
                        continue;
                    }
    ...
    private static boolean canBreakFatalPerchBlock(
            ...
            Capabilities capabilities) {
        if (!canModifyWorld(level, mob) || !fr.vanillainstincts.compat.Minecraft17Compat.isBlockLoaded(level, position)
        ...
        float hardness = state.getBlockHardness(level, position);
    ...
    private static boolean canJumpAttack(EntityLiving mob, EntityLivingBase target,
                                         Capabilities capabilities) {
        double vertical = target.posY - mob.posY;
        double bodyGap = fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(target).minY
                - fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(mob).maxY;
    ...
    private static Vec3 jumpImpulse(EntityLiving mob, EntityLivingBase target,
                                    Capabilities capabilities) {
        Vec3 direction = fr.vanillainstincts.compat.Minecraft17Compat.position(target).subtract(fr.vanillainstincts.compat.Minecraft17Compat.position(mob))
                ;
    ...
    private static Optional<BlockPos> findConstructionAnchor(
        ...
        candidates.sort(Comparator.comparingDouble(position -> {
            ...
            double mobDistance = horizontalDistanceSqr(fr.vanillainstincts.compat.Minecraft17Compat.position(mob),
                    Minecraft115VectorCompat.atBottomCenterOf(position));
        ...
        fallback.sort(Comparator.comparingDouble(position ->
                horizontalDistanceSqr(fr.vanillainstincts.compat.Minecraft17Compat.position(mob),
                        Minecraft115VectorCompat.atBottomCenterOf(position))));
    ...
    private static Optional<BlockPos> firstUsableAnchor(
            ...
            LegacyBlockState scaffold, Capabilities capabilities) {
    ...
    private static boolean placeVirtualBlock(WorldServer level, EntityLiving mob,
        ...
        if (!TemporaryGolemBlockRegistry.register(level, position, scaffold,
            ...
            WorldPermissionService.setBlock(level, mob, position,
                    fr.vanillainstincts.compat.Minecraft17Compat.defaultState(Blocks.air),
                    3,
    ...
    private static boolean canPlaceScaffold(WorldServer level, EntityLiving mob,
        ...
        return fr.vanillainstincts.compat.Minecraft17Compat.isBlockLoaded(level, position) && fr.vanillainstincts.compat.Minecraft17Compat.isBlockLoaded(level, support)
                && validHeight(position)
                && Minecraft112Compat.isAir(fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, position))
                && !fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, position).getBlock().getMaterial().isLiquid()
                && fr.vanillainstincts.compat.Minecraft17Compat.getTileEntity(level, position) == null
                && Minecraft112Compat.entityCanStandOn(fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, support),
                level, support, mob)
                && fr.vanillainstincts.compat.Minecraft17Compat.canPlaceBlockAt(scaffold.getBlock(), level, position);
    }
    ...
    private static List<BlockPos> findBlockingAscentBlocks(
        ...
        AxisAlignedBB swept = AxisAlignedBB.getBoundingBox(centerX - halfWidth,
                Math.min(fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(mob).minY + 0.02D,
        ...
        for (int y = minY; y <= maxY; y++) {
            for (int x = minX; x <= maxX; x++) {
                for (int z = minZ; z <= maxZ; z++) {
                    ...
                    AxisAlignedBB shape = state.getCollisionBoundingBox(level, pos);
                    if (shape == null) {
                        continue;
                    }
    ...
    private static boolean canModifyWorld(WorldServer level, EntityLiving mob) {
        return level.getGameRules().getGameRuleBooleanValue("mobGriefing")
    ...
    private static BlockPos supportingBlock(EntityLiving mob) {
        return new BlockPos(mob.posX,
                fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(mob).minY - 0.05D, mob.posZ);
    }
    ...
    private static void steerTowardColumn(EntityLiving mob, BlockPos placement) {
        ...
        Vec3 correction = Minecraft112Compat.multiply(
                center.subtract(fr.vanillainstincts.compat.Minecraft17Compat.position(mob)), 1.0D, 0.0D, 1.0D);
    ...
    private static int placementY(EntityLiving mob) {
        return new BlockPos(mob.posX,
                fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(mob).minY + 0.05D, mob.posZ).getY();
    }
