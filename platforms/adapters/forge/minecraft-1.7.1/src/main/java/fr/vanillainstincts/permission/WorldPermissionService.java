...
public final class WorldPermissionService {
    public static class BlockChange {
        ...
        public BlockChange(BlockPos pos, LegacyBlockState state, int flags) {
    ...
    private static PermissionDecision check(WorldServer level,
        ...
        boolean chunkLoaded = fr.vanillainstincts.compat.Minecraft17Compat.isBlockLoaded(level, pos);
        ...
        boolean spawnProtected = config.protectSpawnArea()
                ...
                && !(actor instanceof EntityPlayer)
                && level.provider.dimensionId == 0
                && horizontalDistance(new BlockPos(level.getSpawnPoint()), pos)
        ...
        boolean protectedBlock = chunkLoaded
                && Minecraft115TagCompat.blockStateIs(fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, pos), PROTECTED_FROM_AI);
        ...
        PermissionDecision preliminary = WorldPermissionPolicy.evaluate(action,
                ...
                chunkLoaded, fr.vanillainstincts.compat.Minecraft17Compat.withinWorldBorder(level, pos),
    ...
    public static boolean setBlock(WorldServer level, Entity actor,
                                   BlockPos pos, LegacyBlockState replacement,
                                   int flags, WorldActionType action) {
        if (!check(level, actor, action, pos, replacement).allowed()) {
            return false;
        }
        if (fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, pos).equals(replacement)) {
            clearDenial(level, pos, action);
            return true;
        }
        LegacySnapshot rollbackSnapshot = legacySnapshot(level, pos, flags);
        if (!fr.vanillainstincts.compat.Minecraft17Compat.setBlockState(level, pos, replacement, flags)) {
            rememberDenial(level, pos, action, level.getTotalWorldTime(),
        ...
        if (action != WorldActionType.TEMPORARY_CLEANUP) {
             ...
             */
            LegacySnapshot eventSnapshot = actor instanceof EntityPlayer
                    ? rollbackSnapshot
                    : legacySnapshot(level, pos, flags);
    ...
    public static boolean setPlayerBlocksAtomically(WorldServer level,
        ...
        for (BlockChange change : changes) {
            if (!fr.vanillainstincts.compat.Minecraft17Compat.isBlockLoaded(level, change.pos())
                    || !fr.vanillainstincts.compat.Minecraft17Compat.withinWorldBorder(level, change.pos())) {
                return false;
            }
        }

        List<LegacySnapshot> snapshots = new ArrayList<>(changes.size());
        for (BlockChange change : changes) {
            snapshots.add(legacySnapshot(level, change.pos(), change.flags()));
        }
        for (int i = 0; i < changes.size(); i++) {
            BlockChange change = changes.get(i);
            if (!fr.vanillainstincts.compat.Minecraft17Compat.setBlockState(level, change.pos(), change.state(), change.flags())) {
    ...
    public static boolean setBlocksAtomically(WorldServer level,
        ...
        List<BlockChange> effective = changes.stream()
                .filter(change -> !fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, change.pos())
                        .equals(change.state()))
        ...
        List<LegacySnapshot> snapshots = new ArrayList<>(effective.size());
        for (BlockChange change : effective) {
            snapshots.add(legacySnapshot(level, change.pos(), change.flags()));
        }
    ...
    public static boolean destroyBlock(WorldServer level,
        ...
        PermissionDecision decision = check(level, actor,
                ...
                fr.vanillainstincts.compat.Minecraft17Compat.defaultState(net.minecraft.init.Blocks.air));
        ...
        boolean changed = fr.vanillainstincts.compat.Minecraft17Compat.destroyBlock(level, pos, drop);
        if (changed) {
    ...
    private static LegacySnapshot legacySnapshot(WorldServer level, BlockPos pos, int flags) {
        return new LegacySnapshot(level, pos,
                fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, pos), flags);
    }

    private static void restoreReverse(List<LegacySnapshot> snapshots,
                                       int last) {
    ...
    /** Minimal rollback snapshot for Forge 1.7.2, before Forge BlockSnapshot existed. */
    private static final class LegacySnapshot {
        ...
        private LegacySnapshot(WorldServer level, BlockPos pos, LegacyBlockState state, int flags) {
            this.level = level;
            this.pos = immutableBlockPos(pos);
            this.state = state;
            this.flags = flags;
        }

        private void restore() {
            fr.vanillainstincts.compat.Minecraft17Compat.setBlockState(level, pos, state, flags);
        }
    }
    private static boolean legacyEntityDestroyHook(EntityLivingBase actor, BlockPos pos,
        ...
        try {
            java.lang.reflect.Method method = ForgeEventFactory.class.getMethod(
                    "onEntityDestroyBlock", EntityLivingBase.class, BlockPos.class, LegacyBlockState.class);
