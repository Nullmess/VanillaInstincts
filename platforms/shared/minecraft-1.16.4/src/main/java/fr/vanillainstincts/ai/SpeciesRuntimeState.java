...
public final class SpeciesRuntimeState {
    ...
    public static SpeciesRuntimeState load(MobEntity mob) {
        ...
        state.creeperWatchedTicks = bounded(tag.getInt("creeper_watched_ticks"), 0, 400);
        state.creeperUnwatchedTicks = bounded(tag.getInt("creeper_unwatched_ticks"), 0, 400);
        ...
        state.creeperStalledSamples = bounded(tag.getInt("creeper_stalled_samples"), 0, 100);
    ...
    public void save(MobEntity mob) {
        if (!dirty) {
            return;
        }
    ...
    public void sampleCreeperMovement(Vector3d position, long gameTime,
        ...
        Vector3d previous = new Vector3d(creeperMovementX, creeperMovementY,
                creeperMovementZ);
    ...
    private void setCreeperMovementSample(Vector3d position, long gameTime) {
    ...
    private void sanitize(long gameTime) {
        if (!finite(new Vector3d(creeperMovementX, creeperMovementY,
                creeperMovementZ))) {
