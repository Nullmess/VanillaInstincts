...
public final class MobEcologyController {
    ...
    public static void contribute(AnimalEntity animal, MobDecisionPlan plan,
        ...
        if (animal == null || plan == null || level == null
                ...
                || (animal instanceof TameableEntity && ((TameableEntity) (animal)).isTame())
        ...
        Vector3d requested = fleeDestination(animal, source);
        if (requested == null) return;
        Vector3d destination = SafePositionFinder.resolveGroundDestination(
    ...
    public static Optional<EcologyThreat> nearestAvoidedThreat(
            AnimalEntity animal, ServerWorld level) {
        ...
        AxisAlignedBB area = animal.getBoundingBox().inflate(radius);
    ...
    public static boolean isHuntRelation(MobEntity predator,
                                         LivingEntity prey) {
    ...
    public static Vector3d fleeDestination(AnimalEntity animal, LivingEntity threat) {
        if (animal == null || threat == null) return null;
        Vector3d away = animal.position().subtract(threat.position());
        Vector3d horizontal = new Vector3d(away.x, 0.0D, away.z);
        if (horizontal.lengthSqr() < 1.0E-6D) {
            double sign = (animal.getId() & 1) == 0 ? 1.0D : -1.0D;
            horizontal = new Vector3d(sign, 0.0D, 0.5D);
        }
        horizontal = horizontal.normalize();
        double side = Math.floorMod(animal.getId(), 3) - 1;
        Vector3d lateral = new Vector3d(-horizontal.z, 0.0D, horizontal.x)
                .scale(side * 0.8D);
    ...
    public static class EcologyThreat {
        ...
        public EcologyThreat(LivingEntity entity, RelationDecision relation, int inspectedCandidates) {
            this.entity = entity;
