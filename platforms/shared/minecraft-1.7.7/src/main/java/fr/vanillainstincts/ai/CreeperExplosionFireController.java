...
public final class CreeperExplosionFireController {
    ...
    private static void ignite(WorldServer level, EntityCreeper creeper,
                               BlockPos center) {
        LegacyBlockState fire = fr.vanillainstincts.compat.Minecraft17Compat.defaultState(Blocks.fire);
        int placed = 0;
        ...
        for (int i = 0; i < OFFSETS.length && placed < MAX_FIRE_BLOCKS; i++) {
            ...
            if (!fr.vanillainstincts.compat.Minecraft17Compat.isBlockLoaded(level, pos) || !fr.vanillainstincts.compat.Minecraft112Compat.isAir(fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, pos))
                    || !fr.vanillainstincts.compat.Minecraft17Compat.canPlaceBlockAt(fire.getBlock(), level, pos)) {
                continue;
            }
