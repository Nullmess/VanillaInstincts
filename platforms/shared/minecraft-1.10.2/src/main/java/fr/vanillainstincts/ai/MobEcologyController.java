...
public final class MobEcologyController {
    ...
    public static Optional<EcologyThreat> nearestAvoidedThreat(
        ...
        AxisAlignedBB area = animal.getEntityBoundingBox().expandXyz(radius);
        List<EntityLivingBase> nearby = new ArrayList<>(level.getEntitiesWithinAABB(
    ...
    public static Vec3d fleeDestination(EntityAnimal animal, EntityLivingBase threat) {
        if (animal == null || threat == null) return null;
        Vec3d away = animal.getPositionVector().subtract(threat.getPositionVector());
        Vec3d horizontal = new Vec3d(away.xCoord, 0.0D, away.zCoord);
        ...
        Vec3d lateral = new Vec3d(-horizontal.zCoord, 0.0D, horizontal.xCoord)
                .scale(side * 0.8D);
        return animal.getPositionVector().add(horizontal.scale(
