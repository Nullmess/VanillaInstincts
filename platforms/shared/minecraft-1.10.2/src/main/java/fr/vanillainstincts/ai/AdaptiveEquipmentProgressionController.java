...
public final class AdaptiveEquipmentProgressionController {
    ...
    private static ItemStack armorFor(Tier tier, EntityEquipmentSlot slot) {
        Item item = fr.vanillainstincts.compat.LegacyJava8.eval(() -> { switch ((tier)) { case IRON:  return fr.vanillainstincts.compat.LegacyJava8.eval(() -> { switch ((slot)) { case HEAD:  return Items.IRON_HELMET; case CHEST:  return Items.IRON_CHESTPLATE; case LEGS:  return Items.IRON_LEGGINGS; case FEET:  return Items.IRON_BOOTS; default:  return net.minecraft.item.Item.getItemFromBlock(net.minecraft.init.Blocks.AIR); } }); case DIAMOND:  return fr.vanillainstincts.compat.LegacyJava8.eval(() -> { switch ((slot)) { case HEAD:  return Items.DIAMOND_HELMET; case CHEST:  return Items.DIAMOND_CHESTPLATE; case LEGS:  return Items.DIAMOND_LEGGINGS; case FEET:  return Items.DIAMOND_BOOTS; default:  return net.minecraft.item.Item.getItemFromBlock(net.minecraft.init.Blocks.AIR); } }); case NETHERITE:  return fr.vanillainstincts.compat.LegacyJava8.eval(() -> { switch ((slot)) { case HEAD:  return Items.DIAMOND_HELMET; case CHEST:  return Items.DIAMOND_CHESTPLATE; case LEGS:  return Items.DIAMOND_LEGGINGS; case FEET:  return Items.DIAMOND_BOOTS; default:  return net.minecraft.item.Item.getItemFromBlock(net.minecraft.init.Blocks.AIR); } });  default: throw new AssertionError("Unexpected switch value"); } });
        return new ItemStack(item);
    }
    ...
    private static int countLocal(WorldServer level, EntityLiving origin, Tier tier) {
        ...
        return level.getEntitiesWithinAABB(EntityLiving.class,
                origin.getEntityBoundingBox().expandXyz(radius), candidate ->
