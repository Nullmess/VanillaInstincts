...
public final class Minecraft115TagCompat {
    ...
    public static LegacyTag<Block> blockTag(ResourceLocation id) {
        ...
        return new LegacyTag<>(block -> block != null && blockIds(name).contains(Block.REGISTRY.getNameForObject(block)));
    }

    public static LegacyTag<Item> itemTag(ResourceLocation id) {
        final String name = id == null ? "" : fr.vanillainstincts.compat.LegacyResourceLocation.path(id);
        return new LegacyTag<>(item -> item != null && itemIds(name).contains(Item.REGISTRY.getNameForObject(item)));
    }
    ...
    public static boolean blockStateIs(IBlockState state, LegacyTag<Block> tag) {
