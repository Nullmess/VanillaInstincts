...
public final class PillagerTacticsController {
    ...
    public static void contribute(PillagerEntity pillager, MobDecisionPlan plan,
        ...
        List<PillagerEntity> squad = level.getEntitiesWithinAABB(PillagerEntity.class,
                pillager.getEntityBoundingBox().expandXyz(
    ...
    public static Vec3d coverDestination(PillagerEntity pillager,
        ...
        Vec3d away = pillager.getPositionVector().subtract(target.getPositionVector())
        ...
        if ((pillager.getUniqueID().hashCode() & 1) != 0) side = side.scale(-1.0D);
        return pillager.getPositionVector()
    ...
    public static Vec3d flankDestination(PillagerEntity pillager,
                                        EntityLivingBase target,
                                        int squadSize) {
        Vec3d radial = pillager.getPositionVector().subtract(target.getPositionVector())
                .multiply(1.0D, 0.0D, 1.0D);
