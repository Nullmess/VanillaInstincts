...
public final class GolemConstructionState {
    ...
    public static GolemConstructionState load(Mob mob) {
        ...
        if (!fr.vanillainstincts.persistence.NbtCompat.contains(persistent, ROOT_KEY)) return state;
        CompoundTag tag = fr.vanillainstincts.persistence.NbtCompat.getCompound(persistent, ROOT_KEY);
        state.hasMovementSample = fr.vanillainstincts.persistence.NbtCompat.getBoolean(tag, "has_movement_sample");
        state.movementX = fr.vanillainstincts.persistence.NbtCompat.getDouble(tag, "movement_x");
        state.movementY = fr.vanillainstincts.persistence.NbtCompat.getDouble(tag, "movement_y");
        state.movementZ = fr.vanillainstincts.persistence.NbtCompat.getDouble(tag, "movement_z");
        state.movementSampleAt = Math.max(0L,
                fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "movement_sample_at"));
        state.stalledSamples = Math.max(0,
                Math.min(100, fr.vanillainstincts.persistence.NbtCompat.getInt(tag, "stalled_samples")));
        state.obstacleReadyAt = Math.max(0L,
                fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "obstacle_ready_at"));
        state.buildReadyAt = Math.max(0L,
                fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "build_ready_at"));
        state.constructionTargetId = fr.vanillainstincts.persistence.NbtCompat.hasUuid(tag, "construction_target_id")
                ? fr.vanillainstincts.persistence.NbtCompat.getUuid(tag, "construction_target_id") : null;
        state.constructionBlocksRemaining = Math.max(0,
                Math.min(64, fr.vanillainstincts.persistence.NbtCompat.getInt(tag, "construction_blocks_remaining")));
        state.constructionBlocksPlaced = Math.max(0,
                Math.min(64, fr.vanillainstincts.persistence.NbtCompat.getInt(tag, "construction_blocks_placed")));
        state.constructionExpiresAt = Math.max(0L,
                fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "construction_expires_at"));
        state.constructionStartY = fr.vanillainstincts.persistence.NbtCompat.getDouble(tag, "construction_start_y");
        state.constructionAnchorSet = fr.vanillainstincts.persistence.NbtCompat.getBoolean(tag, "construction_anchor_set");
        state.constructionAnchorPos = fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "construction_anchor_pos");
        state.constructionJumpAttempts = Math.max(0,
                Math.min(8, fr.vanillainstincts.persistence.NbtCompat.getInt(tag, "construction_jump_attempts")));
        state.constructionPillarArmed = fr.vanillainstincts.persistence.NbtCompat.getBoolean(tag, "construction_pillar_armed");
        state.constructionPillarPos = fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "construction_pillar_pos");
        state.constructionPillarArmedAt = Math.max(0L,
                fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "construction_pillar_armed_at"));
        state.constructionGoalY = Math.max(0,
                fr.vanillainstincts.persistence.NbtCompat.getInt(tag, "construction_goal_y"));
        state.constructionTargetOriginSet = fr.vanillainstincts.persistence.NbtCompat.getBoolean(tag, "construction_target_origin_set");
        state.constructionTargetOriginPos = fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "construction_target_origin_pos");
        state.constructionTargetDriftSince = Math.max(0L,
                fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "construction_target_drift_since"));
        state.constructionObstructionSince = Math.max(0L,
                fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "construction_obstruction_since"));
        state.constructionReplans = Math.max(0,
                Math.min(4, fr.vanillainstincts.persistence.NbtCompat.getInt(tag, "construction_replans")));
        state.constructionDescentActive = fr.vanillainstincts.persistence.NbtCompat.getBoolean(tag, "construction_descent_active");
    ...
    public void save(Mob mob) {
        ...
        if (constructionTargetId != null) {
            fr.vanillainstincts.persistence.NbtCompat.putUuid(tag, "construction_target_id", constructionTargetId);
        }
