...
public final class VillageEvolutionController {
    ...
    public static void tickLevel(ServerLevel level, long gameTime) {
        ...
        long day = Math.floorDiv(level.getOverworldClockTime(), 24_000L);
        ...
        if (VillagerRoutineController.phaseFor(level.getOverworldClockTime())
    ...
    private static void processBuilding(ServerLevel level, VillageEvolutionRuntime state,
        ...
        VillagerProfession owner = placement.road()
                ? VillagerProfessionCompat.value(VillagerProfession.NONE) : project.profession;
    ...
    private static void processRepair(ServerLevel level, VillageEvolutionRuntime state,
        ...
        Villager builder = builder(level, task.builderId, task.pos,
                ...
                && task.profession == VillagerProfessionCompat.value(VillagerProfession.NONE));
        if (builder == null) {
            rotateRepair(state, task, true);
            return;
        }
    ...
    private static void restoreProject(ServerLevel level, VillageEvolutionRuntime state) {
        ...
        Identifier templateId = parseId(saved.templateId());
    ...
    private static VillageBuildingProject discoverRememberedProject(
        ...
        for (VillageEvolutionSavedData.KnownStructure remembered
            ...
            Identifier id = parseId(remembered.templateId());
    ...
    private static boolean overlapsDifferentKnownStructure(ServerLevel level,
        ...
        for (VillageEvolutionSavedData.KnownStructure remembered
            ...
            Identifier otherId = parseId(remembered.templateId());
    ...
    private static List<VanillaVillageStructureCatalog.TemplatePlan>
        ...
        Map<Identifier, Integer> uses = new HashMap<>();
        for (VillageEvolutionSavedData.KnownStructure structure : nearby) {
            Identifier id = parseId(structure.templateId());
        ...
        List<VanillaVillageStructureCatalog.TemplatePlan> candidates = all.stream()
                .filter(plan -> switch (request.kind) {
                    case HOUSE -> hasBed(plan)
                            && professionFor(plan) == VillagerProfessionCompat.value(VillagerProfession.NONE);
                    ...
                    .forEach(pos -> {
                        ...
                        if (isRoadPosition(level, pos, blockState)) {
                            roads.put(pos.immutable(), new VillageSnapshotBlock(
                                    blockState, VillagerProfessionCompat.value(VillagerProfession.NONE), true));
                        }
                    });
            ...
            for (Map.Entry<BlockPos, VillageSnapshotBlock> expected
                ...
                if (!current.equals(remembered.state())
                    ...
                    state.repairs.add(new VillageRepairTask(expected.getKey(),
                            remembered.state(), VillagerProfessionCompat.value(VillagerProfession.NONE), true,
                            true, null));
                }
            }
        }
    ...
    private static Villager builder(ServerLevel level, UUID preferred,
        ...
        if (entity instanceof Villager villager && villager.isAlive()
                ...
                && VillageConstructionCapability.canWorkOn(
                villager.getVillagerData().profession().value(), profession,
                road, simpleHouse)) {
            return villager;
        }
    ...
    static String professionName(VillagerProfession profession) {
        if (profession == null || profession == VillagerProfessionCompat.value(VillagerProfession.NONE))
            return "none";
        if (profession == VillagerProfessionCompat.value(VillagerProfession.ARMORER)) return "armorer";
        if (profession == VillagerProfessionCompat.value(VillagerProfession.TOOLSMITH)) return "toolsmith";
        if (profession == VillagerProfessionCompat.value(VillagerProfession.WEAPONSMITH)) return "weaponsmith";
        if (profession == VillagerProfessionCompat.value(VillagerProfession.CLERIC)) return "cleric";
        if (profession == VillagerProfessionCompat.value(VillagerProfession.FISHERMAN)) return "fisherman";
        if (profession == VillagerProfessionCompat.value(VillagerProfession.FARMER)) return "farmer";
        if (profession == VillagerProfessionCompat.value(VillagerProfession.LIBRARIAN)) return "librarian";
        if (profession == VillagerProfessionCompat.value(VillagerProfession.CARTOGRAPHER)) return "cartographer";
        if (profession == VillagerProfessionCompat.value(VillagerProfession.BUTCHER)) return "butcher";
        if (profession == VillagerProfessionCompat.value(VillagerProfession.FLETCHER)) return "fletcher";
        if (profession == VillagerProfessionCompat.value(VillagerProfession.SHEPHERD)) return "shepherd";
        if (profession == VillagerProfessionCompat.value(VillagerProfession.LEATHERWORKER)) return "leatherworker";
        if (profession == VillagerProfessionCompat.value(VillagerProfession.MASON)) return "mason";
        return "none";
    }

    static VillagerProfession profession(String name) {
        if (name == null) return VillagerProfessionCompat.value(VillagerProfession.NONE);
        return switch (name) {
            case "armorer" -> VillagerProfessionCompat.value(VillagerProfession.ARMORER);
            case "toolsmith" -> VillagerProfessionCompat.value(VillagerProfession.TOOLSMITH);
            case "weaponsmith" -> VillagerProfessionCompat.value(VillagerProfession.WEAPONSMITH);
            case "cleric" -> VillagerProfessionCompat.value(VillagerProfession.CLERIC);
            case "fisherman" -> VillagerProfessionCompat.value(VillagerProfession.FISHERMAN);
            case "farmer" -> VillagerProfessionCompat.value(VillagerProfession.FARMER);
            case "librarian" -> VillagerProfessionCompat.value(VillagerProfession.LIBRARIAN);
            case "cartographer" -> VillagerProfessionCompat.value(VillagerProfession.CARTOGRAPHER);
            case "butcher" -> VillagerProfessionCompat.value(VillagerProfession.BUTCHER);
            case "fletcher" -> VillagerProfessionCompat.value(VillagerProfession.FLETCHER);
            case "shepherd" -> VillagerProfessionCompat.value(VillagerProfession.SHEPHERD);
            case "leatherworker" -> VillagerProfessionCompat.value(VillagerProfession.LEATHERWORKER);
            case "mason" -> VillagerProfessionCompat.value(VillagerProfession.MASON);
            default -> VillagerProfessionCompat.value(VillagerProfession.NONE);
        };
    }
    ...
    private static void put(Map<Long, VillagePlacement> placements, BlockPos pos,
                            BlockState state, boolean terrainReplacement,
                            boolean road, Identifier lootTable,
                            long lootTableSeed) {
        placements.put(pos.asLong(), new VillagePlacement(pos.immutable(), state,
    ...
    private record ProjectSeed(BlockPos origin, BuildingKind kind,
                               VillagerProfession profession,
                               Identifier templateId, Rotation rotation,
                               List<VillagePlacement> placements) {
    }
