...
public final class EntityDataMigrationService {
    ...
    public static boolean migrate(Entity entity, long gameTime) {
        ...
        int storedVersion = fr.vanillainstincts.persistence.NbtCompat.getInt(persistent, SCHEMA_KEY);
    ...
    private static boolean migratePrefix(CompoundTag persistent,
        ...
        for (String key : new ArrayList<>(persistent.keySet())) {
            ...
            if (!fr.vanillainstincts.persistence.NbtCompat.contains(persistent, replacement) && value != null) {
    ...
    private static boolean clampTemporalValues(CompoundTag persistent) {
        boolean changed = false;
        for (String key : List.copyOf(persistent.keySet())) {
            if (!key.startsWith("vanillainstincts_")
                    || !fr.vanillainstincts.persistence.NbtCompat.contains(persistent, key)
                    || !temporalKey(key)) {
                continue;
            }
            long value = fr.vanillainstincts.persistence.NbtCompat.getLong(persistent, key);
            if (value < 0L) {
    ...
    private static boolean sanitizeVillagerActivities(CompoundTag tag,
                                                       long gameTime) {
        boolean changed = false;
        int cartographerStage = fr.vanillainstincts.persistence.NbtCompat.getInt(tag, CARTOGRAPHER_STAGE);
        ...
        if (fr.vanillainstincts.persistence.NbtCompat.contains(tag, PRODUCTION_UNTIL)
                && (!fr.vanillainstincts.persistence.NbtCompat.contains(tag, "vanillainstincts_production_product_id")
                || !fr.vanillainstincts.persistence.NbtCompat.contains(tag, "vanillainstincts_production_job_pos"))) {
        ...
        if (fr.vanillainstincts.persistence.NbtCompat.contains(tag, CLERIC_STAND)
                && !fr.vanillainstincts.persistence.NbtCompat.contains(tag, "vanillainstincts_cleric_brew_started_at")) {
        ...
        if (fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "vanillainstincts_cartographer_ready_at")
                > gameTime + 2_400_000L) {
    ...
    private static boolean remove(CompoundTag tag, String... keys) {
        boolean changed = false;
        for (String key : keys) {
            if (fr.vanillainstincts.persistence.NbtCompat.contains(tag, key)) {
                tag.remove(key);
                changed = true;
            }
        }
