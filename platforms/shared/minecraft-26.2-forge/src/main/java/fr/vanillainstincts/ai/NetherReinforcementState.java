...
public final class NetherReinforcementState {
    ...
    public static boolean hasPersistentMission(Mob mob) {
        return mob != null && mob.getPersistentData().contains(ROOT_KEY)
                && fr.vanillainstincts.persistence.NbtCompat.getBoolean(
                        fr.vanillainstincts.persistence.NbtCompat.getCompound(mob.getPersistentData(), ROOT_KEY), "active");
    }
    ...
    public static NetherReinforcementState load(Mob mob) {
        ...
        CompoundTag tag = fr.vanillainstincts.persistence.NbtCompat.getCompound(mob.getPersistentData(), ROOT_KEY);
        state.active = fr.vanillainstincts.persistence.NbtCompat.getBoolean(tag, "active");
        state.missionId = fr.vanillainstincts.persistence.NbtCompat.hasUuid(tag, "mission_id")
                ? fr.vanillainstincts.persistence.NbtCompat.getUuid(tag, "mission_id") : null;
        state.aggressorId = fr.vanillainstincts.persistence.NbtCompat.hasUuid(tag, "aggressor_id")
                ? fr.vanillainstincts.persistence.NbtCompat.getUuid(tag, "aggressor_id") : null;
        state.kind = readEnum(fr.vanillainstincts.persistence.NbtCompat.getString(tag, "kind"),
                NetherReinforcementKind.ZOMBIFIED_PIGLIN,
                NetherReinforcementKind.class);
        state.role = readEnum(fr.vanillainstincts.persistence.NbtCompat.getString(tag, "role"),
                NetherReinforcementRole.REINFORCEMENT,
                NetherReinforcementRole.class);
        state.phase = readEnum(fr.vanillainstincts.persistence.NbtCompat.getString(tag, "phase"),
                NetherReinforcementPhase.NONE,
                NetherReinforcementPhase.class);
        state.overworldPortalSet = fr.vanillainstincts.persistence.NbtCompat.getBoolean(tag, "overworld_portal_set");
        state.overworldPortalPos = fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "overworld_portal_pos");
        state.netherPortalSet = fr.vanillainstincts.persistence.NbtCompat.getBoolean(tag, "nether_portal_set");
        state.netherPortalPos = fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "nether_portal_pos");
        state.startedAt = Math.max(0L, fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "started_at"));
        state.expiresAt = Math.max(0L, fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "expires_at"));
        state.searchUntil = Math.max(0L, fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "search_until"));
        state.cooldownUntil = Math.max(0L, fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "cooldown_until"));
        state.lastShareAt = Math.max(0L, fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "last_share_at"));
        state.propagationDepth = Math.max(0,
                Math.min(8, fr.vanillainstincts.persistence.NbtCompat.getInt(tag, "propagation_depth")));
        state.lastKnownTargetSet = fr.vanillainstincts.persistence.NbtCompat.getBoolean(tag, "last_target_set");
        state.lastKnownTargetPos = fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "last_target_pos");
    ...
    public void save(Mob mob) {
        ...
        if (missionId != null) fr.vanillainstincts.persistence.NbtCompat.putUuid(tag, "mission_id", missionId);
        if (aggressorId != null) fr.vanillainstincts.persistence.NbtCompat.putUuid(tag, "aggressor_id", aggressorId);
