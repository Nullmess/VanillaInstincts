...
public final class MobRuntimeState {
    ...
    public static MobRuntimeState load(EntityLiving mob) {
        MobRuntimeState state = new MobRuntimeState();
        if (mob == null || !mob.getEntityData().hasKey(ROOT_KEY)) {
            return state;
        }
        NBTTagCompound root = mob.getEntityData().getCompoundTag(ROOT_KEY);
        ...
        state.priority = Math.max(0, root.getInteger("priority"));
    ...
    public void save(EntityLiving mob) {
        ...
        root.setString("state", currentState.name());
        root.setString("owner", owner.name());
        root.setString("intent", currentIntent.name());
        root.setInteger("priority", priority);
        root.setLong("locked_until", lockedUntil);
        root.setLong("intent_until", intentUntil);
        mob.getEntityData().setTag(ROOT_KEY, root);
        dirty = false;
    }
    ...
    public void maintainIntent(EntityLiving mob, long gameTime) {
