...
public final class VillagerProductionController {
    ...
    public static boolean contribute(VillagerEntity villager,
        ...
        if (job != null && !entityBlockPos(villager).closerThan(job,
    ...
    private static void start(VillagerEntity villager, ServerWorld level,
    ...
    private static void complete(VillagerEntity villager, ServerWorld level,
                                 ProfessionRecipe recipe) {
        ItemStack product = recipe.result();
        long sequence = villager.getPersistentData().getLong(SEQUENCE);
        villager.setItemInHand(Hand.MAIN_HAND, product.copy());
        ...
        if (!stored) {
            ...
            if (!inventoryProduct.isEmpty()) {
                villager.spawnAtLocation(inventoryProduct);
            }
    ...
    private static void animate(VillagerEntity villager, ServerWorld level,
                                BlockPos job) {
        villager.swing(Hand.MAIN_HAND);
    ...
    private static ProfessionRecipe storedRecipe(VillagerEntity villager) {
        return ProfessionRecipeCatalog.byId(villager.getPersistentData()
                .getString(RECIPE_ID));
    }

    private static void clearExpired(VillagerEntity villager, long gameTime) {
    ...
    private static void cancel(VillagerEntity villager, long gameTime,
                               boolean refund) {
        ProfessionRecipe recipe = storedRecipe(villager);
    ...
    private static boolean active(VillagerEntity villager, long gameTime) {
    ...
    public static ProfessionRecipe nextRecipe(VillagerEntity villager) {
    ...
    public static void recordSocialSale(VillagerEntity seller, ServerWorld level,
    ...
    public static boolean socialBoostActive(VillagerEntity villager,
                                            long gameTime) {
        return villager != null && gameTime < villager.getPersistentData()
                .getLong(SOCIAL_BOOST_UNTIL);
    }

    public static int socialSaleCount(VillagerEntity villager) {
        return villager == null ? 0 : Math.max(0,
                villager.getPersistentData().getInt(SOCIAL_SALES));
    }

    public static int productionCooldown(VillagerEntity villager,
                                         long sequence) {
