...
public final class ZombieHordePressureController {
    ...
    public static void contribute(ZombieEntity zombie, MobDecisionPlan plan,
        ...
        Vec3d destination = target != null && target.isAlive()
        ...
        Vec3d direction = horizontalDirection(zombie.position(), destination);
        if (direction == null) {
            return;
        }
    ...
    public static List<ZombieEntity> rearAllies(ZombieEntity zombie, ServerWorld level,
        ...
        for (ZombieEntity other : candidates) {
            Vec3d relative = other.position().subtract(zombie.position())
    ...
    public static Optional<Vec3d> pressureImpulse(ZombieEntity zombie,
                                                  ServerWorld level,
                                                  Vec3d direction,
                                                  int obstacleHeight,
                                                  int rearCount) {
    ...
    public static Vec3d horizontalDirection(Vec3d from, Vec3d to) {
        ...
        Vec3d direction = to.subtract(from).multiply(1.0D, 0.0D, 1.0D);
