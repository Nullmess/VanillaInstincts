...
public final class MobRuntimeState {
    ...
    public static MobRuntimeState load(MobEntity mob) {
        MobRuntimeState state = new MobRuntimeState();
        if (mob == null || !mob.getPersistentData().contains(ROOT_KEY)) {
            return state;
        }
        CompoundNBT root = mob.getPersistentData().getCompound(ROOT_KEY);
    ...
    public void maintainIntent(MobEntity mob, long gameTime) {
