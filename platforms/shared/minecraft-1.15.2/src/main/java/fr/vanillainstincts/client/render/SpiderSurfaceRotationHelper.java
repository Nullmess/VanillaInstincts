...
public final class SpiderSurfaceRotationHelper {
    private static final Map<SpiderEntity, SpiderVisualPose> SPIDER_POSES =
            new WeakHashMap<>();
    ...
    public static void apply(SpiderEntity spider, MatrixStack poseStack,
                             float partialTick) {
        if (spider.isInWater()) {
            return;
        }
    ...
    private static SurfaceAngles detectSurfaceAngles(SpiderEntity spider) {
        World level = spider.level;
        ...
        if (!spider.horizontalCollision && spider.onGround) {
