...
public final class MobPerceptionMemory {
    ...
    public static void observe(MobEntity mob, ServerWorld level, long gameTime) {
        ...
        rememberSeen(mob, entityBlockPos(target), target.getUUID(), gameTime,
    ...
    public static Optional<Vec3d> bestKnownPosition(MobEntity mob, long gameTime) {
        if (mob == null) return Optional.empty();
        expire(mob, gameTime);
        if (hasFreshSeen(mob, gameTime)
                && mob.getPersistentData().contains(SEEN_POS)) {
            return Optional.of(Minecraft115VectorCompat.atBottomCenterOf(BlockPos.of(
