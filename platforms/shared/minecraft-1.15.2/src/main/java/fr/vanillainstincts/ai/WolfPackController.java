...
public final class WolfPackController {
    ...
    public static boolean supports(WolfEntity wolf) {
        return wolf != null && !wolf.isTame();
    }

    public static void contribute(WolfEntity wolf, MobDecisionPlan plan,
        ...
        List<WolfEntity> pack = level.getEntitiesOfClass(WolfEntity.class,
        ...
        WolfEntity injured = mostInjured(pack, wolf);
        double averageRatio = averageHealthRatio(pack, wolf);
        boolean targetDangerous = target.getMaxHealth() >= 40.0F
                || target instanceof PlayerEntity
                && (((PlayerEntity) (target)).getArmorValue() >= 12
                || ((PlayerEntity) (target)).getHealth() + ((PlayerEntity) (target)).getAbsorptionAmount() >= 28.0F);
        ...
        if (injured != null && injured != wolf
            ...
            Vec3d guard = guardPosition(injured, target);
        ...
        Vec3d flank = flankPosition(target, wolf, pack.size() + 1);
    ...
    public static Vec3d flankPosition(LivingEntity target, WolfEntity wolf,
                                     int packSize) {
    ...
    private static LivingEntity resolvePackTarget(WolfEntity wolf,
                                                   List<WolfEntity> pack,
                                                   long gameTime) {
    ...
    private static void offerRetreat(WolfEntity wolf, Vec3d danger,
                                     MobDecisionPlan plan) {
        Vec3d away = horizontal(wolf.position().subtract(danger));
    ...
    private static Vec3d guardPosition(WolfEntity injured, LivingEntity target) {
        Vec3d toward = horizontal(target.position().subtract(injured.position()));
        if (toward == null) toward = new Vec3d(1.0D, 0.0D, 0.0D);
        return injured.position().add(toward.scale(2.0D));
    }

    private static WolfEntity mostInjured(List<WolfEntity> pack, WolfEntity self) {
    ...
    private static double averageHealthRatio(List<WolfEntity> pack, WolfEntity self) {
        double sum = self.getHealth() / Math.max(1.0F, self.getMaxHealth());
        for (WolfEntity wolf : pack) {
            sum += wolf.getHealth() / Math.max(1.0F, wolf.getMaxHealth());
        }
        return sum / (pack.size() + 1.0D);
    }

    private static void rememberTrack(WolfEntity wolf, BlockPos pos,
                                      long gameTime) {
    ...
    private static void rememberTerritory(WolfEntity wolf, List<WolfEntity> pack,
                                          long gameTime) {
    ...
    private static boolean outsideTerritory(WolfEntity wolf, LivingEntity target,
                                            long gameTime) {
        if (wolf.getPersistentData().getLong(TERRITORY_UNTIL) <= gameTime) {
            return false;
        }
        ...
        return center.distSqr(entityBlockPos(target))
                > AnimalRules.WOLF_TERRITORY_RADIUS
                * AnimalRules.WOLF_TERRITORY_RADIUS;
    }

    private static Vec3d horizontal(Vec3d value) {
        Vec3d flat = value.multiply(1.0D, 0.0D, 1.0D);
