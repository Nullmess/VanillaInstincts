...
public final class SpiderRetreatController {
    ...
    public static boolean contribute(SpiderEntity spider, LivingEntity target,
        ...
        Vec3d away = spider.position().subtract(target.position())
                .multiply(1.0D, 0.0D, 1.0D);
        if (away.lengthSqr() < 1.0E-8D) {
            away = new Vec3d((spider.getId() & 1) == 0 ? 1.0D : -1.0D,
                    0.0D, 1.0D);
        }
        away = away.normalize();
        Vec3d side = new Vec3d(-away.z, 0.0D, away.x)
                .scale((spider.getId() & 2) == 0 ? 2.0D : -2.0D);
        Vec3d destination = spider.position().add(away.scale(
        ...
        if (spider.onGround && (gameTime + spider.getId()) % 12L == 0L) {
            Vec3d impulse = away.scale(0.28D).add(0.0D, 0.34D, 0.0D);
    ...
    private static void maybePlaceRetreatWeb(SpiderEntity spider,
        ...
        Vec3d towardThreat = target.position().subtract(spider.position())
                .multiply(1.0D, 0.0D, 1.0D);
