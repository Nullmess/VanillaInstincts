...
public final class VanillaInstinctsController {
    ...
    public static void tick(MobEntity mob) {
        if (!(mob.level instanceof ServerWorld) || !mob.isAlive()) {
            return;
        ...
        try {
            ...
            if (mob instanceof PillagerEntity
                    ...
                    && PillagerOutpostLootController.maintain(((PillagerEntity) (mob)), level,
                    gameTime)) { PillagerEntity pillager = (PillagerEntity) (mob); 
                return;
            }
            ...
            if (mob instanceof VillagerEntity) { VillagerEntity villager = (VillagerEntity) (mob); 
            ...
            } else if (mob instanceof ZombieVillagerEntity) { ZombieVillagerEntity zombieVillager = (ZombieVillagerEntity) (mob); 
                if (FeatureGate.enabled(FeatureFlag.IRON_DOOR_LEARNING, level)) {
                    LearnedDoorController.maintainZombieVillager(zombieVillager,
                            level, gameTime);
                }
            } else if (mob instanceof ZombieEntity) { ZombieEntity zombie = (ZombieEntity) (mob); 
                if (FeatureGate.enabled(FeatureFlag.ZOMBIE_TACTICS, level)) {
                    tickZombie(zombie, level, gameTime);
                }
            } else if (mob instanceof WanderingTraderEntity) { WanderingTraderEntity trader = (WanderingTraderEntity) (mob); 
                if (FeatureGate.enabled(FeatureFlag.SOCIAL_TRADING, level)) {
                    RecoveredTradeController.maintain(trader, level, gameTime);
                }
            } else if (mob instanceof IronGolemEntity) { IronGolemEntity golem = (IronGolemEntity) (mob); 
                if (FeatureGate.enabled(FeatureFlag.GOLEM_AI, level)) {
                    tickGolem(golem, level, gameTime);
                }
            } else if (mob instanceof EndermanEntity) { EndermanEntity enderman = (EndermanEntity) (mob); 
                if (FeatureGate.enabled(FeatureFlag.ENDERMAN_TACTICS, level)) {
                    tickEnderman(enderman, level, gameTime);
                }
            } else if (mob instanceof CreeperEntity) { CreeperEntity creeper = (CreeperEntity) (mob); 
                if (FeatureGate.enabled(FeatureFlag.CREEPER_TACTICS, level)) {
                    tickCreeper(creeper, level, gameTime);
                }
            } else if (mob instanceof SpiderEntity) { SpiderEntity spider = (SpiderEntity) (mob); 
                if (FeatureGate.enabled(FeatureFlag.SPIDER_TACTICS, level)) {
                    tickSpider(spider, level, gameTime);
                }
            } else if (mob instanceof PillagerEntity) { PillagerEntity pillager = (PillagerEntity) (mob); 
                if (FeatureGate.enabled(FeatureFlag.PILLAGER_TACTICS, level)) {
                    tickPillager(pillager, level, gameTime);
                }
            } else if (mob instanceof AbstractSkeletonEntity) { AbstractSkeletonEntity skeleton = (AbstractSkeletonEntity) (mob); 
                tickSkeleton(skeleton, level, gameTime);
            } else if (mob instanceof WolfEntity) { WolfEntity wolf = (WolfEntity) (mob); 
                if (FeatureGate.enabled(FeatureFlag.WOLF_PACKS, level)) {
                    ...
                    || FeatureGate.enabled(FeatureFlag.MOB_ECOLOGY, level))) { AnimalEntity animal = (AnimalEntity) (mob); 
    ...
    private static void tickVillager(VillagerEntity villager, ServerWorld level,
                                     long gameTime) {
    ...
    private static void tickGolem(IronGolemEntity golem, ServerWorld level,
                                  long gameTime) {
    ...
    private static void tickEnderman(EndermanEntity enderman, ServerWorld level,
                                     long gameTime) {
    ...
    private static void tickCreeper(CreeperEntity creeper, ServerWorld level,
                                    long gameTime) {
        ...
        try {
            ...
            if (target instanceof PlayerEntity && validCombatTarget(((PlayerEntity) (target)))
                    && VanillaInstinctsServerConfig.creeperTacticsAllowedAtY(
                    entityBlockPos(creeper).getY())) { PlayerEntity player = (PlayerEntity) (target); 
    ...
    private static void tickSpider(SpiderEntity spider, ServerWorld level,
                                   long gameTime) {
    ...
    private static void tickZombie(ZombieEntity zombie, ServerWorld level,
                                   long gameTime) {
    ...
    private static void tickPillager(PillagerEntity pillager, ServerWorld level,
                                     long gameTime) {
    ...
    private static void tickWolf(WolfEntity wolf, ServerWorld level,
                                 long gameTime) {
    ...
    private static void tickAnimal(AnimalEntity animal, ServerWorld level,
                                   long gameTime) {
    ...
    private static boolean scheduled(MobEntity mob, ServerWorld level,
                                     FeatureFlag feature) {
