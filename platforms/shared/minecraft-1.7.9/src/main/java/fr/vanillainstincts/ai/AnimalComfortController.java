...
public final class AnimalComfortController {
    ...
    public static void contribute(EntityAnimal animal, MobDecisionPlan plan,
        ...
        if (need == ComfortNeed.REST
                && fr.vanillainstincts.compat.Minecraft112Compat.distanceSq(fr.vanillainstincts.compat.Minecraft17Compat.position(animal), target) <= 2.25D) {
    ...
    public static ComfortNeed currentEnvironmentalNeed(
        ...
        if (profile.needsShelter()
                && fr.vanillainstincts.compat.Minecraft112Compat.isRainingAt(level, pos) && fr.vanillainstincts.compat.Minecraft17Compat.canSeeSky(level, pos.up())) {
    ...
    public static boolean isHeatStressed(EntityAnimal animal, WorldServer level,
        ...
        return fr.vanillainstincts.compat.Minecraft112Compat.isDay(level)
                && !fr.vanillainstincts.compat.Minecraft112Compat.isRainingAt(level, pos)
                && fr.vanillainstincts.compat.Minecraft17Compat.canSeeSky(level, pos.up())
    ...
    public static void recordFear(EntityAnimal animal, Vec3 source, long gameTime) {
    ...
    public static Optional<Vec3> fearSource(EntityAnimal animal, long gameTime) {
        if (animal == null
    ...
    public static Optional<Vec3> rememberedShelter(EntityAnimal animal,
                                                    WorldServer level,
                                                    long gameTime) {
    ...
    public static double comfortScore(EntityAnimal animal, WorldServer level,
        ...
        double distance = Math.sqrt(fr.vanillainstincts.compat.Minecraft112Compat.distanceSq(fr.vanillainstincts.compat.Minecraft17Compat.position(animal), candidate));
    ...
    private static Vec3 localHerdAnchor(EntityAnimal animal, WorldServer level) {
        List<EntityAnimal> herd = fr.vanillainstincts.compat.Minecraft112Compat.entitiesWithinAABB(level, EntityAnimal.class,
                fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(animal).expand(AnimalRules.ANIMAL_HERD_RADIUS * 0.65D, AnimalRules.ANIMAL_HERD_RADIUS * 0.65D, AnimalRules.ANIMAL_HERD_RADIUS * 0.65D),
        ...
        for (int dx = -radius; dx <= radius; dx++) {
            for (int dz = -radius; dz <= radius; dz++) {
                ...
                for (int dy = -1; dy <= 1; dy++) {
                    if (fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, fr.vanillainstincts.compat.Minecraft112Compat.offset(feet, dx, dy, dz)).getBlock().getMaterial() == Material.water) {
                        return true;
                    }
                }
            }
    ...
    private static int openSpaceScore(WorldServer level, BlockPos feet) {
        ...
        for (int dx = -1; dx <= 1; dx++) {
            for (int dz = -1; dz <= 1; dz++) {
                ...
                if (!fr.vanillainstincts.compat.Minecraft112Compat.hasCollision(fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, neighbour), level, neighbour)
                        && !fr.vanillainstincts.compat.Minecraft112Compat.hasCollision(fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, neighbour.up()), level, neighbour.up())
                        && !fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, neighbour).getBlock().getMaterial().isLiquid()) {
                    open++;
                }
            }
        }
    ...
    public static Optional<Vec3> findShelter(EntityAnimal animal,
                                             WorldServer level) {
        if (animal == null || level == null) {
            return Optional.empty();
        }
        ...
        for (int dx = -radius; dx <= radius; dx++) {
            for (int dz = -radius; dz <= radius; dz++) {
                ...
                for (int dy : COMFORT_VERTICAL_OFFSETS) {
                    ...
                    double score = fr.vanillainstincts.compat.Minecraft112Compat.distanceSq(fr.vanillainstincts.compat.Minecraft17Compat.position(animal), candidate);
                    if (score < bestScore) {
                        bestScore = score;
                        best = candidate;
                    }
    ...
    public static Optional<Vec3> findDryGround(EntityAnimal animal,
                                               WorldServer level) {
        if (animal == null || level == null) {
            return Optional.empty();
        }
    ...
    private static boolean isComfortStandingPosition(EntityAnimal animal,
        ...
        if (!fr.vanillainstincts.compat.Minecraft17Compat.isBlockLoaded(level, feet) || !fr.vanillainstincts.compat.Minecraft17Compat.isBlockLoaded(level, floor)
                || !fr.vanillainstincts.compat.Minecraft112Compat.withinWorldBorder(level, feet)) {
            return false;
        }
        if (!!fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, feet).getBlock().getMaterial().isLiquid()) {
            return false;
        }
        if (!fr.vanillainstincts.compat.Minecraft112Compat.entityCanStandOn(fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, floor), level, floor, animal)
        ...
        AxisAlignedBB targetBox = AxisAlignedBB.getBoundingBox(candidate.xCoord - halfWidth, candidate.yCoord,
        ...
        for (int y = feet.getY(); y <= occupiedTopY; y++) {
            ...
            if (!!fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, occupied).getBlock().getMaterial().isLiquid()
    ...
    private static boolean hasRoof(WorldServer level, BlockPos headProbe) {
        if (!fr.vanillainstincts.compat.Minecraft17Compat.canSeeSky(level, headProbe)) {
            return true;
        }
        ...
        for (int y = headProbe.getY() + 1; y <= maxY; y++) {
            ...
            if (fr.vanillainstincts.compat.Minecraft112Compat.hasCollision(fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, roofPos), level, roofPos)) {
                return true;
            }
        }
        return false;
