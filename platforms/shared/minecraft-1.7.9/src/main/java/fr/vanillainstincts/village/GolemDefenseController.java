...
public final class GolemDefenseController {
    ...
    public static void contribute(EntityIronGolem golem,
        ...
        Vec3 protectedPoint = protectedVillager != null
                ? fr.vanillainstincts.compat.Minecraft17Compat.position(protectedVillager)
                : state.villageAnchor() == null ? fr.vanillainstincts.compat.Minecraft17Compat.position(golem)
                : Minecraft115VectorCompat.atCenterOf(state.villageAnchor());

        Vec3 desired = fr.vanillainstincts.compat.LegacyJava8.eval(() -> { switch ((state.role())) { case INTERCEPTOR:  return interpositionPoint(fr.vanillainstincts.compat.Minecraft17Compat.position(target),
                    protectedPoint,
                    GolemRules.GOLEM_INTERPOSITION_DISTANCE); case PURSUER: case REINFORCEMENT:  return fr.vanillainstincts.compat.Minecraft17Compat.position(target); case GUARDIAN:  return guardianPosition(fr.vanillainstincts.compat.Minecraft17Compat.position(target),
    ...
    public static GolemDefenseRole assignRole(EntityIronGolem golem,
                                               WorldServer level,
                                               BlockPos anchor) {
        AxisAlignedBB area = fr.vanillainstincts.compat.Minecraft112Compat.expandBox(fr.vanillainstincts.compat.Minecraft112Compat.blockBox(anchor), 
                GolemRules.GOLEM_DEFENSE_ROLE_RADIUS);
        List<EntityIronGolem> defenders = fr.vanillainstincts.compat.Minecraft112Compat.entitiesWithinAABB(level, 
    ...
    public static Vec3 interpositionPoint(Vec3 threat, Vec3 protectedPoint,
        ...
        if (threat == null || protectedPoint == null) {
            return threat == null ? new Vec3(0.0D, 0.0D, 0.0D) : threat;
        }
        Vec3 towardProtected = fr.vanillainstincts.compat.Minecraft112Compat.multiply(protectedPoint.subtract(threat), 1.0D, 0.0D, 1.0D);
        if (fr.vanillainstincts.compat.Minecraft112Compat.lengthSq(towardProtected) < 1.0E-8D) {
            return threat;
        }
        return threat.add(fr.vanillainstincts.compat.Minecraft112Compat.scale(towardProtected.normalize(), 
    ...
    private static void observeNearbyDefender(EntityIronGolem golem,
        ...
        AxisAlignedBB area = fr.vanillainstincts.compat.Minecraft112Compat.expandBox(fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(golem), 
                GolemRules.GOLEM_ALLY_SIGNAL_RADIUS);
        EntityIronGolem ally = fr.vanillainstincts.compat.Minecraft112Compat.entitiesWithinAABB(level, EntityIronGolem.class, area,
    ...
    private static UUID resolveWitnessVillagerId(
        ...
        Entity witness = fr.vanillainstincts.compat.Minecraft17Compat.getEntityFromUuid(level, threat.witnessId());
    ...
    private static EntityLivingBase resolveProtectedVillager(
        ...
        Entity entity = fr.vanillainstincts.compat.Minecraft17Compat.getEntityFromUuid(level, state.protectedVillagerId());
    ...
    private static Vec3 guardianPosition(Vec3 target,
                                         Vec3 protectedPoint) {
        Vec3 towardThreat = fr.vanillainstincts.compat.Minecraft112Compat.multiply(target.subtract(protectedPoint), 1.0D, 0.0D, 1.0D);
        if (fr.vanillainstincts.compat.Minecraft112Compat.lengthSq(towardThreat) < 1.0E-8D) return protectedPoint;
        return protectedPoint.add(fr.vanillainstincts.compat.Minecraft112Compat.scale(towardThreat.normalize(), 2.5D));
    }
