...
public final class FarmerReplantQueue {
    ...
    public static BlockPos next(WorldServer level, EntityVillager farmer,
        ...
        while (iterator.hasNext()) {
            ...
            if (entry.getValue() < gameTime || !fr.vanillainstincts.compat.Minecraft17Compat.isBlockLoaded(level, pos)
                    || !FarmerCropRegistry.hasPotentialPlantingGround(
                    level, pos)) {
                iterator.remove();
                continue;
            }
            if (fr.vanillainstincts.compat.Minecraft112Compat.isAir(fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, pos))) {
