...
public final class VillagePathEvaluator {
    ...
    public static boolean isDangerous(WorldServer level, BlockPos position) {
        return SafePositionFinder.isHazard(level, position)
                || Minecraft115TagCompat.blockStateIs(fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, position),
                        VanillaInstinctsTags.VILLAGER_DANGEROUS_BLOCKS);
    }

    public static double blockPreference(LegacyBlockState floorState) {
    ...
    public static double professionPreference(EntityVillager villager,
        ...
        LegacyBlockState through = fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, feet);
    ...
    private static double legacyProfessionPreference(
        ...
        if (profession == LegacyVillagerProfession.FARMER) {
            ...
            if (floor.getBlock().equals(Blocks.gravel)) score += 1.25D;
    ...
    public static int nearbyVillagerCount(WorldServer level,
                                           ...
                                           double radius) {
        AxisAlignedBB area = fr.vanillainstincts.compat.Minecraft112Compat.expandBox(fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(villager), Math.max(0.5D, radius));
        return fr.vanillainstincts.compat.Minecraft112Compat.entitiesWithinAABB(level, EntityVillager.class, area,
    ...
    public static double score(EntityVillager villager, WorldServer level,
        ...
        Vec3 destination = Minecraft115VectorCompat.atBottomCenterOf(feet);
        ...
        double preference = blockPreference(fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, feet.down()));
        if (FeatureGate.enabled(FeatureFlag.VILLAGER_PATH_PREFERENCES, level)) {
            preference += professionPreference(villager, level, feet);
        }
        int crowd = fr.vanillainstincts.compat.Minecraft112Compat.entitiesWithinAABB(level, EntityVillager.class,
                fr.vanillainstincts.compat.Minecraft112Compat.blockBox(feet).expand(VillageSocialRules.VILLAGER_CROWD_RADIUS, VillageSocialRules.VILLAGER_CROWD_RADIUS, VillageSocialRules.VILLAGER_CROWD_RADIUS),
