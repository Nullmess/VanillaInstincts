...
public final class VillagerProductionController {
    ...
    private static void start(EntityVillager villager, WorldServer level,
        ...
        if (!ProfessionRecipeCatalog.consume(fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(villager),
                recipe, levelValue)) {
            setReady(villager, gameTime, configuredTicks(
                    ProfessionRules.PROFESSION_PRODUCTION_IDLE_TICKS));
            return;
    ...
    private static void complete(EntityVillager villager, WorldServer level,
        ...
        villager.setCurrentItemOrArmor(0, product.copy());
        villager.swingItem();
        ...
        int villageStock = ProfessionStockController.countExact(
                fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(villager), product);
        ...
        if (!stored) {
            ...
            ProfessionStockController.insert(fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(villager),
                    inventoryProduct);
            if (!fr.vanillainstincts.compat.Minecraft110ItemStackCompat.isEmpty(inventoryProduct)) {
    ...
    private static void animate(EntityVillager villager, WorldServer level,
        ...
        if (profession == LegacyVillagerProfession.BUTCHER) {
            fr.vanillainstincts.compat.Minecraft17Compat.spawnParticle(level, EnumParticleTypes.SMOKE_NORMAL,
                    source.getX() + 0.5D, source.getY() + 1.0D,
                    source.getZ() + 0.5D, 3,
        ...
        if (profession == LegacyVillagerProfession.SHEPHERD) {
            fr.vanillainstincts.compat.Minecraft17Compat.spawnParticle(level, EnumParticleTypes.EXPLOSION_NORMAL,
                    source.getX() + 0.5D, source.getY() + 1.0D,
                    source.getZ() + 0.5D, 3,
                    0.2D, 0.2D, 0.2D, 0.01D);
            return;
    ...
    private static void cancel(EntityVillager villager, long gameTime,
        ...
        if (refund && recipe != null) {
            for (ItemStack overflow : ProfessionRecipeCatalog.refund(
                    fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(villager), recipe)) {
    ...
    public static ProfessionRecipe nextRecipe(EntityVillager villager) {
        ...
        for (int offset = 0; offset < recipes.size(); offset++) {
            ...
            if (ProfessionRecipeCatalog.canCraft(fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(villager),
                    recipe, level)) {
                return recipe;
            }
        }
    ...
    public static int productionUses(ItemStack product, long sequence) {
        return product == null || fr.vanillainstincts.compat.Minecraft110ItemStackCompat.isEmpty(product) ? 0 : 1;
    }
    ...
    public static void recordSocialSale(EntityVillager seller, WorldServer level,
        ...
        if (seller == null || level == null || sold == null || fr.vanillainstincts.compat.Minecraft110ItemStackCompat.isEmpty(sold)) {
            return;
        }
    ...
    public static int productionCooldown(EntityVillager villager,
        ...
        int cooldown = configuredTicks(
                ...
                + variation;
        if (socialBoostActive(villager, villager.worldObj.getTotalWorldTime())) {
