...
public final class EndermanAdvancedController {
    ...
    public static boolean contributeDrowningRescue(EndermanEntity enderman,
        ...
        plan.offerSpecial(VanillaInstinctsState.TELEPORT_CARGO,
                ...
                16, () -> {
                    Vec3d before = drowning.position();
    ...
    public static boolean contributeCarriedBlockPresentation(
        ...
        Vec3d look = player.getLookAngle().multiply(1.0D, 0.0D, 1.0D);
        ...
        Vec3d requested = player.position().subtract(look.scale(
    ...
    private static Optional<Vec3d> safeNearEntity(PlayerEntity player,
                                                 LivingEntity entity,
                                                 EndermanEntity enderman,
                                                 ServerWorld level) {
        for (int i = 0; i < 8; i++) {
            double angle = Math.PI * 2.0D * i / 8.0D;
            Vec3d candidate = entity.position().add(Math.cos(angle) * 3.0D,
    ...
    private static Optional<Vec3d> findLoadedLava(PlayerEntity player,
                                                 EndermanEntity enderman,
                                                 ServerWorld level) {
    ...
    private static boolean safeForPlayer(PlayerEntity player, Vec3d candidate,
                                         EndermanEntity enderman,
                                         ServerWorld level) {
    ...
    private static Optional<Vec3d> dryDestination(EndermanEntity enderman,
                                                 ServerWorld level,
                                                 Vec3d near) {
        for (int i = 0; i < 12; i++) {
            double angle = Math.PI * 2.0D * i / 12.0D;
            Vec3d requested = near.add(Math.cos(angle) * 5.0D,
    ...
    private static void emitTeleport(ServerWorld level, Vec3d pos) {
