...
public final class SkeletonTacticsController {
    ...
    public static Optional<Vec3d> safeRetreatDestination(
        ...
        Vec3d away = horizontal(skeleton.position().subtract(target.position()));
        ...
        for (double retreat = Math.max(3.0D, distance * 0.45D);
            ...
            for (double sideOffset : RETREAT_SIDE_OFFSETS) {
                Vec3d candidate = skeleton.position()
    ...
    public static Optional<Vec3d> bestStrafeDestination(
            AbstractSkeletonEntity skeleton, LivingEntity target,
            ServerWorld level) {
        Vec3d radial = horizontal(skeleton.position().subtract(target.position()));
        ...
        for (double angleOffset : ANGLE_OFFSETS) {
            ...
            Vec3d candidate = target.position().add(
    ...
    public static boolean hasLineOfFire(AbstractSkeletonEntity skeleton,
                                        ...
                                        ServerWorld level,
                                        Vec3d fromFeet) {
        Vec3d start = fromFeet.add(0.0D, skeleton.getEyeHeight(), 0.0D);
        Vec3d end = target.getEyePosition(1.0F);
    ...
    public static Vec3d retreatDestination(AbstractSkeletonEntity skeleton,
                                          LivingEntity target,
                                          double distance) {
        Vec3d away = horizontal(skeleton.position().subtract(target.position()));
        if (away == null) away = new Vec3d(1.0D, 0.0D, 0.0D);
    ...
    public static Vec3d flankDestination(AbstractSkeletonEntity skeleton,
                                        LivingEntity target,
                                        double sideDistance) {
        Vec3d radial = horizontal(skeleton.position().subtract(target.position()));
        if (radial == null) radial = new Vec3d(1.0D, 0.0D, 0.0D);
    ...
    private static Optional<Vec3d> safeRangeCorrection(
        ...
        Vec3d radial = horizontal(skeleton.position().subtract(target.position()));
        if (radial == null) radial = new Vec3d(1.0D, 0.0D, 0.0D);
        Vec3d ideal = target.position().add(radial.scale(
    ...
    private static Vec3d reacquireDestination(AbstractSkeletonEntity skeleton,
                                             Vec3d remembered) {
        Vec3d delta = remembered.subtract(skeleton.position())
                .multiply(1.0D, 0.0D, 1.0D);
        if (delta.lengthSqr() < 1.0E-8D) {
            return remembered;
        }
        Vec3d side = new Vec3d(-delta.z, 0.0D, delta.x).normalize()
    ...
    private static Vec3d horizontal(Vec3d value) {
        Vec3d horizontal = value.multiply(1.0D, 0.0D, 1.0D);
