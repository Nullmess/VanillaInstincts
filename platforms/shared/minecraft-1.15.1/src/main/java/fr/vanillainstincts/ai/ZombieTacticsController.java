...
public final class ZombieTacticsController {
    ...
    private static void offerContextLeap(ZombieEntity zombie, LivingEntity target,
        ...
        Vec3d horizontal = target.position().subtract(zombie.position())
        ...
        Vec3d impulse = direction.scale(ZombieRules.LEAP_HORIZONTAL)
    ...
    private static void offerToolPassage(ZombieEntity zombie, Vec3d destination,
                                         MobDecisionPlan plan,
                                         ServerWorld level, long gameTime) {
    ...
    public static Optional<BlockPos> findToolObstacle(ZombieEntity zombie,
                                                       Vec3d destination,
                                                       ServerWorld level) {
        Vec3d horizontal = destination.subtract(zombie.position())
                .multiply(1.0D, 0.0D, 1.0D);
    ...
    private static void updateStall(ZombieEntity zombie) {
        long current = entityBlockPos(zombie).asLong();
