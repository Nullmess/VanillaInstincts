...
public final class CreeperCoordinationController {
    ...
    public static boolean contribute(Creeper creeper, Player target,
        ...
        if (watched && creeper.distanceToSqr(target) > 36.0D
                && gameTime >= fr.vanillainstincts.persistence.NbtCompat.getLong(creeper.getPersistentData(), HIDE_READY_AT)
    ...
    private static boolean continueHide(Creeper creeper, Player target,
                                        MobDecisionPlan plan,
                                        ServerLevel level, long gameTime) {
        long hideUntil = fr.vanillainstincts.persistence.NbtCompat.getLong(creeper.getPersistentData(), HIDE_UNTIL);
        if (hideUntil <= gameTime) return false;
        Vec3 cover = new Vec3(
                fr.vanillainstincts.persistence.NbtCompat.getDouble(creeper.getPersistentData(), HIDE_X),
                fr.vanillainstincts.persistence.NbtCompat.getDouble(creeper.getPersistentData(), HIDE_Y),
                fr.vanillainstincts.persistence.NbtCompat.getDouble(creeper.getPersistentData(), HIDE_Z));
        ...
        long waitUntil = fr.vanillainstincts.persistence.NbtCompat.getLong(creeper.getPersistentData(), WAIT_UNTIL);
