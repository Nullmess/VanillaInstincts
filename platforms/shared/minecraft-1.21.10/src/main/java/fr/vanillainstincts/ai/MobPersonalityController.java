...
public final class MobPersonalityController {
    ...
    public static MobPersonality profileFor(EnderMan enderman) {
        if (enderman == null) return MobPersonality.GENERIC;
        CompoundTag persistent = enderman.getPersistentData();
        if (fr.vanillainstincts.persistence.NbtCompat.contains(persistent, ROOT_KEY)) {
            MobPersonality stored = read(fr.vanillainstincts.persistence.NbtCompat.getString(persistent, ROOT_KEY));
        ...
        persistent.putString(ROOT_KEY, assigned.name());
        return assigned;
    }

    public static int stableBucket(EnderMan enderman, int bound) {
    ...
    public static int stableBucket(EnderMan enderman, int bound,
        ...
        long mixed = enderman.getUUID().getMostSignificantBits()
                ^ Long.rotateLeft(
                enderman.getUUID().getLeastSignificantBits(), 23)
                ^ SALT ^ channelSalt;
