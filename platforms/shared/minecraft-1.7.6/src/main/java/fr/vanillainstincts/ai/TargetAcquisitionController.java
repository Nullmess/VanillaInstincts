...
public final class TargetAcquisitionController {
    ...
    public static AcquisitionResult findBestTarget(EntityLiving mob,
        ...
        AxisAlignedBB area = fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(mob).expand(radius, radius, radius);
    ...
    public static boolean eligibleCandidate(EntityLiving mob, EntityLivingBase candidate) {
        ...
        if (candidate instanceof EntityPlayer
                && (((EntityPlayer) (candidate)).capabilities.isCreativeMode || fr.vanillainstincts.compat.Minecraft17Compat.isSpectator((EntityPlayer) (candidate)))) { EntityPlayer player = (EntityPlayer) (candidate); 
            return false;
        }
