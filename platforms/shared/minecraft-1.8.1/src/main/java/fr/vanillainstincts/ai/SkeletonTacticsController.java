...
public final class SkeletonTacticsController {
    ...
    public static Optional<Vec3> safeRetreatDestination(
        ...
        Vec3 away = horizontal(skeleton.getPositionVector().subtract(target.getPositionVector()));
        ...
        for (double retreat = Math.max(3.0D, distance * 0.45D);
            ...
            for (double sideOffset : RETREAT_SIDE_OFFSETS) {
                Vec3 candidate = skeleton.getPositionVector()
    ...
    public static Optional<Vec3> bestStrafeDestination(
            EntitySkeleton skeleton, EntityLivingBase target,
            WorldServer level) {
        Vec3 radial = horizontal(skeleton.getPositionVector().subtract(target.getPositionVector()));
        ...
        for (double angleOffset : ANGLE_OFFSETS) {
            ...
            Vec3 candidate =fr.vanillainstincts.compat.Minecraft112Compat.add(target.getPositionVector(), 
    ...
    public static boolean hasLineOfFire(EntitySkeleton skeleton,
        ...
        Vec3 end = target.getPositionEyes(1.0F);
        MovingObjectPosition result = level.rayTraceBlocks(start, end, false, true, false);
    ...
    public static Vec3 retreatDestination(EntitySkeleton skeleton,
        ...
        return skeleton.getPositionVector().add(fr.vanillainstincts.compat.Minecraft112Compat.scale(away, distance * 0.55D)).add(side);
    }
    ...
    private static Optional<Vec3> safeRangeCorrection(
        ...
        Vec3 radial = horizontal(skeleton.getPositionVector().subtract(target.getPositionVector()));
        if (radial == null) radial = new Vec3(1.0D, 0.0D, 0.0D);
        Vec3 ideal = target.getPositionVector().add(fr.vanillainstincts.compat.Minecraft112Compat.scale(radial, 
    ...
    private static Vec3 reacquireDestination(EntitySkeleton skeleton,
                                             Vec3 remembered) {
        Vec3 delta = fr.vanillainstincts.compat.Minecraft112Compat.multiply(remembered.subtract(skeleton.getPositionVector()), 1.0D, 0.0D, 1.0D);
