...
public final class CreeperCoordinationController {
    ...
    public static boolean hasIgnitedNeighbor(EntityCreeper creeper,
        ...
        return !level.getEntitiesWithinAABB(EntityCreeper.class,
                fr.vanillainstincts.compat.Minecraft112Compat.expandBox(creeper.getEntityBoundingBox(), 
    ...
    public static boolean contribute(EntityCreeper creeper, EntityPlayer target,
        ...
        Vec3 toTarget = fr.vanillainstincts.compat.Minecraft112Compat.multiply(target.getPositionVector().subtract(creeper.getPositionVector()), 1.0D, 0.0D, 1.0D);
        ...
        Vec3 destination = target.getPositionVector()
    ...
    private static Optional<Vec3> findCover(EntityCreeper creeper, EntityPlayer target,
                                            WorldServer level) {
        Vec3 away = fr.vanillainstincts.compat.Minecraft112Compat.multiply(creeper.getPositionVector().subtract(target.getPositionVector()), 1.0D, 0.0D, 1.0D);
        ...
        Vec3[] candidates = {
                creeper.getPositionVector().add(fr.vanillainstincts.compat.Minecraft112Compat.scale(away, 
                        CreeperRules.CREEPER_HIDE_DISTANCE)),
                creeper.getPositionVector().add(fr.vanillainstincts.compat.Minecraft112Compat.scale(away, 5.0D)).add(fr.vanillainstincts.compat.Minecraft112Compat.scale(side, 4.0D)),
                creeper.getPositionVector().add(fr.vanillainstincts.compat.Minecraft112Compat.scale(away, 5.0D)).subtract(fr.vanillainstincts.compat.Minecraft112Compat.scale(side, 4.0D))
        };
        for (Vec3 candidate : candidates) {
            Optional<Vec3> safe = SafePositionFinder.resolveGroundDestination(
                    creeper, candidate);
            if (!safe.isPresent()) continue;
            MovingObjectPosition ray = level.rayTraceBlocks(target.getPositionEyes(1.0F),fr.vanillainstincts.compat.Minecraft112Compat.add(safe.get(), 0.0D, creeper.getEyeHeight(), 0.0D), false, true, false);
