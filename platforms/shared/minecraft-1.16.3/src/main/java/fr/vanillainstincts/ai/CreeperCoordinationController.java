...
public final class CreeperCoordinationController {
    ...
    public static boolean contribute(CreeperEntity creeper, PlayerEntity target,
        ...
        if (watched && creeper.distanceToSqr(target) > 36.0D
            ...
            Optional<Vector3d> cover = findCover(creeper, target, level);
            if (cover.isPresent()) {
        ...
        Vector3d toTarget = target.position().subtract(creeper.position())
                .multiply(1.0D, 0.0D, 1.0D);
        if (toTarget.lengthSqr() < 1.0E-8D) return false;
        Vector3d forward = toTarget.normalize();
    ...
    private static Optional<Vector3d> findCover(CreeperEntity creeper, PlayerEntity target,
                                            ServerWorld level) {
        Vector3d away = creeper.position().subtract(target.position())
                .multiply(1.0D, 0.0D, 1.0D);
    ...
    private static void storeCover(CreeperEntity creeper, Vector3d cover,
