...
public final class CreeperTacticsController {
    ...
    public static void contributeVisible(CreeperEntity creeper, PlayerEntity player,
        ...
        if (watched) {
            if (species.creeperAmbushReady(gameTime)) {
                Optional<Vector3d> ambush = findBestAmbushPosition(
            ...
            Optional<Vector3d> orbit = findBlindSideOrbitPosition(
                    creeper, player, level);
            if (orbit.isPresent()) {
        ...
        if (!watched && species.creeperUnwatchedTicks()
            ...
            Vector3d destination = stealthAdvanceDestination(
    ...
    private static void advanceWhileSwelling(CreeperEntity creeper,
        ...
        Vector3d direction = target.position().subtract(creeper.position())
    ...
    public static Vector3d fuseAdvanceVelocity(Vector3d current, Vector3d direction) {
        Vector3d safeCurrent = current == null ? Vector3d.ZERO : current;
        Vector3d horizontal = direction == null ? Vector3d.ZERO
        ...
        return new Vector3d(horizontal.x, safeCurrent.y, horizontal.z);
    }
    ...
    public static Optional<Vector3d> findBestAmbushPosition(CreeperEntity creeper,
                                                        PlayerEntity player,
                                                        ServerWorld level) {
        Vector3d playerLook = horizontal(player.getLookAngle());
        if (playerLook == null) {
            return Optional.empty();
        }
        Vector3d left = new Vector3d(-playerLook.z, 0.0D, playerLook.x);
        Vector3d right = left.scale(-1.0D);
        Vector3d behind = playerLook.scale(-1.0D);
        List<Vector3d> directions = fr.vanillainstincts.compat.LegacyJava8.listOf(
                behind,
        ...
        for (int radius = 3; radius <= 6; radius++) {
            for (Vector3d direction : directions) {
                Vector3d raw = player.position().add(direction.scale(radius));
    ...
    public static Optional<Vector3d> findBlindSideOrbitPosition(
        ...
        Vector3d behind = look.scale(-1.0D);
        List<Vector3d> candidates = fr.vanillainstincts.compat.LegacyJava8.listOf(
    ...
    public static Vector3d blindSideOrbitDestination(Vector3d playerPosition,
                                                  Vector3d playerLook,
                                                  boolean leftSide) {
        Vector3d look = horizontal(playerLook);
        if (look == null) {
    ...
    public static boolean isPlayerLookingAt(PlayerEntity player, CreeperEntity creeper) {
        ...
        Vector3d toCreeper = creeper.getEyePosition(1.0F)
    ...
    public static double rearPreferenceScore(Vector3d playerLook,
                                             Vector3d fromPlayerToCandidate) {
        Vector3d horizontalLook = horizontal(playerLook);
        Vector3d horizontalCandidate = horizontal(fromPlayerToCandidate);
    ...
    private static double scoreCandidate(CreeperEntity creeper, PlayerEntity player,
                                         ServerWorld level, Vector3d candidate,
                                         Vector3d playerLook) {
        Vector3d fromPlayer = candidate.subtract(player.position());
    ...
    private static Vector3d stealthAdvanceDestination(PlayerEntity player,
    ...
    private static boolean tryContextualLeap(CreeperEntity creeper, PlayerEntity player,
        ...
        Vector3d horizontal = horizontal(player.position()
    ...
    private static boolean hasClearRay(ServerWorld level, Vector3d from, Vector3d to,
                                       PlayerEntity source) {
    ...
    private static class ScoredPosition {
        ...
        public ScoredPosition(Vector3d position, double score) {
