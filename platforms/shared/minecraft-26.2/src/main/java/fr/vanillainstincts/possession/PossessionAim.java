...
final class PossessionAim {
    ...
    static double heldAttackRange(ServerPlayer controller) {
        ItemStack held = controller.getMainHandItem();
        AttackRange range = held.get(DataComponents.ATTACK_RANGE);
        if (range == null) range = AttackRange.defaultFor(controller);
        return Math.max(0.0D, range.effectiveMaxRange(controller));
    }

    static boolean heldAttackInRange(ServerPlayer controller,
                                     LivingEntity target) {
        ItemStack held = controller.getMainHandItem();
        AttackRange range = held.get(DataComponents.ATTACK_RANGE);
        if (range == null) range = AttackRange.defaultFor(controller);
        return range.isInRange(controller, target.getBoundingBox(), 0.0D);
    }
