...
public final class EndermanRuntimeState {
    ...
    public static EndermanRuntimeState load(EnderMan enderman) {
        EndermanRuntimeState state = new EndermanRuntimeState();
        CompoundTag root = enderman.getPersistentData();
        if (!fr.vanillainstincts.persistence.NbtCompat.contains(root, ROOT_KEY)) {
            return state;
        }
        CompoundTag tag = fr.vanillainstincts.persistence.NbtCompat.getCompound(root, ROOT_KEY);
        state.role = readEnum(fr.vanillainstincts.persistence.NbtCompat.getString(tag, "role"), EndermanCargoRole.NONE,
                EndermanCargoRole.class);
        state.cargoId = fr.vanillainstincts.persistence.NbtCompat.hasUuid(tag, "cargo_id") ? fr.vanillainstincts.persistence.NbtCompat.getUuid(tag, "cargo_id") : null;
        state.objectiveId = fr.vanillainstincts.persistence.NbtCompat.hasUuid(tag, "objective_id")
                ? fr.vanillainstincts.persistence.NbtCompat.getUuid(tag, "objective_id") : null;
        state.carryingSince = Math.max(0L, fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "carrying_since"));
        state.releaseAt = Math.max(0L, fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "release_at"));
        state.pickupReadyAt = Math.max(0L, fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "pickup_ready_at"));
        state.teleportReadyAt = Math.max(0L, fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "teleport_ready_at"));
        state.deliveryCommitAt = Math.max(0L,
                fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "delivery_commit_at"));
        state.suspendedAngerTargetId = fr.vanillainstincts.persistence.NbtCompat.hasUuid(tag, "suspended_anger_target")
                ? fr.vanillainstincts.persistence.NbtCompat.getUuid(tag, "suspended_anger_target") : null;
        state.suspendedAngerTicks = Math.max(0,
                fr.vanillainstincts.persistence.NbtCompat.getInt(tag, "suspended_anger_ticks"));
    ...
    public void save(EnderMan enderman) {
        if (!dirty) {
            return;
        }
        ...
        if (cargoId != null) {
            fr.vanillainstincts.persistence.NbtCompat.putUuid(tag, "cargo_id", cargoId);
        }
        if (objectiveId != null) {
            fr.vanillainstincts.persistence.NbtCompat.putUuid(tag, "objective_id", objectiveId);
        }
        if (suspendedAngerTargetId != null) {
            fr.vanillainstincts.persistence.NbtCompat.putUuid(tag, "suspended_anger_target", suspendedAngerTargetId);
        }
