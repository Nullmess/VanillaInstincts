...
public final class GolemDefenseState {
    ...
    public static GolemDefenseState load(IronGolem golem) {
        ...
        state.threatId = Math.max(0L, fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "threat_id"));
        state.targetId = fr.vanillainstincts.persistence.NbtCompat.hasUuid(tag, "target_id")
                ? fr.vanillainstincts.persistence.NbtCompat.getUuid(tag, "target_id") : null;
        state.protectedVillagerId = fr.vanillainstincts.persistence.NbtCompat.hasUuid(tag, "protected_villager_id")
                ? fr.vanillainstincts.persistence.NbtCompat.getUuid(tag, "protected_villager_id") : null;
        state.villageAnchor = fr.vanillainstincts.persistence.NbtCompat.contains(tag, "village_anchor")
                ? BlockPos.of(fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "village_anchor")) : null;
        state.role = readRole(fr.vanillainstincts.persistence.NbtCompat.getString(tag, "role"));
        state.assignedUntil = Math.max(0L, fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "assigned_until"));
        state.sprintUntil = Math.max(0L, fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "sprint_until"));
        state.signalReadyAt = Math.max(0L, fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "signal_ready_at"));
    ...
    public void save(IronGolem golem) {
        ...
        if (targetId != null) fr.vanillainstincts.persistence.NbtCompat.putUuid(tag, "target_id", targetId);
        if (protectedVillagerId != null) {
            fr.vanillainstincts.persistence.NbtCompat.putUuid(tag, "protected_villager_id", protectedVillagerId);
        }
