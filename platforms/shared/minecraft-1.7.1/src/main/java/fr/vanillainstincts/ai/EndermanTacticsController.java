...
public final class EndermanTacticsController {
    ...
    public static void maintain(EntityEnderman enderman,
        ...
        if (cargo instanceof EntityLiving) { EntityLiving cargoMob = (EntityLiving) (cargo); 
            if (state.role() == EndermanCargoRole.ALLY_RESCUE) {
                ...
                cargoMob.getNavigator().clearPathEntity();
    ...
    private static void offerPickup(EntityEnderman enderman, Entity cargo,
        ...
        plan.offerSpecial(VanillaInstinctsState.PICKUP_CARGO,
                ...
                () -> {
                    if (fr.vanillainstincts.compat.Minecraft112Compat.startRiding(cargo, enderman)) {
                        cargo.fallDistance = 0.0F;
                        ...
                        state.beginCarry(cargo, objective, role, gameTime,
                                duration);
                        positionCargoPassenger(enderman, cargo);
                        emitTeleportParticles(level, fr.vanillainstincts.compat.Minecraft17Compat.position(enderman));
                    } else {
    ...
    private static void contributeCarrying(EntityEnderman enderman,
        ...
        if (state.role() == EndermanCargoRole.ALLY_RESCUE
            ...
            Vec3 away = horizontal(fr.vanillainstincts.compat.Minecraft17Compat.position(enderman)
                    .subtract(fr.vanillainstincts.compat.Minecraft17Compat.position(objective)));
            if (away == null) {
                away = new Vec3(1.0D, 0.0D, 0.0D);
            }
            Vec3 desired = fr.vanillainstincts.compat.Minecraft17Compat.position(enderman).add(fr.vanillainstincts.compat.Minecraft112Compat.scale(away, 
        ...
        if (state.role() == EndermanCargoRole.SPIDER_PLATFORM
            ...
            if (spiderTarget != null && spiderTarget.isEntityAlive()) {
                ...
                Vec3 away = horizontal(fr.vanillainstincts.compat.Minecraft17Compat.position(enderman)
                        .subtract(fr.vanillainstincts.compat.Minecraft17Compat.position(spiderTarget)));
                if (away == null) away = new Vec3(1.0D, 0.0D, 0.0D);
                Vec3 side = fr.vanillainstincts.compat.Minecraft112Compat.scale(new Vec3(-away.zCoord, 0.0D, away.xCoord), (enderman.getEntityId() & 1) == 0 ? 3.0D : -3.0D);
                Vec3 destination = fr.vanillainstincts.compat.Minecraft17Compat.position(spiderTarget)
                        .add(fr.vanillainstincts.compat.Minecraft112Compat.scale(away, 7.0D)).add(side);
                offerTacticalTeleport(enderman, cargo, state, plan, level,
        ...
        if (state.role() == EndermanCargoRole.ARCHER_PLATFORM
                && cargo instanceof EntitySkeleton) { EntitySkeleton skeleton = (EntitySkeleton) (cargo); 
            if (objective != null) {
    ...
    private static void offerCarryNavigation(EntityEnderman enderman,
        ...
        if (fr.vanillainstincts.compat.Minecraft112Compat.distanceSq(enderman, objective)
            ...
            enderman.getNavigator().clearPathEntity();
            return;
        }
        plan.offerNavigation(state, ActionOwner.ENDERMAN_TACTICS,
                priority, fr.vanillainstincts.compat.Minecraft17Compat.position(objective), 1.0D,
    ...
    private static void offerApproachCargoTeleport(EntityEnderman enderman,
        ...
        Vec3 away = horizontal(fr.vanillainstincts.compat.Minecraft17Compat.position(enderman).subtract(fr.vanillainstincts.compat.Minecraft17Compat.position(cargo)));
        if (away == null) {
            away = new Vec3(1.0D, 0.0D, 0.0D);
        }
        Optional<Vec3> safe = SafePositionFinder.resolveGroundDestination(
                enderman, fr.vanillainstincts.compat.Minecraft17Compat.position(cargo).add(fr.vanillainstincts.compat.Minecraft112Compat.scale(away, 1.4D)));
        if (!safe.isPresent()) {
            return;
        }
        ...
        plan.offerSpecial(VanillaInstinctsState.TELEPORT_CARGO,
                ...
                () -> {
                    Vec3 before = fr.vanillainstincts.compat.Minecraft17Compat.position(enderman);
                    fr.vanillainstincts.compat.Minecraft112Compat.teleport(enderman, destination.xCoord, destination.yCoord,
                            destination.zCoord);
    ...
    private static void offerTacticalTeleport(EntityEnderman enderman, Entity cargo,
        ...
        Vec3 destination = safe.get();
        if (fr.vanillainstincts.compat.Minecraft112Compat.distanceSq(fr.vanillainstincts.compat.Minecraft17Compat.position(enderman), destination) < 9.0D) {
            return;
        }
    ...
    private static void offerIdleCarryTeleport(EntityEnderman enderman, Entity cargo,
        ...
        Vec3 desired =fr.vanillainstincts.compat.Minecraft112Compat.add(fr.vanillainstincts.compat.Minecraft17Compat.position(enderman), Math.cos(angle) * distance,
    ...
    public static EndermanCargoRole roleFor(Entity cargo) {
    ...
    public static boolean canCarryType(Entity cargo) {
        return EndermanCargoSelector.canCarryType(cargo);
    }

    public static boolean isSafeCargoCandidate(Entity cargo) {
        return EndermanCargoSelector.isSafeCargoCandidate(cargo);
    }

    private static EntityLivingBase chooseObjective(EntityEnderman enderman,
                                                          WorldServer level) {
        if (enderman.getAttackTarget() instanceof EntityPlayer
                && validObjective(((EntityPlayer) (enderman.getAttackTarget())))
    ...
    private static EntityPlayer nearestPlayerRecipient(EntityEnderman enderman,
                                                            WorldServer level) {
    ...
    private static Entity chooseCargo(EntityEnderman enderman,
                                                EntityLivingBase objective,
                                                WorldServer level) {
    ...
    private static boolean validObjective(EntityLivingBase entity,
                                          Entity cargo) {
        return entity != null && entity != cargo && entity.isEntityAlive();
    }

    private static EntityLivingBase resolveEffectiveObjective(
    ...
    public static void positionCargoPassenger(EntityEnderman enderman,
        ...
        Vec3 position = cargoCarryPosition(fr.vanillainstincts.compat.Minecraft17Compat.position(enderman),
                enderman.rotationYaw, cargo.width, cargo.height);
        fr.vanillainstincts.compat.Minecraft112Compat.teleport(cargo, position.xCoord, position.yCoord, position.zCoord);
        cargo.fallDistance = 0.0F;
    }
    ...
    public static Vec3 cargoCarryPosition(Vec3 endermanPosition, float yaw,
                                          double cargoWidth,
                                          double cargoHeight) {
        Vec3 origin = endermanPosition == null ? new Vec3(0.0D, 0.0D, 0.0D) : endermanPosition;
    ...
    private static Entity resolveCargo(WorldServer level,
                                                 EntityEnderman enderman,
                                                 EndermanRuntimeState state) {
        if (state.cargoId() != null) {
            Entity entity = fr.vanillainstincts.compat.Minecraft17Compat.getEntityFromUuid(level, state.cargoId());
            if (entity != null) {
                return entity;
            }
        }
    ...
    private static EntityLivingBase resolveObjective(
            WorldServer level, EndermanRuntimeState state) {
        if (state.objectiveId() == null) {
            return null;
        }
        Entity entity = fr.vanillainstincts.compat.Minecraft17Compat.getEntityFromUuid(level, state.objectiveId());
    ...
    private static void offerCreeperReleaseAndRetreat(
        ...
        Vec3 release = resolveCreeperReleaseDestination(
                creeper, enderman, objective).orElse(fr.vanillainstincts.compat.Minecraft17Compat.position(enderman));
        Vec3 retreat = resolveCreeperRetreatDestination(
                enderman, objective, release).orElse(fr.vanillainstincts.compat.Minecraft17Compat.position(enderman));
        plan.offerSpecial(VanillaInstinctsState.RELEASE_CARGO,
                ActionOwner.ENDERMAN_TACTICS,
    ...
    private static void releaseCreeperAndRetreatNow(
        ...
        Vec3 release = resolveCreeperReleaseDestination(creeper, enderman,
                objective).orElse(fr.vanillainstincts.compat.Minecraft17Compat.position(enderman));
        Vec3 retreat = resolveCreeperRetreatDestination(enderman, objective,
                release).orElse(fr.vanillainstincts.compat.Minecraft17Compat.position(enderman));
    ...
    private static Optional<Vec3> resolveCreeperReleaseDestination(
        ...
        Vec3 requested = objective == null
                ? fr.vanillainstincts.compat.Minecraft17Compat.position(enderman).add(fr.vanillainstincts.compat.Minecraft112Compat.scale(fr.vanillainstincts.compat.Minecraft17Compat.look(enderman), 1.6D))
        ...
        for (double radius : radii) {
            for (int index = 0; index < 8; index++) {
                ...
                Vec3 candidate =fr.vanillainstincts.compat.Minecraft112Compat.add(fr.vanillainstincts.compat.Minecraft17Compat.position(objective), 
    ...
    private static Optional<Vec3> resolveCreeperRetreatDestination(
            EntityEnderman enderman, EntityLivingBase objective,
        ...
        Vec3 away = objective == null
                ? horizontal(fr.vanillainstincts.compat.Minecraft17Compat.position(enderman).subtract(releasePosition))
                : horizontal(fr.vanillainstincts.compat.Minecraft17Compat.position(enderman).subtract(fr.vanillainstincts.compat.Minecraft17Compat.position(objective)));
        if (away == null) {
            away = horizontal(fr.vanillainstincts.compat.Minecraft112Compat.scale(fr.vanillainstincts.compat.Minecraft17Compat.look(enderman), -1.0D));
        }
        if (away == null) {
            away = new Vec3(1.0D, 0.0D, 0.0D);
        }
        ...
        for (double distance : distances) {
            for (double lateral : laterals) {
                Vec3 candidate = releasePosition.add(fr.vanillainstincts.compat.Minecraft112Compat.scale(away, distance))
                        .add(fr.vanillainstincts.compat.Minecraft112Compat.scale(side, lateral));
                Optional<Vec3> safe = SafePositionFinder
    ...
    private static void releaseCreeperAndRetreat(
        ...
        if (fr.vanillainstincts.compat.Minecraft112Compat.vehicle(creeper) == enderman) {
            ...
            if (!fr.vanillainstincts.compat.Minecraft112Compat.removed(creeper)) {
                fr.vanillainstincts.compat.Minecraft112Compat.teleport(creeper, release.xCoord, release.yCoord, release.zCoord);
                ...
                if (objective != null && objective.isEntityAlive()) {
                    ...
                    fr.vanillainstincts.compat.Minecraft17Compat.igniteCreeper(creeper);
                } else {
                    fr.vanillainstincts.compat.Minecraft112Compat.setCreeperSwell(creeper, -1);
                }
            }
        ...
        if (!fr.vanillainstincts.compat.Minecraft112Compat.removed(enderman)) {
            fr.vanillainstincts.compat.Minecraft112Compat.teleport(enderman, retreat.xCoord, retreat.yCoord, retreat.zCoord);
    ...
    private static void releaseHostileCargoAndRetreat(
        ...
        Vec3 away = objective == null
                ? horizontal(fr.vanillainstincts.compat.Minecraft112Compat.scale(fr.vanillainstincts.compat.Minecraft17Compat.look(enderman), -1.0D))
                : horizontal(fr.vanillainstincts.compat.Minecraft17Compat.position(enderman).subtract(fr.vanillainstincts.compat.Minecraft17Compat.position(objective)));
        if (away == null) {
            away = new Vec3(1.0D, 0.0D, 0.0D);
        }
        Vec3 requested = fr.vanillainstincts.compat.Minecraft17Compat.position(enderman).add(fr.vanillainstincts.compat.Minecraft112Compat.scale(away, 9.0D));
    ...
    private static void releaseCargo(EntityEnderman enderman,
        ...
        if (cargo != null && fr.vanillainstincts.compat.Minecraft112Compat.vehicle(cargo) == enderman) {
            ...
            if (!fr.vanillainstincts.compat.Minecraft112Compat.removed(cargo) && cargo.worldObj == level) {
                Optional<Vec3> safe =
                        SafePositionFinder.resolveGroundDestination(
                                enderman, fr.vanillainstincts.compat.Minecraft17Compat.position(enderman).add(
                                        fr.vanillainstincts.compat.Minecraft112Compat.scale(fr.vanillainstincts.compat.Minecraft17Compat.look(enderman), 1.4D)));
                safe.ifPresent(destination -> fr.vanillainstincts.compat.Minecraft112Compat.teleport(cargo, destination.xCoord,
    ...
    private static Vec3 creeperDeliveryPosition(EntityLivingBase objective,
                                                EntityEnderman enderman) {
        Vec3 away = horizontal(fr.vanillainstincts.compat.Minecraft17Compat.position(enderman)
                .subtract(fr.vanillainstincts.compat.Minecraft17Compat.position(objective)));
        if (away == null) {
            away = horizontal(fr.vanillainstincts.compat.Minecraft112Compat.scale(fr.vanillainstincts.compat.Minecraft17Compat.look(objective), -1.0D));
        }
        if (away == null) {
            away = new Vec3(1.0D, 0.0D, 0.0D);
        }
        Vec3 lateral = fr.vanillainstincts.compat.Minecraft112Compat.scale(new Vec3(-away.zCoord, 0.0D, away.xCoord), (enderman.getEntityId() & 1) == 0 ? 0.75D : -0.75D);
        return fr.vanillainstincts.compat.Minecraft17Compat.position(objective).add(fr.vanillainstincts.compat.Minecraft112Compat.scale(away, 
    ...
    private static Vec3 creeperRetreatPosition(
            EntityEnderman enderman, EntityLivingBase objective,
            Vec3 releasePosition) {
        Vec3 away = objective == null ? horizontal(fr.vanillainstincts.compat.Minecraft17Compat.position(enderman)
                .subtract(releasePosition)) : horizontal(fr.vanillainstincts.compat.Minecraft17Compat.position(enderman)
                .subtract(fr.vanillainstincts.compat.Minecraft17Compat.position(objective)));
        if (away == null) {
            away = horizontal(fr.vanillainstincts.compat.Minecraft112Compat.scale(fr.vanillainstincts.compat.Minecraft17Compat.look(enderman), -1.0D));
        }
        ...
        Vec3 lateral = fr.vanillainstincts.compat.Minecraft112Compat.scale(new Vec3(-away.zCoord, 0.0D, away.xCoord), (enderman.getEntityId() & 1) == 0
                        ? EndermanRules.ENDERMAN_CREEPER_RETREAT_LATERAL
                        : -EndermanRules.ENDERMAN_CREEPER_RETREAT_LATERAL);
        return releasePosition.add(fr.vanillainstincts.compat.Minecraft112Compat.scale(away, 
                EndermanRules.ENDERMAN_CREEPER_RETREAT_DISTANCE)).add(lateral);
    }

    private static Vec3 archerDestination(EntityLivingBase objective,
                                          EntityEnderman enderman,
                                          boolean retreat) {
        Vec3 away = horizontal(fr.vanillainstincts.compat.Minecraft17Compat.position(enderman)
                .subtract(fr.vanillainstincts.compat.Minecraft17Compat.position(objective)));
        ...
        Vec3 lateral = fr.vanillainstincts.compat.Minecraft112Compat.scale(new Vec3(-away.zCoord, 0.0D, away.xCoord), (enderman.getEntityId() & 1) == 0 ? 2.5D : -2.5D);
        return fr.vanillainstincts.compat.Minecraft17Compat.position(objective).add(fr.vanillainstincts.compat.Minecraft112Compat.scale(away, distance)).add(lateral);
    }

    private static Vec3 presentationDestination(EntityLivingBase objective,
                                                EntityEnderman enderman) {
        Vec3 look = horizontal(fr.vanillainstincts.compat.Minecraft17Compat.look(objective));
        if (look == null) {
            look = horizontal(fr.vanillainstincts.compat.Minecraft17Compat.position(objective).subtract(
                    fr.vanillainstincts.compat.Minecraft17Compat.position(enderman)));
        }
        if (look == null) {
            return fr.vanillainstincts.compat.Minecraft17Compat.position(objective);
        }
        Vec3 lateral = fr.vanillainstincts.compat.Minecraft112Compat.scale(new Vec3(-look.zCoord, 0.0D, look.xCoord), (enderman.getEntityId() & 1) == 0 ? 1.4D : -1.4D);
        return fr.vanillainstincts.compat.Minecraft17Compat.position(objective).subtract(fr.vanillainstincts.compat.Minecraft112Compat.scale(look, 
    ...
    private static Optional<Vec3> playerExtractionDestination(
        ...
        for (int i = 0; i < 12; i++) {
            ...
            Vec3 requested =fr.vanillainstincts.compat.Minecraft112Compat.add(fr.vanillainstincts.compat.Minecraft17Compat.position(enderman), 
                    Math.cos(angle) * radius, 0.0D,
                    Math.sin(angle) * radius);
            Optional<Vec3> safe = SafePositionFinder.resolveGroundDestination(
                    enderman, requested);
            if (safe.isPresent() && !fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, 
                    new BlockPos(safe.get().xCoord, safe.get().yCoord, safe.get().zCoord)).getBlock().getMaterial().isLiquid()) {
                return safe;
            }
        }
        return Optional.empty();
    ...
    private static boolean safePlayerRelease(EntityEnderman enderman,
        ...
        return !enderman.isInWater()
                && !fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, entityBlockPos(enderman)).getBlock().getMaterial().isLiquid()
                && enderman.onGround;
    }
    ...
    private static void emitTeleportParticles(WorldServer level, Vec3 pos) {
        fr.vanillainstincts.compat.Minecraft17Compat.spawnParticle(level, EnumParticleTypes.PORTAL,
                pos.xCoord, pos.yCoord + 1.0D, pos.zCoord,
