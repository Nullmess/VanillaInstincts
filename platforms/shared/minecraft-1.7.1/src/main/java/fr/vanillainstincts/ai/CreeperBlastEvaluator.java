...
public final class CreeperBlastEvaluator {
    ...
    public static double estimatedDamage(EntityCreeper creeper,
        ...
        Vec3 center =fr.vanillainstincts.compat.Minecraft112Compat.add(fr.vanillainstincts.compat.Minecraft17Compat.position(creeper), 0.0D,
                creeper.height * 0.5D, 0.0D);
        double distance = center.distanceTo(fr.vanillainstincts.compat.Minecraft110Compat.boxCenter(fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(target)));
        if (distance > power * 2.0D) {
            return 0.0D;
        }
        double exposure = fr.vanillainstincts.compat.Minecraft112Compat.explosionExposure(creeper.worldObj, center, target);
