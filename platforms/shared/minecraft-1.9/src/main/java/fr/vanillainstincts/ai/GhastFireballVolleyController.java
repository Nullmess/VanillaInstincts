...
public final class GhastFireballVolleyController {
    ...
    public static boolean onFireballJoin(EntityFireball fireball,
        ...
        ballData.setUniqueId(SOURCE_GHAST, ghast.getUniqueID());
        if (ghast.getAttackTarget() instanceof EntityPlayer) { EntityPlayer player = (EntityPlayer) (ghast.getAttackTarget()); 
            ballData.setUniqueId(TARGET_PLAYER, player.getUniqueID());
        }
        ballData.setInteger(RETURNS, 0);
        ballData.setInteger(MAX_RETURNS, maxReturns(level.getDifficulty(),fr.vanillainstincts.compat.Minecraft112Compat.random(ghast).nextInt()));
    ...
    public static void tickFireball(EntityFireball fireball,
        ...
        if (!ballData.getBoolean(TRACKED)
                || !ballData.hasUniqueId(SOURCE_GHAST)) {
            return;
        }

        Entity source = level.getEntityFromUuid(ballData.getUniqueId(SOURCE_GHAST));
        if (!(source instanceof EntityGhast) || !((EntityGhast) (source)).isEntityAlive()) return; EntityGhast ghast = (EntityGhast) (source);
        EntityPlayer player = null;
        if (ballData.hasUniqueId(TARGET_PLAYER)) {
            Entity target = level.getEntityFromUuid(ballData.getUniqueId(TARGET_PLAYER));
        ...
        if (player == null || !player.isEntityAlive() || player.isCreative()
                || player.isSpectator()) {
            return;
        }
        ...
        Vec3d toGhast = ghast.getPositionEyes(1.0F).subtract(fireball.getPositionVector());
    ...
    private static void returnToPlayer(EntityFireball fireball, EntityGhast ghast,
        ...
        Vec3d destination = predictedTarget(player, fireball.getPositionVector(), speed);
        Vec3d direction = destination.subtract(fireball.getPositionVector());
        if (fr.vanillainstincts.compat.Minecraft112Compat.lengthSq(direction) < 1.0E-6D) {
            direction = player.getPositionEyes(1.0F).subtract(fireball.getPositionVector());
        }
        ...
        fr.vanillainstincts.compat.Minecraft112Compat.setMotion(fireball, direction.normalize().scale(speed));
        ...
        level.playSound(null, entityBlockPos(ghast), SoundEvents.ENTITY_GHAST_WARN,
                SoundCategory.HOSTILE, 1.35F, pitch);
        level.playSound(null, entityBlockPos(fireball), SoundEvents.ENTITY_GHAST_SHOOT,
    ...
    private static EntityGhast nearestSourceGhast(EntityFireball fireball,
        ...
        for (EntityGhast candidate : level.getEntitiesWithinAABB(
                EntityGhast.class, fireball.getEntityBoundingBox().expandXyz(16.0D),
                EntityGhast::isEntityAlive)) {
            if (!(candidate.getAttackTarget() instanceof EntityPlayer)) continue;
            double distance = fr.vanillainstincts.compat.Minecraft112Compat.distanceSq(candidate.getPositionVector(), fireball.getPositionVector());
    ...
    private static Vec3d predictedTarget(EntityPlayer player, Vec3d origin,
                                        double speed) {
        double distance = Math.sqrt(fr.vanillainstincts.compat.Minecraft112Compat.distanceSq(origin, player.getPositionEyes(1.0F)));
        double leadTicks = Math.max(2.0D, Math.min(14.0D,
                distance / Math.max(0.1D, speed)));
        return player.getPositionEyes(1.0F).add(
                fr.vanillainstincts.compat.Minecraft112Compat.motion(player).scale(leadTicks));
    }
