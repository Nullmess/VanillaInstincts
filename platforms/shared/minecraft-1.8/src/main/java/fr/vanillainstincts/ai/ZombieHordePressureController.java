...
public final class ZombieHordePressureController {
    ...
    public static void contribute(EntityZombie zombie, MobDecisionPlan plan,
        ...
        Vec3 direction = horizontalDirection(zombie.getPositionVector(), destination);
        if (direction == null) {
            return;
        }
    ...
    public static List<EntityZombie> rearAllies(EntityZombie zombie, WorldServer level,
        ...
        List<EntityZombie> candidates = fr.vanillainstincts.compat.Minecraft112Compat.entitiesWithinAABB(level, EntityZombie.class,
                zombie.getEntityBoundingBox().expand(ZombieRules.PRESSURE_SCAN_RADIUS, ZombieRules.PRESSURE_SCAN_RADIUS, ZombieRules.PRESSURE_SCAN_RADIUS),
        ...
        for (EntityZombie other : candidates) {
            Vec3 relative = fr.vanillainstincts.compat.Minecraft112Compat.multiply(other.getPositionVector().subtract(zombie.getPositionVector()), 1.0D, 0.0D, 1.0D);
    ...
    private static boolean hasStackClearance(EntityZombie zombie,
        ...
        return !fr.vanillainstincts.compat.Minecraft112Compat.hasCollision(level.getBlockState(top), level, top)
                ...
                && !level.getBlockState(top).getBlock().getMaterial().isLiquid()
                && !level.getBlockState(top.up()).getBlock().getMaterial().isLiquid();
    }
}
