...
public final class FarmerCropRegistry {
    ...
    public static boolean isMature(IBlockState state) {
        ...
        if (state.getBlock() instanceof BlockCrops) { BlockCrops crop = (BlockCrops) (state.getBlock()); 
            return crop.getMetaFromState(state) >= 7;
        }
        return false;
    }
    ...
    public static class CropDefinition {
        ...
        public boolean isMature(IBlockState state) {
            ...
            if (ageProperty == null || ageProperty.trim().isEmpty()) {
                return crop instanceof BlockCrops
                        && ((BlockCrops) (crop)).getMetaFromState(state) >= 7;
            }
            IProperty property = null;
            for (Object candidateObject : state.getPropertyNames()) {
                if (!(candidateObject instanceof IProperty)) continue;
                IProperty candidate = (IProperty) candidateObject;
                if (ageProperty.equals(candidate.getName())) {
                    property = candidate;
                    break;
                }
            }
            ...
            int current = ((Number) state.getValue(integerProperty)).intValue();
            int minimum = ((Number) java.util.Collections.min(integerProperty.getAllowedValues())).intValue();
            int maximum = ((Number) java.util.Collections.max(integerProperty.getAllowedValues())).intValue();
        ...
        public IBlockState plantingState() {
            ...
            IProperty property = propertyByName(crop, ageProperty);
    ...
    private static IProperty propertyByName(Block block, String name) {
        if (block == null || name == null) return null;
        for (Object propertyObject : block.getBlockState().getProperties()) {
            if (!(propertyObject instanceof IProperty)) continue;
