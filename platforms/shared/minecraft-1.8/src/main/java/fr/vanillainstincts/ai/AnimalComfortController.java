...
public final class AnimalComfortController {
    ...
    public static ComfortNeed currentEnvironmentalNeed(
        ...
        if (profile.needsShelter()
                && fr.vanillainstincts.compat.Minecraft112Compat.isRainingAt(level, pos) && level.canSeeSky(pos.up())) {
    ...
    private static Vec3 localHerdAnchor(EntityAnimal animal, WorldServer level) {
        List<EntityAnimal> herd = fr.vanillainstincts.compat.Minecraft112Compat.entitiesWithinAABB(level, EntityAnimal.class,
                animal.getEntityBoundingBox().expand(AnimalRules.ANIMAL_HERD_RADIUS * 0.65D, AnimalRules.ANIMAL_HERD_RADIUS * 0.65D, AnimalRules.ANIMAL_HERD_RADIUS * 0.65D),
        ...
        for (int dx = -radius; dx <= radius; dx++) {
            for (int dz = -radius; dz <= radius; dz++) {
                ...
                for (int dy = -1; dy <= 1; dy++) {
                    if (level.getBlockState(fr.vanillainstincts.compat.Minecraft112Compat.offset(feet, dx, dy, dz)).getBlock().getMaterial() == Material.water) {
                        return true;
                    }
                }
            }
        ...
        for (int dx = -1; dx <= 1; dx++) {
            for (int dz = -1; dz <= 1; dz++) {
                ...
                if (!fr.vanillainstincts.compat.Minecraft112Compat.hasCollision(level.getBlockState(neighbour), level, neighbour)
                        ...
                        && !level.getBlockState(neighbour).getBlock().getMaterial().isLiquid()) {
                    open++;
                }
            }
        }
    ...
    private static boolean isComfortStandingPosition(EntityAnimal animal,
        ...
        for (int y = feet.getY(); y <= occupiedTopY; y++) {
            ...
            if (!!level.getBlockState(occupied).getBlock().getMaterial().isLiquid()
