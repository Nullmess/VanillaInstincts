...
public final class MobRelationManager {
    ...
    public static Optional<RelationDecision> relation(EntityLiving observer,
        ...
        ResourceLocation observerType = EntityList.getKey(observer);
        ResourceLocation targetType = EntityList.getKey(target);
    ...
    public static double maximumHuntDistance(EntityLiving observer) {
        ...
        return MAX_HUNT_DISTANCE.computeIfAbsent(EntityList.getKey(observer), type -> {
            double maximum = 0.0D;
            for (RelationRule rule : rules) {
    ...
    public static double maximumRelationDistance(EntityLiving observer) {
        if (observer == null) return 0.0D;
        return MAX_RELATION_DISTANCE.computeIfAbsent(EntityList.getKey(observer), type -> {
            double maximum = 0.0D;
            for (RelationRule rule : rules) {
