...
public final class FishermanController {
    ...
    public static boolean contribute(EntityVillager fisher,
        ...
        if (fr.vanillainstincts.compat.Minecraft112Compat.distanceSq(fisher.getPositionVector(), bank)
    ...
    public static void maintain(EntityVillager fisher, WorldServer level,
        ...
        if (catchEntity != null) {
            fr.vanillainstincts.compat.Minecraft112Compat.setMotion(catchEntity,
                    fr.vanillainstincts.compat.Minecraft112Compat.add(
                            fisher.getPositionVector().subtract(catchEntity.getPositionVector())
        ...
        fisher.setHeldItem(EnumHand.MAIN_HAND,
        ...
        if (gameTime == biteAt) {
            ...
            level.spawnParticle(EnumParticleTypes.WATER_SPLASH, hook.posX, hook.posY,
                    hook.posZ, 8, 0.25D, 0.08D, 0.25D, 0.06D);
            level.playSound(null, entityBlockPos(hook),
                    SoundEvents.ENTITY_BOBBER_SPLASH, SoundCategory.NEUTRAL,
                    0.8F, 0.9F +fr.vanillainstincts.compat.Minecraft112Compat.random(fisher).nextFloat() * 0.2F);
        }
        if (gameTime >= reelAt) {
    ...
    private static void maintainCast(EntityVillager fisher, WorldServer level,
        ...
        if (water == null || gameTime >= castEnd) {
            fr.vanillainstincts.compat.Minecraft112Compat.setMotion(hook, Vec3d.ZERO);
            if (water != null) {
        ...
        Vec3d target =fr.vanillainstincts.compat.Minecraft112Compat.add(Minecraft115VectorCompat.atCenterOf(water), 0.0D, 0.32D, 0.0D);
        Vec3d delta = target.subtract(hook.getPositionVector());
        if (fr.vanillainstincts.compat.Minecraft112Compat.lengthSq(delta) <= 0.36D) {
            fr.vanillainstincts.compat.Minecraft112Compat.teleport(hook, target.x, target.y, target.z);
            fr.vanillainstincts.compat.Minecraft112Compat.setMotion(hook, Vec3d.ZERO);
            fisher.getEntityData().setLong(CAST_END_AT, gameTime);
            level.playSound(null, water, SoundEvents.ENTITY_BOBBER_SPLASH,
                    SoundCategory.NEUTRAL, 0.55F, 1.15F);
            return;
        }
        fr.vanillainstincts.compat.Minecraft112Compat.setMotion(hook, delta.normalize().scale(0.72D));
    }
    ...
    private static void renderLine(EntityVillager fisher, WorldServer level,
        ...
        Vec3d start =fr.vanillainstincts.compat.Minecraft112Compat.add(fisher.getPositionVector(), 0.0D, 1.35D, 0.0D);
        ...
        for (int index = 1; index < points; index++) {
            ...
            level.spawnParticle(EnumParticleTypes.CRIT, point.x, point.y, point.z,
    ...
    private static void cast(EntityVillager fisher, WorldServer level,
        ...
        fisher.setHeldItem(EnumHand.MAIN_HAND,
                new ItemStack(Items.FISHING_ROD));
        fisher.swingArm(EnumHand.MAIN_HAND);
        Vec3d origin =fr.vanillainstincts.compat.Minecraft112Compat.add(fisher.getPositionVector(), 0.0D, 1.35D, 0.0D);
        Vec3d target =fr.vanillainstincts.compat.Minecraft112Compat.add(Minecraft115VectorCompat.atCenterOf(water), 0.0D, 0.32D, 0.0D);
        EntityItem hook = new EntityItem(level, origin.x, origin.y, origin.z,
        ...
        hook.setThrower(fisher.getUniqueID().toString());
        fr.vanillainstincts.compat.Minecraft112Compat.setMotion(hook, target.subtract(origin).normalize().scale(0.72D));
        ...
        level.spawnEntity(hook);
        fisher.getEntityData().setUniqueId(HOOK_ID, hook.getUniqueID());
    ...
    private static void reel(EntityVillager fisher, WorldServer level,
                             EntityItem hook, long gameTime) {
        Vec3d origin = hook.getPositionVector();
        ...
        EntityItem catchEntity = new EntityItem(level, origin.x, origin.y,
                origin.z, catchStack);
        fr.vanillainstincts.compat.Minecraft112Compat.setPickupDelay(catchEntity, 32_767);
        catchEntity.setThrower(fisher.getUniqueID().toString());
        catchEntity.getEntityData().setBoolean(
                VillagerFoodExchangeController.VILLAGE_ORIGIN, true);
        fr.vanillainstincts.compat.Minecraft112Compat.setMotion(catchEntity,
                fr.vanillainstincts.compat.Minecraft112Compat.add(
                        fisher.getPositionVector().subtract(origin).normalize().scale(0.22D),
                        0.0D, 0.18D, 0.0D));
        level.spawnEntity(catchEntity);
        fisher.getEntityData().setUniqueId(CATCH_ID, catchEntity.getUniqueID());
    ...
    private static void collectCatch(EntityVillager fisher, WorldServer level,
        ...
        fisher.playSound(SoundEvents.ENTITY_ITEM_PICKUP, 0.5F,
        ...
        if (!represented) {
            ...
            ProfessionStockController.insert(fisher.getVillagerInventory(), social);
            if (!social.isEmpty()) fr.vanillainstincts.compat.Minecraft112Compat.spawnItem(fisher, social);
            fisher.getVillagerInventory().markDirty();
        }
        fisher.setHeldItem(EnumHand.MAIN_HAND, ItemStack.EMPTY);
        fisher.getEntityData().setLong(READY_AT,
    ...
    public static boolean isFishable(WorldServer level, BlockPos pos) {
        if (level == null || pos == null || !level.isBlockLoaded(pos)) return false;
        IBlockState state = level.getBlockState(pos);
        return state.getBlock() == Blocks.WATER
                && fr.vanillainstincts.compat.Minecraft112Compat.isAir(level.getBlockState(pos.up()));
    }

    private static Vec3d bankPosition(WorldServer level, BlockPos water,
                                     EntityVillager fisher) {
        return fr.vanillainstincts.compat.LegacyJava8.listOf(water.north(), water.south(), water.east(), water.west())
                .stream()
                .filter(pos -> fr.vanillainstincts.compat.Minecraft112Compat.isAir(level.getBlockState(pos))
                        && fr.vanillainstincts.compat.Minecraft112Compat.isAir(level.getBlockState(pos.up()))
                        && fr.vanillainstincts.compat.Minecraft112Compat.isSolidRender(level.getBlockState(pos.down())))
    ...
    private static boolean hasUuid(EntityVillager villager, String key) {
        return villager.getEntityData().hasUniqueId(key);
    }
    ...
    private static EntityItem resolveItem(WorldServer level, EntityVillager villager,
        ...
        Entity entity = level.getEntityFromUuid(villager.getEntityData().getUniqueId(key));
