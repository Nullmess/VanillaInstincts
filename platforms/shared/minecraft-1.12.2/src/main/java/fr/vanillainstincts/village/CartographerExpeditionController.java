...
public final class CartographerExpeditionController {
    ...
    public static void maintain(EntityVillager villager, WorldServer level,
        ...
        villager.setHeldItem(EnumHand.MAIN_HAND, visual);
    }
    ...
    private static boolean tryStart(EntityVillager villager,
        ...
        Optional<Vec3d> destination = findDestination(villager, level,
                sequence);
        ...
        Vec3d acceptedDestination = destination.get();
    ...
    private static boolean travel(EntityVillager villager, MobDecisionPlan plan,
        ...
        if (target == null || !level.isBlockLoaded(target)) {
        ...
        if (fr.vanillainstincts.compat.Minecraft112Compat.distanceSq(villager.getPositionVector(), safe.get())
    ...
    private static void maintainSurvey(EntityVillager villager, WorldServer level,
        ...
        if (!represented) {
            ...
            ProfessionStockController.insert(villager.getVillagerInventory(), social);
            if (!social.isEmpty()) fr.vanillainstincts.compat.Minecraft112Compat.spawnItem(villager, social);
            villager.getVillagerInventory().markDirty();
        }
        villager.setHeldItem(EnumHand.MAIN_HAND, map.copy());
    ...
    private static ItemStack createMap(EntityVillager villager, WorldServer level,
        ...
        return ItemMap.setupNewMap(level, survey.getX(), survey.getZ(),
                (byte) 1, true, true);
    }

    private static Optional<Vec3d> findDestination(EntityVillager villager,
                                                  WorldServer level,
                                                  long sequence) {
        ...
        for (int offset = 0; offset < attempts; offset++) {
            ...
            Vec3d requested =fr.vanillainstincts.compat.Minecraft112Compat.add(villager.getPositionVector(), 
