...
public final class WolfPackController {
    ...
    public static void contribute(EntityWolf wolf, MobDecisionPlan plan,
        ...
        List<EntityWolf> pack = fr.vanillainstincts.compat.Minecraft112Compat.entitiesWithinAABB(level, EntityWolf.class,
                fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(wolf).expand(AnimalRules.WOLF_PACK_RADIUS, AnimalRules.WOLF_PACK_RADIUS, AnimalRules.WOLF_PACK_RADIUS),
        ...
        if (averageRatio < AnimalRules.WOLF_ABANDON_HEALTH_RATIO
                && targetDangerous) {
            wolf.setAttackTarget(null);
            offerRetreat(wolf, fr.vanillainstincts.compat.Minecraft17Compat.position(target), plan);
            return;
        }
    ...
    public static Vec3 flankPosition(EntityLivingBase target, EntityWolf wolf,
        ...
        return fr.vanillainstincts.compat.Minecraft112Compat.add(fr.vanillainstincts.compat.Minecraft17Compat.position(target), Math.cos(angle) * radius, 0.0D,
    ...
    private static void offerRetreat(EntityWolf wolf, Vec3 danger,
        ...
        Vec3 away = horizontal(fr.vanillainstincts.compat.Minecraft17Compat.position(wolf).subtract(danger));
        ...
        plan.offerNavigation(VanillaInstinctsState.WOLF_RETREAT,
                ...
                fr.vanillainstincts.compat.Minecraft17Compat.position(wolf).add(fr.vanillainstincts.compat.Minecraft112Compat.scale(away, 12.0D)), 1.18D,
                AnimalRules.STATE_HOLD_ANIMAL_TICKS,
                () -> wolf.setAttackTarget(null));
    }

    private static Vec3 guardPosition(EntityWolf injured, EntityLivingBase target) {
        Vec3 toward = horizontal(fr.vanillainstincts.compat.Minecraft17Compat.position(target).subtract(fr.vanillainstincts.compat.Minecraft17Compat.position(injured)));
        if (toward == null) toward = new Vec3(1.0D, 0.0D, 0.0D);
        return fr.vanillainstincts.compat.Minecraft17Compat.position(injured).add(fr.vanillainstincts.compat.Minecraft112Compat.scale(toward, 2.0D));
    }
