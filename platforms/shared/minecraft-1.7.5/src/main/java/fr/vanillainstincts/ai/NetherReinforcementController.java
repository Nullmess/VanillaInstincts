...
public final class NetherReinforcementController {
    ...
    public static boolean onAnimalAttacked(EntityAnimal animal,
                                           ...
                                           long gameTime) {
        ensurePersistentState(level == null ? null : fr.vanillainstincts.compat.Minecraft17Compat.server(level));
        ...
        if (kind == null || animal == null || level == null
                ...
                || aggressor.capabilities.isCreativeMode || fr.vanillainstincts.compat.Minecraft17Compat.isSpectator(aggressor)
        ...
        if (existingId != null) {
            EntityLiving existing = resolveMob(fr.vanillainstincts.compat.Minecraft17Compat.server(level), existingId);
            if (existing != null) {
    ...
    public static boolean maintain(EntityLiving mob, WorldServer level,
        ...
        EntityPlayerMP aggressor = aggressorId == null ? null
                : fr.vanillainstincts.compat.Minecraft17Compat.player(fr.vanillainstincts.compat.Minecraft17Compat.server(level), aggressorId);
        if (aggressor != null
                && (aggressor.capabilities.isCreativeMode || fr.vanillainstincts.compat.Minecraft17Compat.isSpectator(aggressor))) {
            if (state.role() == NetherReinforcementRole.REINFORCEMENT
                    && LegacyDimensionType.OVERWORLD.equals(LegacyDimensionType.of(level))) {
    ...
    public static void resetDefeatedAggressor(WorldServer sourceLevel,
        ...
        ensurePersistentState(fr.vanillainstincts.compat.Minecraft17Compat.server(sourceLevel));
        ...
        NetherMissionSavedData.get(fr.vanillainstincts.compat.Minecraft17Compat.server(sourceLevel))
        ...
        for (VirtualExpedition expedition : VIRTUAL_EXPEDITIONS.values()) {
            if (aggressorId.equals(expedition.aggressorId)) {
                ...
                expedition.returnAt = Math.min(expedition.returnAt,
                        gameTime + 20L);
                persistVirtual(fr.vanillainstincts.compat.Minecraft17Compat.server(sourceLevel), expedition);
            }
        }

        for (WorldServer level : fr.vanillainstincts.compat.Minecraft17Compat.server(sourceLevel).worldServers) {
    ...
    public static void onPlayerLoggedOut(EntityPlayerMP player) {
        ...
        MinecraftServer server = fr.vanillainstincts.compat.Minecraft17Compat.server((WorldServer) player.worldObj);
        ensurePersistentState(server);
    ...
    public static void tickDeferred(MinecraftServer server) {
        ...
        for (VirtualExpedition expedition :
            ...
            if (gameTime >= expedition.expiresAt) {
                ...
                if (!fr.vanillainstincts.compat.Minecraft17Compat.isBlockLoaded(overworld, expedition.overworldPortal)) {
            ...
            if (!fr.vanillainstincts.compat.Minecraft17Compat.isBlockLoaded(overworld, expedition.overworldPortal)) continue;
            ...
            EntityPlayerMP aggressor = fr.vanillainstincts.compat.Minecraft17Compat.player(server, 
                    expedition.aggressorId);
            boolean targetAvailable = aggressor != null
                    && aggressor.isEntityAlive()
                    && !aggressor.capabilities.isCreativeMode
                    && !fr.vanillainstincts.compat.Minecraft17Compat.isSpectator(aggressor)
    ...
    private static boolean maintainReinforcement(EntityLiving reinforcement,
        ...
        if (!targetAvailable) {
            long unavailableSince = markAggressorUnavailable(
                    fr.vanillainstincts.compat.Minecraft17Compat.server(level), aggressorId, gameTime);
            calmReinforcement(reinforcement);
            if (shouldRetreatAfterUnavailable(gameTime,
                    unavailableSince)) {
        ...
        clearAggressorUnavailable(fr.vanillainstincts.compat.Minecraft17Compat.server(level), aggressorId);
    ...
    private static boolean moveToStoredPortal(EntityLiving mob,
        ...
        Vec3 destination = Minecraft115VectorCompat.atCenterOf(portal);
    ...
    private static void materializeVirtualExpedition(
        ...
        fr.vanillainstincts.compat.Minecraft17Compat.spawnParticle(overworld, EnumParticleTypes.PORTAL,
        ...
        for (int radius = 1; radius <= 3; radius++) {
            for (int offset = 0; offset < directions.length; offset++) {
                ...
                for (int dy = -1; dy <= 2; dy++) {
                    ...
                    if (!fr.vanillainstincts.compat.Minecraft17Compat.isBlockLoaded(level, feet)) continue;
                    if (fr.vanillainstincts.compat.Minecraft112Compat.isAir(fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, feet))
                            && fr.vanillainstincts.compat.Minecraft112Compat.isAir(fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, feet.up()))
                            && !fr.vanillainstincts.compat.Minecraft112Compat.isAir(fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, feet.down()))) {
    ...
    private static MinecraftServer levelServer(EntityLiving mob) {
        return mob != null && mob.worldObj instanceof WorldServer
                ? fr.vanillainstincts.compat.Minecraft17Compat.server((WorldServer) mob.worldObj) : null;
    }
    ...
    private static EntityLiving resolveMob(MinecraftServer server, UUID entityId) {
        ...
        for (WorldServer level : server.worldServers) {
            Entity entity = fr.vanillainstincts.compat.Minecraft17Compat.getEntityFromUuid(level, entityId);
    ...
    private static void calmZoglinsTargetingMessenger(
        ...
        AxisAlignedBB area = fr.vanillainstincts.compat.Minecraft112Compat.expandBox(fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(messenger), 
                NetherRules.NETHER_RECRUIT_SEARCH_RADIUS);
        for (EntityPigZombie zoglin : fr.vanillainstincts.compat.Minecraft112Compat.entitiesWithinAABB(level, EntityPigZombie.class, area,
    ...
    private static List<EntityLiving> recruitsAround(WorldServer level, EntityLiving center,
                                            NetherReinforcementKind kind,
                                            double radius) {
        AxisAlignedBB area = fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(center).expand(radius, radius, radius);
        List<EntityLiving> result = new ArrayList<>();
        if (kind == NetherReinforcementKind.ZOMBIFIED_PIGLIN) {
            result.addAll(fr.vanillainstincts.compat.Minecraft112Compat.entitiesWithinAABB(level, EntityPigZombie.class,
    ...
    private static int missionMemberCount(WorldServer level, EntityLiving center,
        ...
        AxisAlignedBB area = fr.vanillainstincts.compat.Minecraft112Compat.expandBox(fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(center), 
                NetherRules.NETHER_REINFORCEMENT_COUNT_RADIUS);
        return fr.vanillainstincts.compat.Minecraft112Compat.entitiesWithinAABB(level, EntityLiving.class, area, candidate -> {
                    NetherReinforcementState candidateState =
                            NetherReinforcementState.load(candidate);
