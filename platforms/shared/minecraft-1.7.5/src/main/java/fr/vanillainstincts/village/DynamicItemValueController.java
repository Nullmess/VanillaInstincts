...
public final class DynamicItemValueController {
    ...
    private static double durabilityAdjusted(ItemStack stack, double fullValue) {
        int maximum = Math.max(1, fr.vanillainstincts.compat.Minecraft17Compat.maxDamage(stack));
        double remaining = Math.max(0.0D,
                (maximum - fr.vanillainstincts.compat.Minecraft17Compat.itemDamage(stack)) / (double) maximum);
    ...
    private static double rarityBonus(ItemStack stack) {
        ...
        if (rarity.contains("epic")) return 7.5D;
        if (rarity.contains("rare")) return 3.0D;
        if (rarity.contains("uncommon")) return 0.8D;
        return 0.0D;
    }
    ...
    private static double componentBonus(ItemStack stack) {
        ...
        if (stack.isItemEnchanted()) {
            bonus += 4.0D + Math.min(8.0D,
                    Math.max(1, fr.vanillainstincts.compat.Minecraft17Compat.maxDamage(stack)) / 500.0D);
        }
