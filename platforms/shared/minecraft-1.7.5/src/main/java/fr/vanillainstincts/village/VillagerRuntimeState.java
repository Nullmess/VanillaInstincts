...
public final class VillagerRuntimeState {
    ...
    public static VillagerRuntimeState load(EntityVillager villager) {
        VillagerRuntimeState state = new VillagerRuntimeState();
        NBTTagCompound persistent = villager.getEntityData();
        ...
        state.reportedAggressorId = fr.vanillainstincts.compat.Minecraft18NbtCompat.hasUniqueId(tag, "reported_aggressor_id")
                ? fr.vanillainstincts.compat.Minecraft18NbtCompat.getUniqueId(tag, "reported_aggressor_id") : null;
        state.reportGolemId = fr.vanillainstincts.compat.Minecraft18NbtCompat.hasUniqueId(tag, "report_golem_id")
                ? fr.vanillainstincts.compat.Minecraft18NbtCompat.getUniqueId(tag, "report_golem_id") : null;
    ...
    public void save(EntityVillager villager) {
        if (!dirty) {
            return;
        }
        ...
        tag.setLong("home_until", homeUntil);
        writePosition(tag, "job_site", jobSite);
        tag.setLong("job_site_until", jobSiteUntil);
        writePosition(tag, "farm_target", farmTarget);
        tag.setLong("farm_target_until", farmTargetUntil);
        writePosition(tag, "danger", danger);
        tag.setLong("danger_until", dangerUntil);
        tag.setLong("collective_alert_id", collectiveAlertId);
        writePosition(tag, "shared_shelter", sharedShelter);
        tag.setLong("collective_alert_until", collectiveAlertUntil);
        tag.setLong("alert_cue_id", alertCueId);
        tag.setLong("poi_scan_ready_at", poiScanReadyAt);
        tag.setLong("farm_scan_ready_at", farmScanReadyAt);
        tag.setLong("farm_action_ready_at", farmActionReadyAt);
        tag.setLong("flee_ready_at", fleeReadyAt);
        writePosition(tag, "door_to_close", doorToClose);
        tag.setLong("door_close_at", doorCloseAt);
        tag.setLong("reported_threat_id", reportedThreatId);
        if (reportedAggressorId != null) {
            fr.vanillainstincts.compat.Minecraft18NbtCompat.setUniqueId(tag, "reported_aggressor_id", reportedAggressorId);
        }
        if (reportGolemId != null) {
            fr.vanillainstincts.compat.Minecraft18NbtCompat.setUniqueId(tag, "report_golem_id", reportGolemId);
        }
        tag.setLong("report_until", reportUntil);
        tag.setBoolean("report_delivered", reportDelivered);
        villager.getEntityData().setTag(ROOT_KEY, tag);
        dirty = false;
    }
    ...
    private static BlockPos readPosition(NBTTagCompound tag, String key) {
        return tag.hasKey(key) ? BlockPos.fromLong(tag.getLong(key)) : null;
    }

    private static void writePosition(NBTTagCompound tag, String key,
                                      BlockPos position) {
        if (position != null) {
            tag.setLong(key, position.toLong());
        }
    }
}
