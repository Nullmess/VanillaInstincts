...
public final class DynamicItemValueController {
    ...
    public static double unitValue(ItemStack stack) {
        ...
        if (stack.getItem().equals(Items.ELYTRA)) return durabilityAdjusted(stack, 30.0D);
        if (stack.getItem().equals(Items.TOTEM_OF_UNDYING)) return 24.0D;
        if (stack.getItem().equals(Items.HEART_OF_THE_SEA)) return 18.0D;
        if (stack.getItem().equals(Items.TRIDENT)) {
            return durabilityAdjusted(stack, 20.0D);
        }
        if (stack.getItem().equals(Items.ENCHANTED_GOLDEN_APPLE)) return 22.0D;
        ...
        if (stack.getItem().equals(Items.NAUTILUS_SHELL)) return 3.5D;
    ...
    private static double componentBonus(ItemStack stack) {
        ...
        if (stack.hasCustomHoverName()) bonus += 2.5D;
