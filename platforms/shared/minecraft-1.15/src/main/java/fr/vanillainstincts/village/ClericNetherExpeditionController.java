...
public final class ClericNetherExpeditionController {
    ...
    public static boolean contribute(VillagerEntity cleric,
        ...
        if (!DimensionType.OVERWORLD.equals(level.dimension.getType())) return phase != IDLE;
        ...
        Vec3d destination = Minecraft115VectorCompat.atBottomCenterOf(portal);
    ...
    public static void maintain(VillagerEntity cleric, ServerWorld level,
        ...
        if (!eligible(cleric)) {
            restore(cleric);
            if (!DimensionType.OVERWORLD.equals(level.dimension.getType())
        ...
        if (phase == APPROACH && DimensionType.NETHER.equals(level.dimension.getType())) {
        ...
        if (phase == RETURN && DimensionType.OVERWORLD.equals(level.dimension.getType())) {
        ...
        if (phase == LIVE && DimensionType.NETHER.equals(level.dimension.getType())) {
            maintainLive(cleric, level, gameTime);
            return;
        }
        if (phase == RETURN && DimensionType.NETHER.equals(level.dimension.getType())) {
    ...
    private static ItemStack barterResult(int seed) {
        return fr.vanillainstincts.compat.LegacyJava8.eval(() -> { switch ((Math.floorMod(seed, 8))) { case 0:  return new ItemStack(Items.ENDER_PEARL, 2); case 1:  return new ItemStack(Items.OBSIDIAN, 2); case 2:  return new ItemStack(Items.GLOWSTONE_DUST, 4); case 3:  return new ItemStack(Items.QUARTZ, 5); case 4:  return new ItemStack(Items.MAGMA_CREAM, 2); case 5:  return new ItemStack(Items.SOUL_SAND, 4); case 6:  return new ItemStack(Items.FIRE_CHARGE, 2); default:  return new ItemStack(Items.STRING, 6); } });
    }
    ...
    private static boolean hasNetherObserver(ServerWorld overworld,
                                             BlockPos portal) {
        ServerWorld nether = Minecraft115WorldCompat.world(overworld.getServer(), DimensionType.NETHER);
    ...
    private static void placeBridgeToward(VillagerEntity cleric, ServerWorld level,
                                          BlockPos target) {
        if (!cleric.getNavigation().isDone()) return;
        ...
        BlockPos support = entityBlockPos(cleric).offset(dx, -1, dz);
            ...
            for (int dx = -ring; dx <= ring; dx += 4) {
                for (int dz = -ring; dz <= ring; dz += 4) {
                    ...
                    for (int dy = -vertical; dy <= vertical; dy++) {
                        ...
                        if (level.getBlockState(pos).getBlock().equals(Blocks.SPAWNER)) {
                            return pos;
                        }
                    }
                }
    ...
    private static boolean forceReturnHome(VillagerEntity cleric, ServerWorld level,
                                           long gameTime) {
        ServerWorld overworld = Minecraft115WorldCompat.world(level.getServer(), DimensionType.OVERWORLD);
        ...
        if (cleric.level != overworld) {
            Entity changed = cleric.changeDimension(DimensionType.OVERWORLD);
