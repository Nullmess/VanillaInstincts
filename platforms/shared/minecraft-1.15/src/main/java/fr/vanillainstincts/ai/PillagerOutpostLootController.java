...
public final class PillagerOutpostLootController {
    ...
    public static int onPlayerDrops(PlayerEntity player, ServerWorld level,
                                    Entity killer,
    ...
    private static PillagerEntity selectCarrier(ServerWorld level,
                                          PillagerEntity preferred,
                                          BlockPos death) {
        if (availableForMission(preferred)) return preferred;
        AxisAlignedBB area = new AxisAlignedBB(death.getX(), death.getY(), death.getZ(),
                ...
                PillagerRules.OUTPOST_LOOT_SCAN_RADIUS);
        return level.getEntitiesOfClass(PillagerEntity.class, area,
                        PillagerOutpostLootController::availableForMission)
                .stream()
                .min(Comparator.comparingDouble(pillager ->
                        entityBlockPos(pillager).distSqr(death)))
                .orElse(null);
    }

    public static boolean availableForMission(PillagerEntity pillager) {
        return pillager != null && pillager.isAlive()
                && !pillager.getPersistentData().hasUUID(PILLAGER_MISSION)
                && !pillager.getPersistentData().getBoolean(PILLAGER_RETURNING);
    }
    ...
    private static void beginMission(PillagerEntity pillager, UUID mission,
        ...
        pillager.getPersistentData().putLong(PILLAGER_HOME,
                entityBlockPos(pillager).asLong());
        pillager.getPersistentData().putUUID(PILLAGER_MISSION, mission);
        ...
        for (int index = 0; index < items.size(); index++) {
            ...
            pillager.getPersistentData().putUUID(PILLAGER_ITEM_PREFIX + index,
                    item.getUUID());
    ...
    public static boolean maintain(PillagerEntity pillager, ServerWorld level,
                                   long gameTime) {
        if (pillager == null || level == null || !pillager.isAlive()) {
            return false;
        }
        migrateLegacyAssignment(pillager, level);
        if (pillager.getPersistentData().getBoolean(PILLAGER_RETURNING)) {
            return returnHome(pillager);
        }
        if (!pillager.getPersistentData().hasUUID(PILLAGER_MISSION)) {
            return false;
        }

        pillager.setTarget(null);
        java.util.List<ItemEntity> items = resolveMissionItems(pillager, level);
        BlockPos death = BlockPos.of(pillager.getPersistentData()
                .getLong(PILLAGER_DEATH));
        int expectedCount = Math.max(0, pillager.getPersistentData()
                .getInt(PILLAGER_ITEM_COUNT));
        long startedAt = pillager.getPersistentData().contains(
                PILLAGER_STARTED_AT)
                ? pillager.getPersistentData().getLong(PILLAGER_STARTED_AT)
                : gameTime;
        ...
        boolean carrying = pillager.getPersistentData()
                .getBoolean(PILLAGER_CARRYING);

        if (!carrying) {
            if (entityBlockPos(pillager).distSqr(death)
        ...
        BlockPos chest = BlockPos.of(pillager.getPersistentData()
                .getLong(PILLAGER_CHEST));
        BlockPos depositTarget = nextStorageTarget(level, chest,
                items.get(0).getItem());
        if (entityBlockPos(pillager).distSqr(depositTarget)
        ...
        if (!level.getBlockState(depositTarget).getBlock().equals(Blocks.CHEST)) {
            pillager.swing(Hand.MAIN_HAND);
        }
    ...
    public static void onCarrierDeath(PillagerEntity pillager, ServerWorld level) {
    ...
    public static boolean isReservedLoot(ItemEntity item) {
        return item != null && item.getPersistentData().hasUUID(LOOT_MISSION);
    }

    private static void reserve(ItemEntity item, UUID mission, UUID carrier,
                                BlockPos chest, BlockPos death,
                                long gameTime) {
        item.getPersistentData().putUUID(LOOT_MISSION, mission);
        ...
        item.getPersistentData().putUUID(LOOT_CARRIER, carrier);
    ...
    private static java.util.List<ItemEntity> resolveMissionItems(
            PillagerEntity pillager, ServerWorld level) {
        java.util.List<ItemEntity> result = new java.util.ArrayList<>();
        UUID mission = pillager.getPersistentData().hasUUID(PILLAGER_MISSION)
                ? pillager.getPersistentData().getUUID(PILLAGER_MISSION) : null;
        int count = Math.max(0, pillager.getPersistentData()
                .getInt(PILLAGER_ITEM_COUNT));
        for (int index = 0; index < count; index++) {
            String key = PILLAGER_ITEM_PREFIX + index;
            if (!pillager.getPersistentData().hasUUID(key)) continue;
            Entity entity = level.getEntity(pillager.getPersistentData()
                    .getUUID(key));
            if (entity instanceof ItemEntity && ((ItemEntity) (entity)).isAlive()
                    && isReservedLoot(((ItemEntity) (entity)))
                    && mission != null && mission.equals(((ItemEntity) (entity))
                    .getPersistentData().getUUID(LOOT_MISSION))
                    && ((ItemEntity) (entity)).getPersistentData().hasUUID(LOOT_CARRIER)
                    && pillager.getUUID().equals(((ItemEntity) (entity)).getPersistentData()
                    .getUUID(LOOT_CARRIER))) { ItemEntity item = (ItemEntity) (entity); 
                result.add(item);
            }
        }
        return result;
    ...
    private static void releaseMissionItems(PillagerEntity pillager,
                                            ServerWorld level) {
        if (!pillager.getPersistentData().hasUUID(PILLAGER_MISSION)) return;
        UUID mission = pillager.getPersistentData().getUUID(PILLAGER_MISSION);
        ...
        BlockPos death = pillager.getPersistentData().contains(PILLAGER_DEATH)
                ? BlockPos.of(pillager.getPersistentData().getLong(
        ...
        for (ItemEntity item : level.getEntitiesOfClass(ItemEntity.class, area,
                candidate -> candidate.isAlive()
                        && candidate.getPersistentData().hasUUID(LOOT_MISSION)
                        && mission.equals(candidate.getPersistentData()
                        .getUUID(LOOT_MISSION)))) {
    ...
    private static void migrateLegacyAssignment(PillagerEntity pillager,
                                                 ServerWorld level) {
        if (pillager.getPersistentData().getInt(PILLAGER_ITEM_COUNT) > 0
                || !pillager.getPersistentData().hasUUID(PILLAGER_ITEM)) {
            return;
        }
        UUID itemId = pillager.getPersistentData().getUUID(PILLAGER_ITEM);
        Entity entity = level.getEntity(itemId);
        if (!(entity instanceof ItemEntity) || !((ItemEntity) (entity)).isAlive()) return; ItemEntity item = (ItemEntity) (entity);
        pillager.getPersistentData().putInt(PILLAGER_ITEM_COUNT, 1);
        pillager.getPersistentData().putUUID(PILLAGER_ITEM_PREFIX + 0, itemId);
        if (!pillager.getPersistentData().contains(PILLAGER_DEATH)) {
            long death = item.getPersistentData().contains(LOOT_DEATH)
                    ? item.getPersistentData().getLong(LOOT_DEATH)
                    : entityBlockPos(item).asLong();
            pillager.getPersistentData().putLong(PILLAGER_DEATH, death);
        }
    }

    private static void prepareCarriedItem(PillagerEntity pillager, ItemEntity item,
                                           int index) {
    ...
    private static void deposit(ServerWorld level, PillagerEntity pillager,
     ...
     */
    public static ItemStack insertIntoOutpostStorage(ServerWorld level,
                                                     BlockPos chest,
                                                     ItemStack input) {
        return insertIntoOutpostStorage(level, null, chest, input);
    }

    private static ItemStack insertIntoOutpostStorage(ServerWorld level,
                                                       Entity actor,
                                                       BlockPos chest,
                                                       ItemStack input) {
        ...
        for (int index = 0; index < PillagerRules.OUTPOST_STORAGE_MAX_GROUPS
            ...
            if (level.getBlockState(anchor).getBlock().equals(Blocks.CHEST)) {
    ...
    public static BlockPos nextStorageTarget(ServerWorld level,
                                             BlockPos chest,
                                             ItemStack input) {
        if (level == null || chest == null || input == null
    ...
    private static ItemStack insertChestGroup(ServerWorld level,
                                              Entity actor,
                                              BlockPos center,
                                              ItemStack input,
        ...
        for (Direction direction : Direction.Plane.HORIZONTAL) {
            ...
            if (!level.getBlockState(neighbor).getBlock().equals(Blocks.CHEST)) continue;
    ...
    private static ItemStack insertChestAt(ServerWorld level,
                                           Entity actor,
                                           BlockPos position,
                                           ItemStack input,
                                           Set<BlockPos> visited) {
        if (input.isEmpty() || !visited.add(immutableBlockPos(position))) {
            return input;
        }
    ...
    private static BlockPos firstAcceptingChest(ServerWorld level,
                                                BlockPos center,
                                                ItemStack input) {
    ...
    private static boolean chestCanAccept(ServerWorld level, BlockPos position,
                                          ItemStack input) {
        TileEntity entity = level.getBlockEntity(position);
    ...
    private static BlockPos tryExpandChest(ServerWorld level, Entity actor,
                                           BlockPos base,
                                           Direction preferred) {
    ...
    private static BlockPos expansionPosition(ServerWorld level, BlockPos base,
                                              Direction preferred) {
    ...
    private static boolean placeSingleChest(ServerWorld level, Entity actor,
                                            BlockPos position,
                                            Direction facing) {
    ...
    private static boolean canPlaceSingleChest(ServerWorld level,
                                               BlockPos position) {
    ...
    private static boolean canPlacePartnerChest(ServerWorld level,
                                                BlockPos position,
                                                BlockPos base) {
        if (!isSafeChestSpace(level, position)) return false;
        for (Direction direction : Direction.Plane.HORIZONTAL) {
            ...
            if (level.getBlockState(neighbor).getBlock().equals(Blocks.CHEST)) return false;
        }
        return true;
    }

    private static boolean isSafeChestSpace(ServerWorld level,
                                            BlockPos position) {
    ...
    private static boolean returnHome(PillagerEntity pillager) {
        ...
        BlockPos home = BlockPos.of(pillager.getPersistentData()
                .getLong(PILLAGER_HOME));
        if (entityBlockPos(pillager).distSqr(home)
    ...
    public static BlockPos findOutpostChest(ServerWorld level,
                                            BlockPos origin) {
        ...
        for (int chunkX = minChunkX; chunkX <= maxChunkX; chunkX++) {
            for (int chunkZ = minChunkZ; chunkZ <= maxChunkZ; chunkZ++) {
                ...
                for (Map.Entry<BlockPos, TileEntity> entry
                    ...
                    if (!level.getBlockState(pos).getBlock().equals(Blocks.CHEST)
    ...
    private static int outpostMaterialScore(ServerWorld level,
                                            BlockPos chest) {
        int score = 0;
    ...
    public static boolean isOutpostMaterial(BlockState state) {
        return state != null && (state.getBlock().equals(Blocks.DARK_OAK_LOG)
                || state.getBlock().equals(Blocks.DARK_OAK_PLANKS)
