...
public final class MobEcologyController {
    ...
    public static void contribute(AnimalEntity animal, MobDecisionPlan plan,
        ...
        Vec3d requested = fleeDestination(animal, source);
        if (requested == null) return;
        Vec3d destination = SafePositionFinder.resolveGroundDestination(
    ...
    public static Vec3d fleeDestination(AnimalEntity animal, LivingEntity threat) {
        if (animal == null || threat == null) return null;
        Vec3d away = animal.position().subtract(threat.position());
        Vec3d horizontal = new Vec3d(away.x, 0.0D, away.z);
        if (horizontal.lengthSqr() < 1.0E-6D) {
            double sign = (animal.getId() & 1) == 0 ? 1.0D : -1.0D;
            horizontal = new Vec3d(sign, 0.0D, 0.5D);
        }
        horizontal = horizontal.normalize();
        double side = Math.floorMod(animal.getId(), 3) - 1;
        Vec3d lateral = new Vec3d(-horizontal.z, 0.0D, horizontal.x)
                .scale(side * 0.8D);
