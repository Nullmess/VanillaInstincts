...
public final class WolfPackController {
    ...
    public static void contribute(EntityWolf wolf, MobDecisionPlan plan,
        ...
        List<EntityWolf> pack = level.getEntitiesWithinAABB(EntityWolf.class,
                wolf.getEntityBoundingBox().expandXyz(AnimalRules.WOLF_PACK_RADIUS),
    ...
    public static Vec3d flankPosition(EntityLivingBase target, EntityWolf wolf,
                                     int packSize) {
    ...
    private static void offerRetreat(EntityWolf wolf, Vec3d danger,
                                     MobDecisionPlan plan) {
        Vec3d away = horizontal(wolf.getPositionVector().subtract(danger));
        ...
        plan.offerNavigation(VanillaInstinctsState.WOLF_RETREAT,
                ...
                wolf.getPositionVector().add(away.scale(12.0D)), 1.18D,
                AnimalRules.STATE_HOLD_ANIMAL_TICKS,
                () -> wolf.setAttackTarget(null));
    }

    private static Vec3d guardPosition(EntityWolf injured, EntityLivingBase target) {
        Vec3d toward = horizontal(target.getPositionVector().subtract(injured.getPositionVector()));
        if (toward == null) toward = new Vec3d(1.0D, 0.0D, 0.0D);
        return injured.getPositionVector().add(toward.scale(2.0D));
    }
