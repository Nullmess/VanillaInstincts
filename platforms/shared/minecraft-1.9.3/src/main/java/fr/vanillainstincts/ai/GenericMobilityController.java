...
public final class GenericMobilityController {
    ...
    public static void contribute(EntityLiving mob, MobDecisionPlan plan,
        ...
        safeGapLanding(mob, target, level).ifPresent(landing -> {
            Vec3d horizontal = landing.subtract(mob.getPositionVector());
            horizontal = new Vec3d(horizontal.xCoord, 0.0D, horizontal.zCoord);
            if (fr.vanillainstincts.compat.Minecraft112Compat.lengthSq(horizontal) < 1.0E-6D) return;
            horizontal = horizontal.normalize();
            Vec3d impulse =fr.vanillainstincts.compat.Minecraft112Compat.add(horizontal.scale(MobilityRules.GAP_FORWARD_IMPULSE), 0.0D, MobilityRules.GAP_UP_IMPULSE, 0.0D);
    ...
    public static Optional<Vec3d> safeGapLanding(EntityLiving mob, EntityLivingBase target,
                                                WorldServer level) {
        if (mob == null || target == null || level == null) {
            return Optional.empty();
        }
        Vec3d direction = target.getPositionVector().subtract(mob.getPositionVector());
        direction = new Vec3d(direction.xCoord, 0.0D, direction.zCoord);
        ...
        Vec3d requested = mob.getPositionVector().add(direction.scale(
    ...
    public static Optional<Vec3d> nearestShore(EntityLiving mob, WorldServer level) {
    ...
    private static boolean adjacentToWater(WorldServer level, BlockPos feet) {
        ...
        return level.getBlockState(feet.down()).getMaterial() == Material.WATER;
    }
}
