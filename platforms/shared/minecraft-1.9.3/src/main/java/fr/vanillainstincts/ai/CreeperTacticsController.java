...
public final class CreeperTacticsController {
    ...
    private static void advanceWhileSwelling(EntityCreeper creeper,
        ...
        Vec3d direction = fr.vanillainstincts.compat.Minecraft112Compat.multiply(target.getPositionVector().subtract(creeper.getPositionVector()), 1.0D, 0.0D, 1.0D);
    ...
    public static Vec3d fuseAdvanceVelocity(Vec3d current, Vec3d direction) {
        ...
        return new Vec3d(horizontal.xCoord, safeCurrent.yCoord, horizontal.zCoord);
    }
    ...
    public static Optional<Vec3d> findBestAmbushPosition(EntityCreeper creeper,
                                                        EntityPlayer player,
                                                        WorldServer level) {
        Vec3d playerLook = horizontal(player.getLookVec());
        if (playerLook == null) {
            return Optional.empty();
        }
        Vec3d left = new Vec3d(-playerLook.zCoord, 0.0D, playerLook.xCoord);
        ...
        for (int radius = 3; radius <= 6; radius++) {
            for (Vec3d direction : directions) {
                Vec3d raw = player.getPositionVector().add(direction.scale(radius));
    ...
    public static Optional<Vec3d> findBlindSideOrbitPosition(
        ...
        Vec3d lateral = new Vec3d(-look.zCoord, 0.0D, look.xCoord)
    ...
    public static boolean isPlayerLookingAt(EntityPlayer player, EntityCreeper creeper) {
        ...
        Vec3d toCreeper = creeper.getPositionEyes(1.0F)
    ...
    private static double scoreCandidate(EntityCreeper creeper, EntityPlayer player,
                                         WorldServer level, Vec3d candidate,
                                         Vec3d playerLook) {
        Vec3d fromPlayer = candidate.subtract(player.getPositionVector());
    ...
    private static Vec3d stealthAdvanceDestination(EntityPlayer player,
        ...
        Vec3d lateral = new Vec3d(-look.zCoord, 0.0D, look.xCoord)
                .scale((creeper.getEntityId() & 1) == 0 ? 1.2D : -1.2D);
    ...
    private static boolean tryContextualLeap(EntityCreeper creeper, EntityPlayer player,
        ...
        Vec3d horizontal = horizontal(player.getPositionVector()
    ...
    private static boolean hasClearRay(WorldServer level, Vec3d from, Vec3d to,
                                       EntityPlayer source) {
        RayTraceResult result = level.rayTraceBlocks(from, to, false, true, false);
    ...
    private static boolean isValidTarget(EntityCreeper creeper, EntityLivingBase target) {
        ...
        if (target instanceof EntityPlayer
                && (((EntityPlayer) (target)).isCreative() || ((EntityPlayer) (target)).isSpectator())) { EntityPlayer player = (EntityPlayer) (target); 
            return false;
        }
