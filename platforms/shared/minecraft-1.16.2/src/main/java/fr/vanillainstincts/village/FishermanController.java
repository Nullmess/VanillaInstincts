...
public final class FishermanController {
    ...
    public static boolean contribute(VillagerEntity fisher,
        ...
        Vector3d bank = bankPosition(level, water, fisher);
    ...
    private static void maintainCast(VillagerEntity fisher, ServerWorld level,
        ...
        if (water == null || gameTime >= castEnd) {
            hook.setDeltaMovement(Vector3d.ZERO);
            if (water != null) {
        ...
        Vector3d target = Vector3d.atCenterOf(water).add(0.0D, 0.32D, 0.0D);
        Vector3d delta = target.subtract(hook.position());
    ...
    private static void renderLine(VillagerEntity fisher, ServerWorld level,
        ...
        Vector3d start = fisher.position().add(0.0D, 1.35D, 0.0D);
    ...
    private static void cast(VillagerEntity fisher, ServerWorld level,
        ...
        Vector3d origin = fisher.position().add(0.0D, 1.35D, 0.0D);
        Vector3d target = Vector3d.atCenterOf(water).add(0.0D, 0.32D, 0.0D);
    ...
    private static Vector3d bankPosition(ServerWorld level, BlockPos water,
