...
public final class VillagerRoutineController {
    ...
    public static void contribute(VillagerEntity villager,
        ...
        if (crowd > VillageSocialRules.VILLAGER_COMFORTABLE_CROWD) {
            Vector3d direction = separationDirection(villager, level);
    ...
    public static Optional<Vector3d> adjacentDestination(VillagerEntity villager,
                                                      BlockPos objective) {
        ...
        for (int[] offset : offsets) {
            ...
            double score = villager.level instanceof ServerWorld
                    ...
                    : -safe.get().distanceToSqr(Vector3d.atCenterOf(objective));
            if (score > bestScore) {
                bestScore = score;
                best = safe.get();
            }
    ...
    private static Vector3d separationDirection(VillagerEntity villager,
                                            ServerWorld level) {
        ...
        for (VillagerEntity other : level.getEntitiesOfClass(VillagerEntity.class,
            ...
            Vector3d delta = villager.position().subtract(other.position());
