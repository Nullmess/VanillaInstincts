...
public final class WolfPackController {
    ...
    public static void contribute(WolfEntity wolf, MobDecisionPlan plan,
        ...
        if (injured != null && injured != wolf
            ...
            Vector3d guard = guardPosition(injured, target);
        ...
        Vector3d flank = flankPosition(target, wolf, pack.size() + 1);
    ...
    public static Vector3d flankPosition(LivingEntity target, WolfEntity wolf,
                                     int packSize) {
    ...
    private static void offerRetreat(WolfEntity wolf, Vector3d danger,
                                     MobDecisionPlan plan) {
        Vector3d away = horizontal(wolf.position().subtract(danger));
    ...
    private static Vector3d guardPosition(WolfEntity injured, LivingEntity target) {
        Vector3d toward = horizontal(target.position().subtract(injured.position()));
        if (toward == null) toward = new Vector3d(1.0D, 0.0D, 0.0D);
