...
public final class VillagerSafetyController {
    ...
    public static boolean contribute(VillagerEntity villager,
        ...
        Optional<Vector3d> destination = privateShelterDestination(villager,
    ...
    public static boolean prepareToFlee(VillagerEntity villager,
        ...
        if (villager.isSleeping()) {
            ...
            villager.setSprinting(false);
            Vector3d velocity = villager.getDeltaMovement();
    ...
    public static Optional<Vector3d> findFleeDestination(VillagerEntity villager,
                                                      ServerWorld level,
                                                      BlockPos danger) {
        Vector3d away = villager.position().subtract(Vector3d.atCenterOf(danger));
        if (away.lengthSqr() < 1.0E-6D) {
            away = new Vector3d(1.0D, 0.0D, 0.0D);
        }
        Vector3d normalized = away.normalize();
        BlockPos origin = villager.blockPosition();
        Vector3d best = null;
        double bestScore = Double.NEGATIVE_INFINITY;
        for (int[] offset : CANDIDATES) {
            Vector3d directional = new Vector3d(offset[0], 0.0D, offset[1]);
            ...
            Optional<Vector3d> safe = SafePositionFinder.resolveGroundDestination(
                    villager, Vector3d.atBottomCenterOf(requested));
            if (!safe.isPresent()) {
                continue;
            }
            double score = safe.get().distanceToSqr(Vector3d.atCenterOf(danger))
     ...
     */
    public static Optional<Vector3d> findBehindDefenderDestination(
        ...
        Vector3d threatPosition = threat != null && threat.isAlive()
                ? threat.position() : Vector3d.atCenterOf(rememberedDanger);
        Vector3d away = golem.position().subtract(threatPosition)
        ...
        Vector3d center = golem.position().add(away.scale(
    ...
    private static Optional<Vector3d> privateShelterDestination(
        ...
        return SafePositionFinder.resolveGroundDestination(villager,
                Vector3d.atBottomCenterOf(home));
    }
