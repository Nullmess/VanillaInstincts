...
public final class VanillaInstinctsScheduler {
    private static final Map<ServerWorld, TickBudget> BUDGETS =
            new WeakHashMap<>();
    ...
    public static boolean isScheduled(Entity entity, int intervalTicks) {
        ...
        if (entity.level instanceof ServerWorld) { ServerWorld level = (ServerWorld) (entity.level); 
            TickBudget budget;
        ...
        return Math.floorMod(entity.level.getGameTime() + entity.getId(),
                interval) == 0L;
    }

    public static boolean claim(ServerWorld level) {
        return claim(level, 1, SchedulerPriority.NORMAL);
    }

    public static boolean claim(ServerWorld level, int cost) {
        return claim(level, cost, SchedulerPriority.NORMAL);
    }

    public static boolean claim(ServerWorld level, Entity entity, int cost) {
    ...
    public static void recordWork(ServerWorld level, long startedAtNanos) {
    ...
    public static int usedCost(ServerWorld level) {
        synchronized (BUDGETS) {
            return current(level).usedCost;
        }
    }

    public static int rejectedCost(ServerWorld level) {
        synchronized (BUDGETS) {
            return current(level).rejectedCost;
        }
    }

    public static int remainingCost(ServerWorld level) {
        synchronized (BUDGETS) {
            TickBudget budget = current(level);
    ...
    public static LoadTier loadTier(ServerWorld level) {
        synchronized (BUDGETS) {
            return current(level).loadState.tier();
        }
    }

    public static SchedulerPriority priority(ServerWorld level,
                                             Entity entity) {
    ...
    public static int precisionLimit(ServerWorld level, Entity entity,
    ...
    public static int precisionLimit(ServerWorld level, int configuredLimit,
                                     int minimum) {
        return precisionLimit(level, null, configuredLimit, minimum);
    }

    public static int heavyTaskMultiplier(ServerWorld level) {
        synchronized (BUDGETS) {
            return current(level).loadState.tier().heavyTaskMultiplier();
        }
    }

    public static SchedulerTelemetry telemetry(ServerWorld level) {
        synchronized (BUDGETS) {
            TickBudget budget = current(level);
    ...
    private static TickBudget current(ServerWorld level) {
    ...
    private static SchedulerPriority priorityFor(ServerWorld level,
                                                 Entity entity,
                                                 TickBudget budget) {
        SchedulerPriority cached = budget.priorities.get(entity.getId());
        if (cached != null) return cached;
        boolean playerCombat = entity instanceof MobEntity
                && ((MobEntity) (entity)).getTarget() instanceof PlayerEntity;
        PlayerEntity nearestPlayer = level.getNearestPlayer(entity.getX(),
                entity.getY(), entity.getZ(),
                PerformanceRules.ACTIVE_PLAYER_DISTANCE,
                candidate -> candidate instanceof PlayerEntity
                        && ((PlayerEntity) (candidate)).isAlive() && !((PlayerEntity) (candidate)).isSpectator());
        double nearestDistanceSqr = nearestPlayer == null
                ? Double.POSITIVE_INFINITY
                : entity.distanceToSqr(nearestPlayer);
        SchedulerPriority priority = AdaptiveLoadPolicy.priority(
                ...
                PerformanceRules.ACTIVE_PLAYER_DISTANCE);
        budget.priorities.put(entity.getId(), priority);
        return priority;
    }
