...
public final class EndermanTacticsController {
    ...
    private static void contributeCarrying(EndermanEntity enderman,
        ...
        if (state.role() == EndermanCargoRole.ALLY_RESCUE
            ...
            Vector3d away = horizontal(enderman.position()
                    .subtract(objective.position()));
            if (away == null) {
                away = new Vector3d(1.0D, 0.0D, 0.0D);
            }
            Vector3d desired = enderman.position().add(away.scale(
        ...
        if (state.role() == EndermanCargoRole.PLAYER_RELOCATION
            ...
            Optional<Vector3d> safe = playerExtractionDestination(enderman, level);
        ...
        if (state.role() == EndermanCargoRole.SPIDER_PLATFORM
            ...
            if (spiderTarget != null && spiderTarget.isAlive()) {
                ...
                Vector3d destination = spiderTarget.position()
        ...
        if (state.role() == EndermanCargoRole.CREEPER_DELIVERY
            ...
            if (objective != null) {
                ...
                if (shouldReleaseCreeper(
                    ...
                    Vector3d destination = presentationDestination(objective,
                            enderman);
                    offerTacticalTeleport(enderman, cargo, state, plan, level,
        ...
        if (state.role() == EndermanCargoRole.ARCHER_PLATFORM
            ...
            if (objective != null) {
                ...
                Vector3d destination = archerDestination(objective, enderman,
                        tooClose);
                offerTacticalTeleport(enderman, cargo, state, plan, level,
        ...
        if (state.role() == EndermanCargoRole.CREATURE_GIFT
            ...
            if (objective != null) {
                Vector3d destination = presentationDestination(objective, enderman);
                offerTacticalTeleport(enderman, cargo, state, plan, level,
        ...
        if (objective != null) {
            Vector3d delivery = presentationDestination(objective, enderman);
            offerTacticalTeleport(enderman, cargo, state, plan, level,
    ...
    private static void offerApproachCargoTeleport(EndermanEntity enderman,
        ...
        Vector3d away = horizontal(enderman.position().subtract(cargo.position()));
        if (away == null) {
            away = new Vector3d(1.0D, 0.0D, 0.0D);
        }
        ...
        plan.offerSpecial(VanillaInstinctsState.TELEPORT_CARGO,
                ...
                () -> {
                    Vector3d before = enderman.position();
    ...
    private static void offerIdleCarryTeleport(EndermanEntity enderman, Entity cargo,
        ...
        Vector3d desired = enderman.position().add(Math.cos(angle) * distance,
    ...
    public static boolean hasLegitimateAggro(EndermanEntity enderman,
        ...
        return enderman.isAngryAt(target)
                || activeVanillaTarget
                || (persistent != null
    ...
    public static void positionCargoPassenger(EndermanEntity enderman,
        ...
        Vector3d position = cargoCarryPosition(enderman.position(),
                enderman.yRot, cargo.getBbWidth(), cargo.getBbHeight());
        cargo.setPos(position.x, position.y, position.z);
        cargo.fallDistance = 0.0F;
    }

    public static Vector3d cargoCarryPosition(Vector3d endermanPosition, float yaw,
                                          double cargoWidth,
                                          double cargoHeight) {
        Vector3d origin = endermanPosition == null ? Vector3d.ZERO : endermanPosition;
    ...
    private static void offerCreeperReleaseAndRetreat(
        ...
        Vector3d release = resolveCreeperReleaseDestination(
                creeper, enderman, objective).orElse(enderman.position());
        Vector3d retreat = resolveCreeperRetreatDestination(
    ...
    private static void releaseCreeperAndRetreatNow(
            ...
            ServerWorld level, long gameTime,
            @Nullable LivingEntity objective) {
        Vector3d release = resolveCreeperReleaseDestination(creeper, enderman,
                objective).orElse(enderman.position());
        Vector3d retreat = resolveCreeperRetreatDestination(enderman, objective,
                release).orElse(enderman.position());
        releaseCreeperAndRetreat(enderman, creeper, objective, state, level,
                gameTime, release, retreat);
    }

    private static Optional<Vector3d> resolveCreeperReleaseDestination(
            CreeperEntity creeper, EndermanEntity enderman,
            @Nullable LivingEntity objective) {
        Vector3d requested = objective == null
                ? enderman.position().add(enderman.getLookAngle().scale(1.6D))
                : creeperDeliveryPosition(objective, enderman);
        Optional<Vector3d> primary = SafePositionFinder.resolveGroundDestination(
                creeper, requested);
        ...
        for (double radius : radii) {
            for (int index = 0; index < 8; index++) {
                ...
                Vector3d candidate = objective.position().add(
                        Math.cos(angle) * radius, 0.0D,
                        Math.sin(angle) * radius);
                Optional<Vector3d> safe = SafePositionFinder
    ...
    private static Optional<Vector3d> resolveCreeperRetreatDestination(
            EndermanEntity enderman, @Nullable LivingEntity objective,
            Vector3d releasePosition) {
        Vector3d primaryRequest = creeperRetreatPosition(enderman, objective,
                releasePosition);
        Optional<Vector3d> primary = SafePositionFinder.resolveGroundDestination(
        ...
        for (double distance : distances) {
            for (double lateral : laterals) {
                Vector3d candidate = releasePosition.add(away.scale(distance))
    ...
    private static void releaseCreeperAndRetreat(
            ...
            Vector3d release, Vector3d retreat) {
    ...
    private static void releaseHostileCargoAndRetreat(
        ...
        Vector3d requested = enderman.position().add(away.scale(9.0D));
    ...
    private static Vector3d creeperDeliveryPosition(LivingEntity objective,
                                                EndermanEntity enderman) {
        Vector3d away = horizontal(enderman.position()
                .subtract(objective.position()));
        if (away == null) {
        ...
        Vector3d lateral = new Vector3d(-away.z, 0.0D, away.x)
    ...
    private static Vector3d creeperRetreatPosition(
            EndermanEntity enderman, @Nullable LivingEntity objective,
            Vector3d releasePosition) {
        Vector3d away = objective == null ? horizontal(enderman.position()
    ...
    private static Vector3d archerDestination(LivingEntity objective,
                                          EndermanEntity enderman,
                                          boolean retreat) {
        Vector3d away = horizontal(enderman.position()
                .subtract(objective.position()));
    ...
    private static Vector3d presentationDestination(LivingEntity objective,
                                                EndermanEntity enderman) {
        Vector3d look = horizontal(objective.getLookAngle());
        if (look == null) {
        ...
        Vector3d lateral = new Vector3d(-look.z, 0.0D, look.x)
    ...
    private static Optional<Vector3d> playerExtractionDestination(
        ...
        for (int i = 0; i < 12; i++) {
            ...
            Vector3d requested = enderman.position().add(
                    Math.cos(angle) * radius, 0.0D,
                    Math.sin(angle) * radius);
            Optional<Vector3d> safe = SafePositionFinder.resolveGroundDestination(
                    enderman, requested);
    ...
    private static void emitTeleportParticles(ServerWorld level, Vector3d pos) {
