...
public final class CreeperDiversionController {
    ...
    public static void maintain(CreeperEntity creeper, long gameTime) {
        if (creeper == null || creeper.isIgnited()) {
            return;
        }
        CompoundNBT root = creeper.getPersistentData();
    ...
    public static boolean contribute(CreeperEntity creeper, PlayerEntity player,
    ...
    public static boolean shouldFeint(CreeperEntity creeper, PlayerEntity player,
                                      long gameTime) {
        if (creeper == null || player == null || creeper.isIgnited()
                || !creeper.canSee(player)
    ...
    private static void beginFeint(CreeperEntity creeper, long gameTime) {
    ...
    private static boolean feintReady(CreeperEntity creeper, long gameTime) {
        CompoundNBT root = creeper.getPersistentData();
