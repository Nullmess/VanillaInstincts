...
public final class CreeperDiversionController {
    ...
    public static void maintain(Creeper creeper, long gameTime) {
        ...
        long until = fr.vanillainstincts.persistence.NbtCompat.getLong(tag, "active_until");
        if (until > gameTime) {
    ...
    private static boolean feintReady(Creeper creeper, long gameTime) {
        CompoundTag root = creeper.getPersistentData();
        return !fr.vanillainstincts.persistence.NbtCompat.contains(root, ROOT_KEY)
                || gameTime >= fr.vanillainstincts.persistence.NbtCompat.getLong(
                        fr.vanillainstincts.persistence.NbtCompat.getCompound(root, ROOT_KEY), "ready_at");
    }
}
