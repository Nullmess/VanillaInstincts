...
public final class SmithRepairController {
    ...
    public static RepairResult maybeRepair(EntityVillager smith, ItemStack stack,
                                           long offerSalt) {
        if (smith == null || stack == null || fr.vanillainstincts.compat.Minecraft110ItemStackCompat.isEmpty(stack)
                || !ProfessionStockController.isSmith(
                LegacyVillagerProfession.of(smith))
                || !stack.isItemStackDamageable() || fr.vanillainstincts.compat.Minecraft17Compat.itemDamage(stack) <= 0
                || !shouldRepair(smith.getUniqueID(), stack, offerSalt)) {
            return new RepairResult(stack, false, 0, 0);
        }
        Item material = repairMaterial(stack);
        int slot = findMaterialSlot(fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(smith), material);
        if (slot < 0) return new RepairResult(stack, false, 0, 0);

        int before = fr.vanillainstincts.compat.Minecraft17Compat.itemDamage(stack);
        int repaired = repairAmount(smith.getUniqueID(), stack, offerSalt);
        if (repaired <= 0) return new RepairResult(stack, false, 0, 0);
        fr.vanillainstincts.compat.Minecraft110ItemStackCompat.shrink(fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(smith).getStackInSlot(slot), 1);
        fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(smith).markDirty();
        fr.vanillainstincts.compat.Minecraft17Compat.setItemDamage(stack, Math.max(0, before - repaired));
        int workmanship = Math.min(8, 1 + repaired
                / Math.max(1, fr.vanillainstincts.compat.Minecraft17Compat.maxDamage(stack) / 10));
    ...
    public static int repairAmount(UUID smithId, ItemStack stack,
                                   long offerSalt) {
        if (stack == null || fr.vanillainstincts.compat.Minecraft110ItemStackCompat.isEmpty(stack) || !stack.isItemStackDamageable()) {
            return 0;
        }
        int damage = fr.vanillainstincts.compat.Minecraft17Compat.itemDamage(stack);
        if (damage <= 0) return 0;
        int seed = smithId == null ? 0 : smithId.hashCode();
        seed = seed * 31 + fr.vanillainstincts.compat.Minecraft17Compat.maxDamage(stack);
        seed = seed * 31 + Long.hashCode(offerSalt);
    ...
    private static int findMaterialSlot(InventoryBasic inventory,
        ...
        for (int slot = 0; slot < inventory.getSizeInventory(); slot++) {
            if (fr.vanillainstincts.compat.Minecraft116Compat.stackIs(inventory.getStackInSlot(slot), material)
                    && !fr.vanillainstincts.compat.Minecraft110ItemStackCompat.isEmpty(inventory.getStackInSlot(slot))) {
                return slot;
            }
        }
        return -1;
