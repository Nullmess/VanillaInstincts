...
public final class MobPerceptionMemory {
    ...
    public static boolean canSee(EntityLiving mob, EntityLivingBase target) {
        ...
        if (!(mob.worldObj instanceof WorldServer)) {
            if (fr.vanillainstincts.compat.Minecraft112Compat.hasEffect(mob, net.minecraft.potion.Potion.blindness)
        ...
        double range = fr.vanillainstincts.compat.Minecraft112Compat.hasEffect(mob, net.minecraft.potion.Potion.blindness)
    ...
    public static void rememberSeen(EntityLiving mob, BlockPos position, UUID target,
        ...
        if (target != null) {
            fr.vanillainstincts.compat.Minecraft18NbtCompat.setUniqueId(mob.getEntityData(), SEEN_TARGET, target);
        } else {
    ...
    public static Optional<Vec3> bestKnownPosition(EntityLiving mob, long gameTime) {
    ...
    public static Optional<UUID> lastSeenTarget(EntityLiving mob, long gameTime) {
        if (mob == null || !hasFreshSeen(mob, gameTime)
                || !fr.vanillainstincts.compat.Minecraft18NbtCompat.hasUniqueId(mob.getEntityData(), SEEN_TARGET)) {
            return Optional.empty();
        }
        return Optional.of(fr.vanillainstincts.compat.Minecraft18NbtCompat.getUniqueId(mob.getEntityData(), SEEN_TARGET));
    }
    ...
    private static void broadcastNoiseInternal(WorldServer level,
        ...
        double scanRadius = PerceptionProfileManager.maximumNoiseScanRadius(
                radius);
        AxisAlignedBB area = fr.vanillainstincts.compat.Minecraft112Compat.blockBox(position).expand(scanRadius, scanRadius, scanRadius);
        Vec3 center = Minecraft115VectorCompat.atCenterOf(position);
        List<EntityLiving> observers = new ArrayList<>();
        if (audience == StimulusAudience.ALL_MOBS) {
            observers.addAll(fr.vanillainstincts.compat.Minecraft112Compat.entitiesWithinAABB(level, EntityLiving.class, area,
                    EntityLivingBase::isEntityAlive));
        } else {
            observers.addAll(fr.vanillainstincts.compat.Minecraft112Compat.entitiesWithinAABB(level, EntityMob.class, area,
                    EntityLivingBase::isEntityAlive));
        }
    ...
    public static <T extends EntityLiving> void shareWithNearbySameType(
        ...
        for (EntityLiving ally : fr.vanillainstincts.compat.Minecraft112Compat.entitiesWithinAABB(level, EntityLiving.class,
                fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(source).expand(radius, radius, radius), other ->
