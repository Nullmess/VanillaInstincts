...
public final class ClericNetherExpeditionController {
    ...
    public static boolean contribute(EntityVillager cleric,
        ...
        if (phase == IDLE) {
            if (gameTime < cleric.getEntityData().getLong(READY_AT)
                    ...
                    || !needsNetherStock(fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(cleric))) {
                return false;
            }
    ...
    private static void maintainVirtual(EntityVillager cleric, WorldServer level,
        ...
        fr.vanillainstincts.compat.Minecraft112Compat.setInvulnerable(cleric, true);
        fr.vanillainstincts.compat.Minecraft17Compat.setSilent(cleric, true);
        if (gameTime < cleric.getEntityData().getLong(RETURN_AT)) return;
        grantVirtualLoot(cleric, gameTime);
        BlockPos portal = portalPos(cleric);
        if (portal != null && fr.vanillainstincts.compat.Minecraft17Compat.isBlockLoaded(level, portal)) {
    ...
    private static void maintainLive(EntityVillager cleric, WorldServer level,
                                     long gameTime) {
        fr.vanillainstincts.compat.Minecraft112Compat.setInvulnerable(cleric, true);
        fr.vanillainstincts.compat.Minecraft112Compat.addEffect(cleric, fr.vanillainstincts.compat.Minecraft17Compat.potionEffect(net.minecraft.potion.Potion.fireResistance.getId(),
                240, 0, true, false));
        fr.vanillainstincts.compat.Minecraft112Compat.addEffect(cleric, fr.vanillainstincts.compat.Minecraft17Compat.potionEffect(net.minecraft.potion.Potion.damageBoost.getId(),
                240, 1, true, false));
        fr.vanillainstincts.compat.Minecraft112Compat.addEffect(cleric, fr.vanillainstincts.compat.Minecraft17Compat.potionEffect(net.minecraft.potion.Potion.regeneration.getId(),
                120, 1, true, false));
        ...
        if (gameTime >= cleric.getEntityData().getLong(ACTION_AT)) {
            ...
            EntityBlaze blaze = fr.vanillainstincts.compat.Minecraft112Compat.entitiesWithinAABB(level, EntityBlaze.class,
                    fr.vanillainstincts.compat.Minecraft112Compat.expandBox(fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(cleric), 
    ...
    private static void collectNetherDrops(EntityVillager cleric,
                                           WorldServer level) {
        for (EntityItem entity : fr.vanillainstincts.compat.Minecraft112Compat.entitiesWithinAABB(level, EntityItem.class,
                fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(cleric).expand(3.0D, 3.0D, 3.0D), item -> item.isEntityAlive()
    ...
    private static void grantVirtualLoot(EntityVillager cleric, long gameTime) {
        ...
        int gold = Math.min(2, count(fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(cleric), Items.gold_ingot));
        for (int i = 0; i < gold; i++) {
            if (consume(fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(cleric), Items.gold_ingot, 1)) {
    ...
    private static boolean hasNetherObserver(WorldServer overworld,
                                             BlockPos portal) {
        WorldServer nether = Minecraft115WorldCompat.world(fr.vanillainstincts.compat.Minecraft17Compat.server(overworld), LegacyDimensionType.NETHER);
    ...
    private static BlockPos tryBuildPortal(EntityVillager cleric,
                                           WorldServer level) {
        if (count(fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(cleric), net.minecraft.item.Item.getItemFromBlock(net.minecraft.init.Blocks.obsidian))
                < requiredPortalObsidian()
                || findSlot(fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(cleric), Items.flint_and_steel) < 0) {
            return null;
        }
        ...
        for (int x = 0; x < 4; x++) {
            for (int y = 0; y < 5; y++) {
                ...
                LegacyBlockState state = frame
                        ? fr.vanillainstincts.compat.Minecraft17Compat.defaultState(Blocks.obsidian)
                        : fr.vanillainstincts.compat.Minecraft17Compat.portalState(EnumFacing.Axis.X);
        ...
        if (!consume(fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(cleric), net.minecraft.item.Item.getItemFromBlock(net.minecraft.init.Blocks.obsidian),
                requiredPortalObsidian())) {
    ...
    public static boolean portalSiteClear(WorldServer level, BlockPos base) {
        ...
        for (int x = 0; x < 4; x++) {
            ...
            if (!fr.vanillainstincts.compat.Minecraft17Compat.isBlockLoaded(level, ground)
                    || !fr.vanillainstincts.compat.Minecraft112Compat.isSolidRender(fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, ground))) return false;
        }
        for (int x = 0; x < 4; x++) {
            for (int y = 0; y < 5; y++) {
                BlockPos pos = fr.vanillainstincts.compat.Minecraft112Compat.offset(base, x, y, 0);
                if (!fr.vanillainstincts.compat.Minecraft17Compat.isBlockLoaded(level, pos)) return false;
    ...
    private static void placeBridgeToward(EntityVillager cleric, WorldServer level,
        ...
        Item material = count(fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(cleric), net.minecraft.item.Item.getItemFromBlock(net.minecraft.init.Blocks.cobblestone)) > 0
                ? net.minecraft.item.Item.getItemFromBlock(net.minecraft.init.Blocks.cobblestone)
                : count(fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(cleric), net.minecraft.item.Item.getItemFromBlock(net.minecraft.init.Blocks.netherrack)) > 0
        ...
        if (!fr.vanillainstincts.compat.Minecraft17Compat.isBlockLoaded(level, support) || !fr.vanillainstincts.compat.Minecraft112Compat.isAir(fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, support))) {
            return;
        }
        LegacyBlockState state = material == net.minecraft.item.Item.getItemFromBlock(net.minecraft.init.Blocks.cobblestone)
                ? fr.vanillainstincts.compat.Minecraft17Compat.defaultState(Blocks.cobblestone)
                : fr.vanillainstincts.compat.Minecraft17Compat.defaultState(Blocks.netherrack);
        if (WorldPermissionService.setBlock(level, cleric, support, state,
                3, WorldActionType.PLACE_BLOCK)) {
            consume(fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(cleric), material, 1);
            cleric.swingItem();
        }
    }
            ...
            for (int dx = -ring; dx <= ring; dx += 4) {
                for (int dz = -ring; dz <= ring; dz += 4) {
                    ...
                    for (int dy = -vertical; dy <= vertical; dy++) {
                        ...
                        if (fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, pos).getBlock().equals(Blocks.mob_spawner)) {
                            return pos;
                        }
                    }
                }
    ...
    private static boolean forceReturnHome(EntityVillager cleric, WorldServer level,
                                           long gameTime) {
        WorldServer overworld = Minecraft115WorldCompat.world(fr.vanillainstincts.compat.Minecraft17Compat.server(level), LegacyDimensionType.OVERWORLD);
        ...
        if (cleric.worldObj != overworld) {
            ...
            Entity changed = fr.vanillainstincts.compat.Minecraft17Compat.getEntityFromUuid(overworld, clericId);
    ...
    private static void discardPendingPayment(EntityVillager cleric,
        ...
        Entity payment = fr.vanillainstincts.compat.Minecraft17Compat.getEntityFromUuid(level, 
    ...
    private static void restore(EntityVillager cleric) {
        ...
        fr.vanillainstincts.compat.Minecraft17Compat.setSilent(cleric, false);
    }
    ...
    private static void damageFlintAndSteel(EntityVillager cleric) {
        int slot = findSlot(fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(cleric), Items.flint_and_steel);
        if (slot < 0) return;
        ItemStack tool = fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(cleric).getStackInSlot(slot);
        fr.vanillainstincts.compat.Minecraft17Compat.setItemDamage(tool, fr.vanillainstincts.compat.Minecraft17Compat.itemDamage(tool) + 1);
        if (fr.vanillainstincts.compat.Minecraft17Compat.itemDamage(tool) >= fr.vanillainstincts.compat.Minecraft17Compat.maxDamage(tool)) {
    ...
    private static void insert(EntityVillager cleric, ItemStack stack) {
        ...
        ProfessionStockController.insert(fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(cleric), stack);
