...
public final class VillagerProductionController {
    ...
    public static void maintain(Villager villager, ServerLevel level,
        ...
        if (!fr.vanillainstincts.persistence.NbtCompat.getBoolean(villager.getPersistentData(), COMPLETED)
                && gameTime >= fr.vanillainstincts.persistence.NbtCompat.getLong(villager.getPersistentData(), RESULT_AT)) {
    ...
    private static void start(Villager villager, ServerLevel level,
                              ...
                              long gameTime) {
        int levelValue = villager.getVillagerData().level();
        ...
        long sequence = fr.vanillainstincts.persistence.NbtCompat.getLong(villager.getPersistentData(), SEQUENCE) + 1L;
    ...
    private static void complete(Villager villager, ServerLevel level,
        ...
        long sequence = fr.vanillainstincts.persistence.NbtCompat.getLong(villager.getPersistentData(), SEQUENCE);
    ...
    private static ProfessionRecipe storedRecipe(Villager villager) {
        return ProfessionRecipeCatalog.byId(fr.vanillainstincts.persistence.NbtCompat.getString(villager.getPersistentData(), RECIPE_ID));
    }

    private static void clearExpired(Villager villager, long gameTime) {
        long activeUntil = fr.vanillainstincts.persistence.NbtCompat.getLong(villager.getPersistentData(), ACTIVE_UNTIL);
        if (activeUntil <= 0L || gameTime < activeUntil) return;
        boolean completed = fr.vanillainstincts.persistence.NbtCompat.getBoolean(villager.getPersistentData(), COMPLETED);
    ...
    private static boolean active(Villager villager, long gameTime) {
        return villager != null && gameTime < fr.vanillainstincts.persistence.NbtCompat.getLong(villager.getPersistentData(), ACTIVE_UNTIL);
    }
    ...
    private static BlockPos readPos(Villager villager, String key) {
        return villager.getPersistentData().contains(key)
                ? BlockPos.of(fr.vanillainstincts.persistence.NbtCompat.getLong(villager.getPersistentData(), key))
                : null;
    }
    ...
    public static void recordSocialSale(Villager seller, ServerLevel level,
        ...
        long currentBoost = fr.vanillainstincts.persistence.NbtCompat.getLong(seller.getPersistentData(), SOCIAL_BOOST_UNTIL);
        seller.getPersistentData().putLong(SOCIAL_BOOST_UNTIL,
                Math.max(currentBoost, gameTime + SOCIAL_BOOST_TICKS));
        int sales = Math.max(0, fr.vanillainstincts.persistence.NbtCompat.getInt(seller.getPersistentData(), SOCIAL_SALES));
        seller.getPersistentData().putInt(SOCIAL_SALES,
                Math.min(4_096, sales + 1));
        long readyAt = fr.vanillainstincts.persistence.NbtCompat.getLong(seller.getPersistentData(), READY_AT);
    ...
    public static boolean socialBoostActive(Villager villager,
                                            long gameTime) {
        return villager != null && gameTime < fr.vanillainstincts.persistence.NbtCompat.getLong(villager.getPersistentData(), SOCIAL_BOOST_UNTIL);
    }

    public static int socialSaleCount(Villager villager) {
        return villager == null ? 0 : Math.max(0,
                fr.vanillainstincts.persistence.NbtCompat.getInt(villager.getPersistentData(), SOCIAL_SALES));
    }
