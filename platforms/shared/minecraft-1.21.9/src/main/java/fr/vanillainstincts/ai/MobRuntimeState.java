...
public final class MobRuntimeState {
    ...
    public static MobRuntimeState load(Mob mob) {
        ...
        CompoundTag root = fr.vanillainstincts.persistence.NbtCompat.getCompound(mob.getPersistentData(), ROOT_KEY);
        state.currentState = readEnum(VanillaInstinctsState.class,
                fr.vanillainstincts.persistence.NbtCompat.getString(root, "state"), VanillaInstinctsState.IDLE);
        state.owner = readEnum(ActionOwner.class,
                fr.vanillainstincts.persistence.NbtCompat.getString(root, "owner"), ActionOwner.NONE);
        state.currentIntent = readEnum(MobIntent.class,
                fr.vanillainstincts.persistence.NbtCompat.getString(root, "intent"), MobIntent.NONE);
        state.priority = Math.max(0, fr.vanillainstincts.persistence.NbtCompat.getInt(root, "priority"));
        state.lockedUntil = Math.max(0L, fr.vanillainstincts.persistence.NbtCompat.getLong(root, "locked_until"));
        state.intentUntil = Math.max(0L, fr.vanillainstincts.persistence.NbtCompat.getLong(root, "intent_until"));
