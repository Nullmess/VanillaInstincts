...
import fr.vanillainstincts.compat.Minecraft1192Compat;
...
public final class ClericBrewingController {
    ...
    private static void loadNextVisibleStep(Villager cleric,
        ...
        if (step == LOAD_FUEL) {
            ...
            changed = placeServiceStack(stand, step - 1,
                    Minecraft1192Compat.copyWithCount(batch.input(), 1));
            ...
            if (missingBottle >= 0) {
                ...
                changed = placeServiceStack(stand, 3,
                        Minecraft1192Compat.copyWithCount(batch.ingredient(), 1));
            }
        } else {
            return;
        }
    ...
    private static ItemStack displayedStackForStep(int step,
        ...
        if (step >= LOAD_FIRST_BOTTLE && step <= LOAD_LAST_BOTTLE) {
            return Minecraft1192Compat.copyWithCount(batch.input(), 1);
        }
        if (step == LOAD_INGREDIENT) {
            return Minecraft1192Compat.copyWithCount(batch.ingredient(), 1);
        }
    ...
    private static void inspectAndCollect(Villager cleric,
        ...
        if (recipe != null && stand.getItem(3).isEmpty()
            ...
            stand.setItem(3, Minecraft1192Compat.copyWithCount(recipe.ingredient(), 1));
            stand.setChanged();
            cleric.setItemInHand(InteractionHand.MAIN_HAND,
                    Minecraft1192Compat.copyWithCount(recipe.ingredient(), 1));
            cleric.swing(InteractionHand.MAIN_HAND);
            cleric.getPersistentData().putLong(ACTIVE_HAND_CLEAR_AT,
                    gameTime + Math.max(4,
                    configuredTicks(ProfessionRules.CLERIC_BREW_LOAD_STEP_TICKS) - 2));
        ...
        ItemStack completedBatch = Minecraft1192Compat.copyWithCount(expectedOutput, available);
        ...
        if (socialBottle) {
            ItemStack socialStock = Minecraft1192Compat.copyWithCount(expectedOutput, 1);
        ...
        if (tradeUses > 0 && RecoveredTradeController.addProducedOffer(
                cleric, Minecraft1192Compat.copyWithCount(expectedOutput, 1), tradeUses)) {
            ...
            for (int index = represented; index < available; index++) {
                ItemStack stock = Minecraft1192Compat.copyWithCount(expectedOutput, 1);
                int inserted = ProfessionStockController.insert(
    ...
    private static List<BatchPlan> buildServiceRecipes(ServerLevel level) {
        Registry<Potion> potions = level.registryAccess()
                .registryOrThrow(Registry.POTION_REGISTRY);
        List<Item> ingredients = Registry.ITEM.stream().toList();
        ...
        for (Potion potion : potions) {
            for (Item container : containers) {
                ...
                for (Item ingredientItem : ingredients) {
                    ...
                    BatchPlan candidate = new BatchPlan(Minecraft1192Compat.copyWithCount(input, 1),
                            Minecraft1192Compat.copyWithCount(ingredient, 1),
                            Minecraft1192Compat.copyWithCount(output, 1));
    ...
    private static boolean moveOne(SimpleContainer inventory,
        ...
        ItemStack moved = Minecraft1192Compat.copyWithCount(source, 1);
