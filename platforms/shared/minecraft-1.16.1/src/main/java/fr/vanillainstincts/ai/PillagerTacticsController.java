...
public final class PillagerTacticsController {
    ...
    public static void contribute(PillagerEntity pillager, MobDecisionPlan plan,
        ...
        if (pillager.isUsingItem()
            ...
            Vector3d cover = coverDestination(pillager, target);
        ...
        if (!squad.isEmpty()
            ...
            Vector3d flank = flankDestination(pillager, target,
                    squad.size() + 1);
    ...
    public static Vector3d coverDestination(PillagerEntity pillager,
                                        LivingEntity target) {
        Vector3d away = pillager.position().subtract(target.position())
    ...
    public static Vector3d flankDestination(PillagerEntity pillager,
                                        LivingEntity target,
                                        int squadSize) {
        Vector3d radial = pillager.position().subtract(target.position())
                .multiply(1.0D, 0.0D, 1.0D);
        if (radial.lengthSqr() < 1.0E-8D) {
            radial = new Vector3d(1.0D, 0.0D, 0.0D);
        } else {
