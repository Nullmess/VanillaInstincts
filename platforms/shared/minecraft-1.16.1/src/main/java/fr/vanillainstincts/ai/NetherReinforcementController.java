...
public final class NetherReinforcementController {
        ...
        for (ServerWorld level : sourceLevel.getServer().getAllLevels()) {
            for (Entity entity : level.getAllEntities()) {
                ...
                if (mob instanceof ZombifiedPiglinEntity
                        && aggressorId.equals(
                        ((ZombifiedPiglinEntity) (mob)).getPersistentAngerTarget())) { ZombifiedPiglinEntity piglin = (ZombifiedPiglinEntity) (mob); 
    ...
    public static void onLevelUnload(ServerWorld level) {
        if (level != null && World.OVERWORLD.equals(level.dimension())) {
            clearRuntime();
        }
    }
    ...
    private static boolean maintainMessenger(MobEntity messenger,
                                               ...
                                               ServerWorld level,
                                               long gameTime) {
        boolean inNether = World.NETHER.equals(level.dimension());
        boolean inOverworld = World.OVERWORLD.equals(level.dimension());
    ...
    private static boolean maintainReinforcement(MobEntity reinforcement,
        ...
        if (reinforcement instanceof ZombifiedPiglinEntity) { ZombifiedPiglinEntity piglin = (ZombifiedPiglinEntity) (reinforcement); 
    ...
    private static int createObservedFallbackPack(
        ...
        for (int index = 0; index < count; index++) {
            ...
            Vector3d emergence = findEmergencePosition(level, netherPortal,
                    index + 1);
    ...
    private static boolean moveToStoredPortal(MobEntity mob,
        ...
        Vector3d destination = Vector3d.atCenterOf(portal);
    ...
    private static void materializeVirtualExpedition(
        ...
        for (int index = 0; index < count; index++) {
            ...
            Vector3d emergence = findEmergencePosition(overworld, portal,
                    index + 1);
            reinforcement.moveTo(emergence.x, emergence.y, emergence.z,
    ...
    private static void spawnReturningMessenger(
        ...
        Vector3d emergence = findEmergencePosition(overworld, portal, 0);
    ...
    private static Vector3d findEmergencePosition(
            ...
            for (int offset = 0; offset < directions.length; offset++) {
                ...
                for (int dy = -1; dy <= 2; dy++) {
                    ...
                    if (level.getBlockState(feet).isAir()
                        ...
                        return Vector3d.atBottomCenterOf(feet);
                    }
                }
            }
        }
        return Vector3d.atCenterOf(portal.above());
    }
    ...
    private static void finishMission(MobEntity mob,
        ...
        if (mob instanceof ZombifiedPiglinEntity) { ZombifiedPiglinEntity piglin = (ZombifiedPiglinEntity) (mob); 
            piglin.setPersistentAngerTarget(null);
            piglin.setRemainingPersistentAngerTime(0);
        }
    ...
    private static void calmZoglinsTargetingMessenger(
        ...
        for (ZoglinEntity zoglin : level.getEntitiesOfClass(ZoglinEntity.class, area,
    ...
    private static List<MobEntity> recruitsAround(ServerWorld level, MobEntity center,
        ...
        if (kind == NetherReinforcementKind.ZOMBIFIED_PIGLIN) {
            result.addAll(level.getEntitiesOfClass(ZombifiedPiglinEntity.class,
                    area, candidate -> candidate.isAlive()));
        } else {
            result.addAll(level.getEntitiesOfClass(ZoglinEntity.class,
                    area, candidate -> candidate.isAlive()));
        }
        return result;
    }
