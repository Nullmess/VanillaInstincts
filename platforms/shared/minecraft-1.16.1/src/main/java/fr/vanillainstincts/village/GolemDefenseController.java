...
public final class GolemDefenseController {
    ...
    public static void contribute(IronGolemEntity golem,
        ...
        if (!state.active(gameTime)) {
            ...
            if (anchor != null && golem.getTarget() == null
                ...
                SafePositionFinder.resolveGroundDestination(golem,
                                Vector3d.atBottomCenterOf(anchor))
        ...
        LivingEntity protectedVillager = resolveProtectedVillager(state,
                level);
        Vector3d protectedPoint = protectedVillager != null
                ? protectedVillager.position()
                : state.villageAnchor() == null ? golem.position()
                : Vector3d.atCenterOf(state.villageAnchor());

        Vector3d desired = fr.vanillainstincts.compat.LegacyJava8.eval(() -> { switch ((state.role())) { case INTERCEPTOR:  return interpositionPoint(target.position(),
                    protectedPoint,
    ...
    public static Vector3d interpositionPoint(Vector3d threat, Vector3d protectedPoint,
                                          double distanceFromThreat) {
        if (threat == null || protectedPoint == null) {
            return threat == null ? Vector3d.ZERO : threat;
        }
        Vector3d towardProtected = protectedPoint.subtract(threat)
    ...
    private static Vector3d guardianPosition(Vector3d target,
                                         Vector3d protectedPoint) {
        Vector3d towardThreat = target.subtract(protectedPoint)
                .multiply(1.0D, 0.0D, 1.0D);
