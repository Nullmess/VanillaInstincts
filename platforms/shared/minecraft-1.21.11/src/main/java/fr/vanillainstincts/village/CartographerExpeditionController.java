...
public final class CartographerExpeditionController {
    ...
    private static boolean tryStart(Villager villager,
                                    ...
                                    ServerLevel level,
                                    long gameTime) {
        if (gameTime < fr.vanillainstincts.persistence.NbtCompat.getLong(villager.getPersistentData(), READY_AT)
    ...
    private static void maintainSurvey(Villager villager, ServerLevel level,
        ...
        if (gameTime < fr.vanillainstincts.persistence.NbtCompat.getLong(villager.getPersistentData(), STAGE_AT)) {
            return;
        }
    ...
    private static boolean isCartographer(Villager villager) {
        return villager != null && villager.getVillagerData().profession().value()
                == VillagerProfessionCompat.value(VillagerProfession.CARTOGRAPHER);
    }

    private static int stage(Villager villager) {
        return fr.vanillainstincts.persistence.NbtCompat.getInt(villager.getPersistentData(), STAGE);
    }
