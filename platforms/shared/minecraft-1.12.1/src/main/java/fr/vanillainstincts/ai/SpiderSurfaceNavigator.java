...
public final class SpiderSurfaceNavigator {
    ...
    public static Optional<SpiderSurfacePath> plan(
            EntitySpider spider, Vec3d destination, WorldServer level, long gameTime) {
    ...
    public static boolean continuesToward(EntitySpider spider, Vec3d destination,
                                          long gameTime) {
    ...
    public static boolean tick(EntitySpider spider, SpeciesRuntimeState species,
        ...
        Vec3d target = targetForActiveStep(spider, level, active, node);
    ...
    public static void clearLevel(WorldServer level) {
        ...
        synchronized (ACTIVE) {
            ACTIVE.entrySet().removeIf(entry -> entry.getKey() == null
                    || entry.getKey().world == level);
        }
    }

    private static boolean shouldUseSurfaceNavigation(
            EntitySpider spider, Vec3d destination) {
        if (spider == null || destination == null) {
            return false;
        }
        double vertical = Math.abs(destination.y - spider.posY);
        return spider.collidedHorizontally
    ...
    private static Vec3d targetForActiveStep(EntitySpider spider, WorldServer level,
                                            ActivePath active,
    ...
    private static boolean trySkipBlockedNode(
        ...
        Vec3d target = SpiderSurfacePathfinder.targetFeet(spider, level, skip);
    ...
    private static void moveToward(EntitySpider spider, SpiderSurfaceNode node,
                                   Vec3d target) {
        Vec3d delta = target.subtract(spider.getPositionVector());
        ...
        Vec3d desired = delta.normalize().scale(speed);

        if (mode == SpiderSurfaceMode.GROUND) {
            spider.setNoGravity(false);
            desired = new Vec3d(desired.x,
                    fr.vanillainstincts.compat.Minecraft112Compat.motion(spider).y, desired.z);
        } else {
        ...
        Vec3d current = fr.vanillainstincts.compat.Minecraft112Compat.motion(spider);
        Vec3d blended = current.scale(
    ...
    private static class ActivePath {
        ...
        private final Vec3d lastProgressPosition;
        private final long lastProgressSampleAt;
        private final int stuckSamples;
        private final boolean cornerCleared;

        public ActivePath(SpiderSurfacePath path, Vec3d destination, int index, long plannedAt, Vec3d lastProgressPosition, long lastProgressSampleAt, int stuckSamples, boolean cornerCleared) {
            this.path = path;
        ...
        public Vec3d lastProgressPosition() { return this.lastProgressPosition; }
        ...
        ActivePath withIndex(int newIndex, Vec3d position, long gameTime) {
            return new ActivePath(path, destination, newIndex, plannedAt,
                    position, gameTime, 0, false);
        }

        ActivePath withProgress(Vec3d position, long gameTime, int samples) {
            return new ActivePath(path, destination, index, plannedAt,
                    position, gameTime, samples, cornerCleared);
        }

        ActivePath withCornerCleared(boolean cleared, Vec3d position,
                                     long gameTime) {
            return new ActivePath(path, destination, index, plannedAt,
