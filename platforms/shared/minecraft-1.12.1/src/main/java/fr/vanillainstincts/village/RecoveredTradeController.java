...
public final class RecoveredTradeController {
    ...
    public static boolean contribute(EntityVillager villager,
        ...
        if (fr.vanillainstincts.compat.Minecraft112Compat.distanceSq(villager, item)
            ...
            plan.offerNavigation(VanillaInstinctsState.RECOVERED_TRADE,
                    ...
                    item.getPositionVector(), ProfessionRules.RECOVERED_TRADE_SPEED,
    ...
    private static EntityItem nearestAccepted(EntityVillager merchant,
        ...
        return level.getEntitiesWithinAABB(EntityItem.class,
                        merchant.getEntityBoundingBox().grow(radius, 16.0D, radius),
                        entity -> entity.isEntityAlive() && !entity.cannotPickup()
    ...
    public static boolean recover(EntityVillager merchant, EntityItem entity) {
        ...
        if (result.isEmpty()) {
            merchant.swingArm(EnumHand.MAIN_HAND);
        ...
        MerchantRecipe recoveredOffer = new MerchantRecipe(
                new ItemStack(Items.EMERALD, emeraldCost), ItemStack.EMPTY, result, 0, maxUses);
    ...
    public static boolean addProducedOffer(EntityVillager merchant,
        ...
        for (int index = 0; index < offers.size(); index++) {
            MerchantRecipe existing = offers.get(index);
        ...
        int incidentScore = merchant.getEntityData()
                .getInteger("vanillainstincts_price_incident_score");
        int emeraldCost = Math.max(1, Math.min(64, price + Math.max(0, incidentScore)));
        MerchantRecipe offer = new MerchantRecipe(
                new ItemStack(Items.EMERALD, emeraldCost), ItemStack.EMPTY, result, 0, uses);
        offers.add(offer);
        return true;
    }
    ...
    public static boolean usefulToWanderingTrader(ItemStack stack) {
        ...
        return !stack.getItem().equals(Items.AIR) && !stack.getItem().equals(net.minecraft.item.Item.getItemFromBlock(net.minecraft.init.Blocks.BARRIER))
                && !stack.getItem().equals(net.minecraft.item.Item.getItemFromBlock(net.minecraft.init.Blocks.STRUCTURE_VOID))
                && !stack.getItem().equals(net.minecraft.item.Item.getItemFromBlock(net.minecraft.init.Blocks.COMMAND_BLOCK))
                && !stack.getItem().equals(net.minecraft.item.Item.getItemFromBlock(net.minecraft.init.Blocks.CHAIN_COMMAND_BLOCK))
                && !stack.getItem().equals(net.minecraft.item.Item.getItemFromBlock(net.minecraft.init.Blocks.REPEATING_COMMAND_BLOCK))
                && !stack.getItem().equals(net.minecraft.item.Item.getItemFromBlock(net.minecraft.init.Blocks.STRUCTURE_BLOCK));
    }
    ...
    private static int pricedFor(EntityVillager merchant, ItemStack stack,
        ...
        if (merchant != null && merchant.world instanceof WorldServer) { WorldServer level = (WorldServer) (merchant.world); 
    ...
    private static long nextOfferSalt(EntityVillager merchant) {
        ...
        long day = merchant.world.getWorldTime() / 24_000L;
