...
public final class GenericMobilityController {
    ...
    public static void contribute(EntityLiving mob, MobDecisionPlan plan,
        ...
        safeGapLanding(mob, target, level).ifPresent(landing -> {
            Vec3 horizontal = landing.subtract(fr.vanillainstincts.compat.Minecraft17Compat.position(mob));
    ...
    public static Optional<Vec3> safeGapLanding(EntityLiving mob, EntityLivingBase target,
        ...
        BlockPos gapFeet = new BlockPos(fr.vanillainstincts.compat.Minecraft17Compat.position(mob).add(direction));
        ...
        if (fr.vanillainstincts.compat.Minecraft112Compat.entityCanStandOn(fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, gapFloor), level, gapFloor, mob)) {
            return Optional.empty();
        }
        if (fr.vanillainstincts.compat.Minecraft112Compat.hasCollision(fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, gapFeet), level, gapFeet)
                || fr.vanillainstincts.compat.Minecraft112Compat.hasCollision(fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, gapFeet.up()), level, gapFeet.up())) {
            return Optional.empty();
        }

        Vec3 requested = fr.vanillainstincts.compat.Minecraft17Compat.position(mob).add(fr.vanillainstincts.compat.Minecraft112Compat.scale(direction, 
    ...
    private static boolean adjacentToWater(WorldServer level, BlockPos feet) {
        for (fr.vanillainstincts.compat.EnumFacing direction
                : fr.vanillainstincts.compat.EnumFacing.Plane.HORIZONTAL) {
            if (fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, feet.offset(direction)).getBlock().getMaterial() == Material.water) {
                return true;
            }
        }
        return fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, feet.down()).getBlock().getMaterial() == Material.water;
    }
}
