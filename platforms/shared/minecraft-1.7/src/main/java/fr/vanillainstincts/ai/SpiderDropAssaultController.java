...
public final class SpiderDropAssaultController {
    ...
    public static boolean contribute(EntitySpider spider, EntityLivingBase target,
        ...
        if (carryExpired || badlyHurt || fr.vanillainstincts.compat.Minecraft112Compat.distanceSq(fr.vanillainstincts.compat.Minecraft17Compat.position(spider), safe.get())
    ...
    public static Optional<Vec3> firingPosition(EntityLivingBase target,
                                                EntitySpider spider,
                                                WorldServer level) {
        Vec3 look = fr.vanillainstincts.compat.Minecraft112Compat.multiply(fr.vanillainstincts.compat.Minecraft17Compat.look(target), 1.0D, 0.0D, 1.0D);
        ...
        for (double range : ranges) {
            Vec3 candidate = fr.vanillainstincts.compat.Minecraft17Compat.position(target).subtract(fr.vanillainstincts.compat.Minecraft112Compat.scale(look, range))
            ...
            Optional<Vec3> safe = SafePositionFinder.resolveGroundDestination(
                    spider, candidate);
            if (safe.isPresent() && fr.vanillainstincts.compat.Minecraft17Compat.isBlockLoaded(level, 
                    new fr.vanillainstincts.compat.BlockPos(safe.get()))) {
                return safe;
            }
        }
    ...
    public static Vec3 dropPosition(EntityLivingBase target, EntitySpider spider) {
        Vec3 look = target == null ? new Vec3(0.0D, 0.0D, 0.0D)
                : fr.vanillainstincts.compat.Minecraft112Compat.multiply(fr.vanillainstincts.compat.Minecraft17Compat.look(target), 1.0D, 0.0D, 1.0D);
        if (fr.vanillainstincts.compat.Minecraft112Compat.lengthSq(look) < 1.0E-8D && target != null && spider != null) {
            look = fr.vanillainstincts.compat.Minecraft112Compat.multiply(fr.vanillainstincts.compat.Minecraft17Compat.position(target).subtract(fr.vanillainstincts.compat.Minecraft17Compat.position(spider)), 1.0D, 0.0D, 1.0D);
        }
        ...
        double sign = spider != null && (spider.getEntityId() & 1) == 0
                ? 1.0D : -1.0D;
        Vec3 origin = target == null ? new Vec3(0.0D, 0.0D, 0.0D) : fr.vanillainstincts.compat.Minecraft17Compat.position(target);
