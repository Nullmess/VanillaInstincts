...
public final class EndermanCargoSelector {
    ...
    public static boolean canCarry(EntityEnderman enderman,
        ...
        if (cargo instanceof EntityPlayer) { EntityPlayer player = (EntityPlayer) (cargo); 
            return !player.capabilities.isCreativeMode && !fr.vanillainstincts.compat.Minecraft17Compat.isSpectator(player)
    ...
    public static Entity chooseCargo(EntityEnderman enderman,
                                                EntityLivingBase objective,
                                                WorldServer level) {
        List<EntityLivingBase> candidates = fr.vanillainstincts.compat.Minecraft112Compat.entitiesWithinAABB(level, 
