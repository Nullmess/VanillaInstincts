...
public final class LegacyRegistry<T> implements Iterable<T> {
    public static final LegacyRegistry<Block> BLOCK = new LegacyRegistry<Block>(
            id -> id == null ? null : (Block) Block.blockRegistry.getObject(id.toString()),
            value -> value == null ? null : new ResourceLocation(String.valueOf(Block.blockRegistry.getNameForObject(value))),
            () -> copyRaw(Block.blockRegistry, Block.class));

    public static final LegacyRegistry<Item> ITEM = new LegacyRegistry<Item>(
            id -> id == null ? null : (Item) Item.itemRegistry.getObject(id.toString()),
            value -> value == null ? null : new ResourceLocation(String.valueOf(Item.itemRegistry.getNameForObject(value))),
            () -> copyRaw(Item.itemRegistry, Item.class));
    ...
    private static <T> Collection<T> copyRaw(Iterable iterable, Class<T> type) {
        ArrayList<T> result = new ArrayList<>();
        if (iterable == null || type == null) return result;
        for (Object value : iterable) if (type.isInstance(value)) result.add(type.cast(value));
        return result;
    }
