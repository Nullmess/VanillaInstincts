...
public final class ClericBrewingController {
    ...
    public static boolean contribute(Villager cleric,
        ...
        if (cleric == null || cleric.isBaby() || cleric.isTrading()
                ...
                || cleric.getVillagerData().profession().value()
        ...
        refreshDailyBatches(cleric, level.getOverworldClockTime());
        if (VillagerRoutineController.phaseFor(level.getOverworldClockTime())
                != VillagerSchedulePhase.WORK
                || !mayStartDailyBatch(fr.vanillainstincts.persistence.NbtCompat.getInt(cleric.getPersistentData(), BATCH_COUNT))
                || gameTime < fr.vanillainstincts.persistence.NbtCompat.getLong(cleric.getPersistentData(), READY_AT)
        ...
        SelectedBatch selected = selectBatch(level, cleric, stand,
                fr.vanillainstincts.persistence.NbtCompat.getLong(cleric.getPersistentData(), BATCH_SEQUENCE));
    ...
    private static boolean contributeActiveBatch(Villager cleric,
        ...
        BlockPos standPos = BlockPos.of(fr.vanillainstincts.persistence.NbtCompat.getLong(cleric.getPersistentData(), ACTIVE_STAND));
        if (!level.hasChunkAt(standPos)
                || !(level.getBlockEntity(standPos)
                instanceof BrewingStandBlockEntity stand)) {
            if (gameTime - fr.vanillainstincts.persistence.NbtCompat.getLong(cleric.getPersistentData(), ACTIVE_STARTED_AT)
        ...
        int loadStep = fr.vanillainstincts.persistence.NbtCompat.getInt(cleric.getPersistentData(), ACTIVE_LOAD_STEP);
        if (loadStep < LOAD_COMPLETE) {
            ...
            long actionAt = fr.vanillainstincts.persistence.NbtCompat.getLong(cleric.getPersistentData(), ACTIVE_ACTION_AT);
            plan.offerSpecial(VanillaInstinctsState.CLERIC_BREW,
                    ActionOwner.VILLAGER_PROFESSION,
        ...
        long inspectAt = fr.vanillainstincts.persistence.NbtCompat.getLong(cleric.getPersistentData(), ACTIVE_INSPECT_AT);
    ...
    private static void startBatch(Villager cleric, ServerLevel level,
        ...
        cleric.getPersistentData().putLong(BATCH_SEQUENCE,
                fr.vanillainstincts.persistence.NbtCompat.getLong(cleric.getPersistentData(), BATCH_SEQUENCE) + 1L);
    ...
    private static void loadNextVisibleStep(Villager cleric,
        ...
        int step = fr.vanillainstincts.persistence.NbtCompat.getInt(cleric.getPersistentData(), ACTIVE_LOAD_STEP);
        boolean changed;
        if (step == LOAD_FUEL) {
    ...
    private static void clearDisplayedServiceItem(Villager cleric,
                                                   long gameTime) {
        long clearAt = fr.vanillainstincts.persistence.NbtCompat.getLong(cleric.getPersistentData(), ACTIVE_HAND_CLEAR_AT);
    ...
    private static void inspectAndCollect(Villager cleric,
                                          ...
                                          BrewingStandBlockEntity stand,
                                          long gameTime) {
        long startedAt = fr.vanillainstincts.persistence.NbtCompat.getLong(cleric.getPersistentData(), ACTIVE_STARTED_AT);
        ...
        boolean expectedIngredientPresent = recipe != null
                ...
                && ItemStack.isSameItemSameComponents(ingredientSlot,
                recipe.ingredient());
        ...
        for (int slot = 0; slot < 3; slot++) {
            ...
            if (!output.isEmpty() && !expectedOutput.isEmpty()
                    && ItemStack.isSameItemSameComponents(output,
                    expectedOutput)) {
                completedSlots.add(slot);
            }
        }

        long sequence = fr.vanillainstincts.persistence.NbtCompat.getLong(cleric.getPersistentData(), BATCH_SEQUENCE);
        int represented = 0;
        ...
        int expected = Math.max(0, fr.vanillainstincts.persistence.NbtCompat.getInt(cleric.getPersistentData(), ACTIVE_EXPECTED));
        clearActiveBatch(cleric);
        refreshDailyBatches(cleric, level.getOverworldClockTime());
        cleric.getPersistentData().putInt(BATCH_COUNT,
                fr.vanillainstincts.persistence.NbtCompat.getInt(cleric.getPersistentData(), BATCH_COUNT) + 1);
        cleric.getPersistentData().putLong(READY_AT,
    ...
    private static void clearServiceIngredientRemainder(
        ...
        ItemStackTemplate remainderTemplate =
                recipe.ingredient().getItem().getCraftingRemainder();
    ...
    private static BatchPlan activeRecipe(ServerLevel level, Villager cleric) {
        List<BatchPlan> plans = serviceRecipes(level);
        int index = fr.vanillainstincts.persistence.NbtCompat.getInt(cleric.getPersistentData(), ACTIVE_RECIPE);
    ...
    private static List<BatchPlan> buildServiceRecipes(ServerLevel level) {
        HolderLookup.RegistryLookup<Potion> potions = level.registryAccess()
        ...
        for (Holder.Reference<Potion> potion : potions.listElements().toList()) {
            for (Item container : containers) {
                ItemStack input = PotionContents.createItemStack(container,
                        potion);
                if (!level.potionBrewing().isValidInput(input)) continue;
                for (Item ingredientItem : ingredients) {
                    ItemStack ingredient = new ItemStack(ingredientItem);
                    if (ingredient.isEmpty()
                            || !level.potionBrewing().hasMix(input, ingredient)) {
                        continue;
                    }
                    ItemStack output = level.potionBrewing().mix(
    ...
    private static boolean sameRecipe(BatchPlan first, BatchPlan second) {
        return ItemStack.isSameItemSameComponents(first.input(), second.input())
                && ItemStack.isSameItemSameComponents(first.ingredient(),
                second.ingredient())
                && ItemStack.isSameItemSameComponents(first.output(),
                second.output());
    }
    ...
    private static boolean standReservedByOther(ServerLevel level,
        ...
        return !level.getEntitiesOfClass(Villager.class,
                        ...
                        other -> other != cleric && other.isAlive()
                                && other.getVillagerData().profession().value()
                                == VillagerProfessionCompat.value(VillagerProfession.CLERIC)
                                && hasActiveBatch(other)
                                && fr.vanillainstincts.persistence.NbtCompat.getLong(other.getPersistentData(), ACTIVE_STAND) == standPos.asLong())
                .isEmpty();
    }
    ...
    private static void refreshDailyBatches(Villager cleric, long dayTime) {
        ...
        if (fr.vanillainstincts.persistence.NbtCompat.getLong(cleric.getPersistentData(), BATCH_DAY) != day) {
