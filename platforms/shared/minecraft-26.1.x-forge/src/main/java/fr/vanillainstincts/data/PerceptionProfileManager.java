...
public final class PerceptionProfileManager
    ...
    private static volatile List<ProfileRule> rules = List.of();
    private static volatile double maximumHearingScale = 1.0D;
    private static final Map<EntityType<?>, PerceptionProfile> PROFILE_CACHE =
            new ConcurrentHashMap<>();

    public PerceptionProfileManager() {
        super("perception_profiles");
    }
    ...
    public static PerceptionProfile profileFor(Mob mob) {
        return mob == null ? FALLBACK : profileFor(mob.getType());
    }

    public static PerceptionProfile profileFor(EntityType<?> type) {
    ...
    public static double visualRange(Mob observer, LivingEntity target,
                                     ServerLevel level) {
        ...
        if (level.isRainingAt(observer.blockPosition())) {
    ...
    public static double hearingRadius(Mob observer, ServerLevel level,
                                       double baseRadius,
        ...
        double radius = baseRadius * profile.hearingMultiplier()
                * difficultyMultiplier(profile, level.getDifficulty())
                * RuntimeConfig.snapshot().distanceMultiplier();
        if (noiseSource != null && noiseSource.isShiftKeyDown()) {
    ...
    public static int seenMemoryTicks(Mob mob) {
        return profileFor(mob).lastSeenTicks();
    }

    public static int heardMemoryTicks(Mob mob) {
        return profileFor(mob).lastHeardTicks();
    }

    public static double blindnessRange(Mob mob) {
    ...
    private static double difficultyMultiplier(PerceptionProfile profile,
        ...
        return switch (difficulty) {
            case PEACEFUL -> profile.easyDifficultyMultiplier() * 0.85D;
            case EASY -> profile.easyDifficultyMultiplier();
            case NORMAL -> profile.normalDifficultyMultiplier();
            case HARD -> profile.hardDifficultyMultiplier();
        };
    }

    private static ProfileRule parse(Identifier id, JsonElement element) {
    ...
    public record PerceptionProfile(double visualRange,
                                    double hearingMultiplier,
                                    double nightVisualMultiplier,
                                    double rainVisualMultiplier,
                                    double sneakingVisualMultiplier,
                                    double invisibleVisualMultiplier,
                                    double blindnessRange,
                                    double sneakingHearingMultiplier,
                                    double easyDifficultyMultiplier,
                                    double normalDifficultyMultiplier,
                                    double hardDifficultyMultiplier,
                                    int lastSeenTicks,
                                    int lastHeardTicks) {
    ...
    private record ProfileRule(Identifier id,
                               int priority,
    ...
    private record EntityMatcher(boolean wildcard,
        ...
        boolean matches(EntityType<?> type) {
            ...
            if (tag) {
                return type.builtInRegistryHolder().is(TagKey.create(Registries.ENTITY_TYPE, id));
            }
