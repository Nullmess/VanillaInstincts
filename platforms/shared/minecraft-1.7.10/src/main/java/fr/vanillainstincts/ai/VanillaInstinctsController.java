...
public final class VanillaInstinctsController {
    ...
    public static void tick(EntityLiving mob) {
        if (!(mob.worldObj instanceof WorldServer) || !mob.isEntityAlive()) {
            return;
        ...
        try {
            ...
            if (mob instanceof EntityVillager) { EntityVillager villager = (EntityVillager) (mob); 
            ...
            } else if (mob instanceof EntityZombie) { EntityZombie zombie = (EntityZombie) (mob); 
                if (fr.vanillainstincts.compat.Minecraft110Compat.isZombieVillager(zombie)) {
                    if (FeatureGate.enabled(FeatureFlag.IRON_DOOR_LEARNING, level)) {
                        LearnedDoorController.maintainZombieVillager(zombie, level, gameTime);
                    }
                } else if (FeatureGate.enabled(FeatureFlag.ZOMBIE_TACTICS, level)) {
                    tickZombie(zombie, level, gameTime);
                }
            } else if (mob instanceof EntityIronGolem) { EntityIronGolem golem = (EntityIronGolem) (mob); 
                if (FeatureGate.enabled(FeatureFlag.GOLEM_AI, level)) {
                    tickGolem(golem, level, gameTime);
                }
            } else if (mob instanceof EntityEnderman) { EntityEnderman enderman = (EntityEnderman) (mob); 
                if (FeatureGate.enabled(FeatureFlag.ENDERMAN_TACTICS, level)) {
                    tickEnderman(enderman, level, gameTime);
                }
            } else if (mob instanceof EntityCreeper) { EntityCreeper creeper = (EntityCreeper) (mob); 
                if (FeatureGate.enabled(FeatureFlag.CREEPER_TACTICS, level)) {
                    tickCreeper(creeper, level, gameTime);
                }
            } else if (mob instanceof EntitySpider) { EntitySpider spider = (EntitySpider) (mob); 
                if (FeatureGate.enabled(FeatureFlag.SPIDER_TACTICS, level)) {
                    tickSpider(spider, level, gameTime);
                }
            } else if (mob instanceof EntitySkeleton) { EntitySkeleton skeleton = (EntitySkeleton) (mob); 
                tickSkeleton(skeleton, level, gameTime);
            } else if (mob instanceof EntityWolf) { EntityWolf wolf = (EntityWolf) (mob); 
                if (FeatureGate.enabled(FeatureFlag.WOLF_PACKS, level)) {
                    ...
                    || FeatureGate.enabled(FeatureFlag.MOB_ECOLOGY, level))) { EntityAnimal animal = (EntityAnimal) (mob); 
    ...
    private static void tickVillager(EntityVillager villager, WorldServer level,
                                     long gameTime) {
    ...
    private static void tickGolem(EntityIronGolem golem, WorldServer level,
                                  long gameTime) {
        ...
        try {
            ...
            GolemDefenseController.contribute(golem, defense, plan, level,
                    gameTime);
            target = golem.getAttackTarget();
            if (constructionEnabled && target != null && target.isEntityAlive()) {
    ...
    private static void tickEnderman(EntityEnderman enderman, WorldServer level,
                                     long gameTime) {
    ...
    private static void tickCreeper(EntityCreeper creeper, WorldServer level,
                                    long gameTime) {
        ...
        try {
            ...
            if (target instanceof EntityPlayer && validCombatTarget(((EntityPlayer) (target)))
                    && VanillaInstinctsServerConfig.creeperTacticsAllowedAtY(
                    entityBlockPos(creeper).getY())) { EntityPlayer player = (EntityPlayer) (target); 
    ...
    private static void tickSpider(EntitySpider spider, WorldServer level,
                                   long gameTime) {
    ...
    private static void tickZombie(EntityZombie zombie, WorldServer level,
                                   long gameTime) {
    ...
    private static void tickSkeleton(EntitySkeleton skeleton,
    ...
    private static void tickWolf(EntityWolf wolf, WorldServer level,
                                 long gameTime) {
    ...
    private static void tickAnimal(EntityAnimal animal, WorldServer level,
                                   long gameTime) {
    ...
    private static boolean validCombatTarget(EntityLivingBase target) {
    ...
    private static boolean scheduled(EntityLiving mob, WorldServer level,
                                     FeatureFlag feature) {
