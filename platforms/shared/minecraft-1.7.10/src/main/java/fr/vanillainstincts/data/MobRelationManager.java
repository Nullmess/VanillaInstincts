...
/**
 * Reload-safe data-driven relation graph used by Acquisition 3.0 and EntityLiving
 ...
 */
public final class MobRelationManager {
    private static final Gson GSON = new GsonBuilder().create();
    private static volatile List<RelationRule> rules = fr.vanillainstincts.compat.LegacyJava8.listOf();
    private static final Map<ResourceLocation, Double> MAX_HUNT_DISTANCE =
            new ConcurrentHashMap<>();
    private static final Map<ResourceLocation, Double> MAX_RELATION_DISTANCE =
            new ConcurrentHashMap<>();
    ...
    public static Optional<RelationDecision> relation(EntityLiving observer,
                                                       EntityLivingBase target) {
        if (observer == null || target == null || observer == target) {
            return Optional.empty();
        }
        ResourceLocation observerType = entityKey(observer);
        ResourceLocation targetType = entityKey(target);
    ...
    public static double maximumHuntDistance(EntityLiving observer) {
        if (observer == null) return 0.0D;
        return MAX_HUNT_DISTANCE.computeIfAbsent(entityKey(observer), type -> {
            double maximum = 0.0D;
            for (RelationRule rule : rules) {
    ...
    public static double maximumRelationDistance(EntityLiving observer) {
        if (observer == null) return 0.0D;
        return MAX_RELATION_DISTANCE.computeIfAbsent(entityKey(observer), type -> {
            double maximum = 0.0D;
            for (RelationRule rule : rules) {
    ...
    private static class RelationRule {
        ...
        boolean matches(ResourceLocation observer, ResourceLocation target) {
            return matchesObserver(observer) && matchesAny(targets, target);
        }

        boolean matchesObserver(ResourceLocation observer) {
    ...
    private static ResourceLocation entityKey(net.minecraft.entity.Entity entity) {
        String key = EntityList.getEntityString(entity);
        return key == null ? new ResourceLocation("minecraft", "unknown")
