...
public final class AdaptiveEquipmentProgressionController {
    ...
    private static void wearEquipment(ItemStack stack, Tier tier,
        ...
        int damage = (int) Math.round(fr.vanillainstincts.compat.Minecraft17Compat.maxDamage(stack) * ratio);
        fr.vanillainstincts.compat.Minecraft17Compat.setItemDamage(stack, Math.min(fr.vanillainstincts.compat.Minecraft17Compat.maxDamage(stack) - 1, damage));
    }
    ...
    private static int countLocal(WorldServer level, EntityLiving origin, Tier tier) {
        ...
        return fr.vanillainstincts.compat.Minecraft112Compat.entitiesWithinAABB(level, EntityLiving.class,
                fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(origin).expand(radius, radius, radius), candidate ->
