...
public final class SpiderSurfaceNavigator {
    ...
    public static void activate(EntitySpider spider, SpiderSurfacePath path,
        ...
        synchronized (ACTIVE) {
            ACTIVE.put(spider, new ActivePath(path, destination,
                    ...
                    fr.vanillainstincts.compat.Minecraft17Compat.position(spider), gameTime, 0, false));
        }
    }
    ...
    public static boolean tick(EntitySpider spider, SpeciesRuntimeState species,
        ...
        double distanceSqr = fr.vanillainstincts.compat.Minecraft112Compat.distanceSq(fr.vanillainstincts.compat.Minecraft17Compat.position(spider), target);
        ...
        if (isWaitingForCornerWaypoint(active)
            ...
            active = active.withCornerCleared(true,
                    fr.vanillainstincts.compat.Minecraft17Compat.position(spider), gameTime);
            synchronized (ACTIVE) {
                ACTIVE.put(spider, active);
            }
            target = SpiderSurfacePathfinder.targetFeet(spider, level, node);
            distanceSqr = fr.vanillainstincts.compat.Minecraft112Compat.distanceSq(fr.vanillainstincts.compat.Minecraft17Compat.position(spider), target);
        }
        ...
        if (!isWaitingForCornerWaypoint(active)
            ...
            active = active.withIndex(nextIndex, fr.vanillainstincts.compat.Minecraft17Compat.position(spider), gameTime);
            synchronized (ACTIVE) {
                ACTIVE.put(spider, active);
            }
    ...
    private static ActivePath sampleProgress(EntitySpider spider, ActivePath active,
        ...
        double moved = fr.vanillainstincts.compat.Minecraft112Compat.distanceSq(fr.vanillainstincts.compat.Minecraft17Compat.position(spider), 
                active.lastProgressPosition);
        int samples = moved < SpiderRules.SPIDER_SURFACE_MIN_PROGRESS_SQR
                ? active.stuckSamples + 1 : 0;
        return active.withProgress(fr.vanillainstincts.compat.Minecraft17Compat.position(spider), gameTime, samples);
    }
    ...
    private static boolean trySkipBlockedNode(
        ...
        if (fr.vanillainstincts.compat.Minecraft112Compat.distanceSq(fr.vanillainstincts.compat.Minecraft17Compat.position(spider), target)
    ...
    private static void moveToward(EntitySpider spider, SpiderSurfaceNode node,
                                   Vec3 target) {
        Vec3 delta = target.subtract(fr.vanillainstincts.compat.Minecraft17Compat.position(spider));
