...
if (tag == null) return;
    ...
    public static class CeremonySessionRecord {
        ...
        private NBTTagCompound save() {
            ...
            tag.setUniqueId("builder", builderId);
            tag.setUniqueId("farmer", farmerId);
            tag.setUniqueId("supplier", supplierId);
            if (clericId != null) {
                tag.setUniqueId("cleric", clericId);
            }
            NBTTagList observerList = new NBTTagList();
            for (UUID observer : observers) {
                NBTTagCompound value = new NBTTagCompound();
                value.setUniqueId("id", observer);
        ...
        private static CeremonySessionRecord load(NBTTagCompound tag) {
            if (tag == null || !tag.hasUniqueId("builder")
                    || !tag.hasUniqueId("farmer")
                    || !tag.hasUniqueId("supplier")
                    || !tag.hasKey("bell", 4)
                    || !tag.hasKey("site", 4)) {
                return null;
            }
            ...
            for (int index = 0; index < observerList.tagCount(); index++) {
                ...
                if (value.hasUniqueId("id")) {
                    observers.add(value.getUniqueId("id"));
                }
            }
            ...
            return new CeremonySessionRecord(
                    ...
                    tag.getUniqueId("builder"),
                    tag.getUniqueId("farmer"),
                    tag.getUniqueId("supplier"),
                    tag.hasUniqueId("cleric") ? tag.getUniqueId("cleric") : null,
                    observers,
                    destinations,
    ...
    public static class DestinationRecord {
        ...
        private NBTTagCompound save() {
            NBTTagCompound tag = new NBTTagCompound();
            tag.setUniqueId("participant", participantId);
            tag.setDouble("x", destination.x);
            tag.setDouble("y", destination.y);
            tag.setDouble("z", destination.z);
            return tag;
        }

        private static DestinationRecord load(NBTTagCompound tag) {
            if (tag == null || !tag.hasUniqueId("participant")) {
                return null;
            }
            ...
            return new DestinationRecord(tag.getUniqueId("participant"),
                    destination);
        }
    }
