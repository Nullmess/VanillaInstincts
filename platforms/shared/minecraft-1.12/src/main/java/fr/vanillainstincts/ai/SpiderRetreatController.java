...
public final class SpiderRetreatController {
    ...
    public static boolean contribute(EntitySpider spider, EntityLivingBase target,
        ...
        Vec3d away = fr.vanillainstincts.compat.Minecraft112Compat.multiply(spider.getPositionVector().subtract(target.getPositionVector()), 1.0D, 0.0D, 1.0D);
        if (fr.vanillainstincts.compat.Minecraft112Compat.lengthSq(away) < 1.0E-8D) {
            away = new Vec3d((spider.getEntityId() & 1) == 0 ? 1.0D : -1.0D,
                    0.0D, 1.0D);
        }
        away = away.normalize();
        Vec3d side = new Vec3d(-away.z, 0.0D, away.x)
                .scale((spider.getEntityId() & 2) == 0 ? 2.0D : -2.0D);
        Vec3d destination = spider.getPositionVector().add(away.scale(
        ...
        if (spider.onGround && (gameTime + spider.getEntityId()) % 12L == 0L) {
            Vec3d impulse =fr.vanillainstincts.compat.Minecraft112Compat.add(away.scale(0.28D), 0.0D, 0.34D, 0.0D);
    ...
    private static void maybePlaceRetreatWeb(EntitySpider spider,
        ...
        Vec3d towardThreat = fr.vanillainstincts.compat.Minecraft112Compat.multiply(target.getPositionVector().subtract(spider.getPositionVector()), 1.0D, 0.0D, 1.0D);
