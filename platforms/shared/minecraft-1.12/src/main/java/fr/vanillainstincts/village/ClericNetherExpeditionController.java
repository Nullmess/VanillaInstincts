...
public final class ClericNetherExpeditionController {
    ...
    private static void maintainLive(EntityVillager cleric, WorldServer level,
                                     long gameTime) {
        fr.vanillainstincts.compat.Minecraft112Compat.setInvulnerable(cleric, true);
        fr.vanillainstincts.compat.Minecraft112Compat.addEffect(cleric, new PotionEffect(MobEffects.FIRE_RESISTANCE,
                240, 0, true, false));
        fr.vanillainstincts.compat.Minecraft112Compat.addEffect(cleric, new PotionEffect(MobEffects.STRENGTH,
                240, 1, true, false));
        fr.vanillainstincts.compat.Minecraft112Compat.addEffect(cleric, new PotionEffect(MobEffects.REGENERATION,
                120, 1, true, false));
        ...
        if (gameTime >= cleric.getEntityData().getLong(ACTION_AT)) {
            ...
            EntityBlaze blaze = level.getEntitiesWithinAABB(EntityBlaze.class,
                    cleric.getEntityBoundingBox().grow(
    ...
    private static void collectNetherDrops(EntityVillager cleric,
        ...
        for (EntityItem entity : level.getEntitiesWithinAABB(EntityItem.class,
                cleric.getEntityBoundingBox().grow(3.0D), item -> item.isEntityAlive()
            ...
            if (stack.isEmpty()) fr.vanillainstincts.compat.Minecraft112Compat.removeEntity(entity);
    ...
    public static boolean portalSiteClear(WorldServer level, BlockPos base) {
        ...
        for (int x = 0; x < 4; x++) {
            for (int y = 0; y < 5; y++) {
                ...
                if (!fr.vanillainstincts.compat.Minecraft112Compat.isAir(state) && !state.getMaterial().isReplaceable()) return false;
            }
        }
        return true;
    }
    ...
    private static boolean forceReturnHome(EntityVillager cleric, WorldServer level,
        ...
        if (cleric.world != overworld) {
            Entity changed = cleric.changeDimension(LegacyDimensionType.OVERWORLD.id());
            if (!(changed instanceof EntityVillager)) return false; EntityVillager moved = (EntityVillager) (changed);
            returned = moved;
        }
    ...
    private static void discardPendingPayment(EntityVillager cleric,
                                              WorldServer level) {
        if (level == null || !cleric.getEntityData()
                .hasUniqueId(BARTER_PAYMENT)) return;
        Entity payment = level.getEntityFromUuid(cleric.getEntityData()
                .getUniqueId(BARTER_PAYMENT));
    ...
    private static int findSlot(InventoryBasic inventory, Item item) {
        for (int i = 0; i < inventory.getSizeInventory(); i++) {
            if (inventory.getStackInSlot(i).getItem().equals(item)
                    && !inventory.getStackInSlot(i).isEmpty()) return i;
        }
        return -1;
    }
    ...
    private static void insert(EntityVillager cleric, ItemStack stack) {
        ...
        if (!stack.isEmpty()) fr.vanillainstincts.compat.Minecraft112Compat.spawnItem(cleric, stack);
        cleric.getVillagerInventory().markDirty();
    }
