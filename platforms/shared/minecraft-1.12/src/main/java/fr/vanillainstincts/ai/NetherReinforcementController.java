...
public final class NetherReinforcementController {
    ...
    public static void resetDefeatedAggressor(WorldServer sourceLevel,
        ...
        for (WorldServer level : sourceLevel.getMinecraftServer().worlds) {
    ...
    private static boolean maintainMessenger(EntityLiving messenger,
        ...
        if (state.phase() == NetherReinforcementPhase.TO_NETHER) {
            if (inNether) {
                ...
                state.setSearchUntil(gameTime
                        + NetherRules.NETHER_REINFORCEMENT_SEARCH_TICKS);
                messenger.getNavigator().clearPath();
                return true;
            }
            if (!inOverworld) {
    ...
    private static int createObservedFallbackPack(
        ...
        for (int index = 0; index < count; index++) {
            ...
            Vec3d emergence = findEmergencePosition(level, netherPortal,
                    index + 1);
            fr.vanillainstincts.compat.Minecraft112Compat.teleport(reinforcement, emergence.x, emergence.y, emergence.z,fr.vanillainstincts.compat.Minecraft112Compat.random(level).nextFloat() * 360.0F, 0.0F);
            ...
            if (level.spawnEntity(reinforcement)) {
    ...
    private static int recruitPack(EntityLiving messenger, EntityLiving first,
        ...
        for (EntityLiving candidate : candidates) {
            ...
            candidate.getNavigator().clearPath();
    ...
    private static boolean moveToStoredPortal(EntityLiving mob,
        ...
        fr.vanillainstincts.compat.Minecraft112Compat.lookAt(mob, destination.x, destination.y,
    ...
    private static void materializeVirtualExpedition(
        ...
        for (int index = 0; index < count; index++) {
            ...
            Vec3d emergence = findEmergencePosition(overworld, portal,
                    index + 1);
            fr.vanillainstincts.compat.Minecraft112Compat.teleport(reinforcement, emergence.x, emergence.y, emergence.z,fr.vanillainstincts.compat.Minecraft112Compat.random(overworld).nextFloat() * 360.0F, 0.0F);
            NetherReinforcementState state =
                    new NetherReinforcementState();
            ...
            if (overworld.spawnEntity(reinforcement)) {
    ...
    private static void spawnReturningMessenger(
        ...
        fr.vanillainstincts.compat.Minecraft112Compat.teleport(messenger, emergence.x, emergence.y, emergence.z,fr.vanillainstincts.compat.Minecraft112Compat.random(overworld).nextFloat() * 360.0F, 0.0F);
        ...
        overworld.spawnEntity(messenger);
    }
    ...
    private static void beginRetreat(EntityLiving reinforcement,
        ...
        state.beginRetreat(gameTime,
                NetherRules.NETHER_REINFORCEMENT_RETREAT_TICKS);
        reinforcement.getNavigator().clearPath();
    }
    ...
    private static MinecraftServer levelServer(EntityLiving mob) {
        return mob != null && mob.world instanceof WorldServer
                ? ((WorldServer) (mob.world)).getMinecraftServer() : null;
    }
    ...
    private static void calmZoglinsTargetingMessenger(
        ...
        AxisAlignedBB area = messenger.getEntityBoundingBox().grow(
    ...
    private static int missionMemberCount(WorldServer level, EntityLiving center,
        ...
        AxisAlignedBB area = center.getEntityBoundingBox().grow(
