...
public final class ZombieTacticsController {
    ...
    public static void contribute(EntityZombie zombie, MobDecisionPlan plan,
        ...
        Optional<Vec3d> remembered = MobPerceptionMemory.bestKnownPosition(
                zombie, gameTime);
        Vec3d destination = visible && target != null
                ? target.getPositionVector() : remembered.orElse(null);
    ...
    private static void offerContextLeap(EntityZombie zombie, EntityLivingBase target,
        ...
        Vec3d horizontal = fr.vanillainstincts.compat.Minecraft112Compat.multiply(target.getPositionVector().subtract(zombie.getPositionVector()), 1.0D, 0.0D, 1.0D);
        ...
        BlockPos front = new BlockPos(zombie.posX + direction.x,
                zombie.posY + 0.2D, zombie.posZ + direction.z);
        if (!fr.vanillainstincts.compat.Minecraft112Compat.hasCollision(zombie.world.getBlockState(front),
                zombie.world, front)) {
            return;
        }
        Vec3d impulse =fr.vanillainstincts.compat.Minecraft112Compat.add(direction.scale(ZombieRules.LEAP_HORIZONTAL), 0.0D, ZombieRules.LEAP_VERTICAL, 0.0D);
    ...
    private static void offerToolPassage(EntityZombie zombie, Vec3d destination,
                                         MobDecisionPlan plan,
                                         WorldServer level, long gameTime) {
    ...
    public static Optional<BlockPos> findToolObstacle(EntityZombie zombie,
                                                       Vec3d destination,
                                                       WorldServer level) {
        Vec3d horizontal = fr.vanillainstincts.compat.Minecraft112Compat.multiply(destination.subtract(zombie.getPositionVector()), 1.0D, 0.0D, 1.0D);
        if (fr.vanillainstincts.compat.Minecraft112Compat.lengthSq(horizontal) < 1.0E-6D) return Optional.empty();
        horizontal = horizontal.normalize();
        BlockPos feet = new BlockPos(zombie.posX + horizontal.x,
                zombie.posY + 0.2D, zombie.posZ + horizontal.z);
    ...
    public static boolean canBreakWithHeldTool(EntityZombie zombie,
        ...
        if (zombie == null || level == null || pos == null
                || !level.isBlockLoaded(pos) || level.getTileEntity(pos) != null) {
            return false;
        }
        ...
        ItemStack tool = zombie.getHeldItemMainhand();
        return !tool.isEmpty() && tool.getDestroySpeed(state) > 1.0F;
    }
