...
public final class FarmerVillageServiceController {
    ...
    public static boolean contribute(EntityVillager farmer,
        ...
        if (carried > 0) {
            List<BlockPos> destinations = readPositions(farmer,
                    ...
                    .filter(pos -> level.isBlockLoaded(pos)
                            && fr.vanillainstincts.compat.Minecraft112Compat.isAir(level.getBlockState(pos)))
            ...
            return approachOrAct(farmer, plan, anchor, () -> {
                ...
                for (BlockPos destination : destinations) {
                    if (!level.isBlockLoaded(destination)
                            || !fr.vanillainstincts.compat.Minecraft112Compat.isAir(level.getBlockState(destination))) {
                        continue;
                    }
                    BlockPos below = destination.down();
                    if (!fr.vanillainstincts.compat.Minecraft112Compat.isSolidRender(level.getBlockState(below))) {
                        continue;
                    }
                    if (WorldPermissionService.setBlock(level, farmer,
                            destination, Blocks.HAY_BLOCK.getDefaultState(),
                ...
                if (!RecoveredTradeController.addProducedOffer(
                    ...
                    ProfessionStockController.insert(farmer.getVillagerInventory(),
                            bread);
                    if (!bread.isEmpty()) fr.vanillainstincts.compat.Minecraft112Compat.spawnItem(farmer, bread);
        ...
        if (!sources.isEmpty()) {
            ...
            return approachOrAct(farmer, plan, anchor, () -> {
                ...
                for (BlockPos source : sources) {
                    if (level.isBlockLoaded(source)
                            && level.getBlockState(source).getBlock().equals(Blocks.HAY_BLOCK)) {
                        if (WorldPermissionService.setBlock(level, farmer,
                                source, Blocks.AIR.getDefaultState(),
                                3,
    ...
    public static boolean canPlaceHay(WorldServer level, BlockPos pos) {
        return level.isBlockLoaded(pos) && fr.vanillainstincts.compat.Minecraft112Compat.isAir(level.getBlockState(pos))
    ...
    public static boolean canPlaceHayCluster(WorldServer level,
        ...
        for (BlockPos pos : positions) {
            ...
            if (!planned.contains(below)
                    && !fr.vanillainstincts.compat.Minecraft112Compat.isSolidRender(level.getBlockState(below))) {
                return false;
            }
        }
        return true;
    ...
    private static void rememberProtectedHay(WorldServer level,
                                             ...
                                             long gameTime,
                                             long duration) {
        List<EntityVillager> farmers = new ArrayList<>(level.getEntitiesWithinAABB(
                EntityVillager.class, farmer.getEntityBoundingBox().grow(
