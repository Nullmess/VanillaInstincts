...
public final class VillagerSafetyController {
    ...
    public static boolean prepareToFlee(EntityVillager villager,
        ...
        if (fr.vanillainstincts.compat.Minecraft112Compat.isSleeping(villager)) {
            ...
            villager.setSprinting(false);
            Vec3d velocity = fr.vanillainstincts.compat.Minecraft112Compat.motion(villager);
            fr.vanillainstincts.compat.Minecraft112Compat.setMotion(villager, 0.0D, velocity.y, 0.0D);
    ...
    public static Optional<Vec3d> findFleeDestination(EntityVillager villager,
                                                      WorldServer level,
                                                      BlockPos danger) {
        Vec3d away = villager.getPositionVector().subtract(Minecraft115VectorCompat.atCenterOf(danger));
    ...
    public static Optional<Vec3d> findBehindDefenderDestination(
        ...
        Vec3d threatPosition = threat != null && threat.isEntityAlive()
                ? threat.getPositionVector() : Minecraft115VectorCompat.atCenterOf(rememberedDanger);
        Vec3d away = fr.vanillainstincts.compat.Minecraft112Compat.multiply(golem.getPositionVector().subtract(threatPosition), 1.0D, 0.0D, 1.0D);
        if (fr.vanillainstincts.compat.Minecraft112Compat.lengthSq(away) < 1.0E-8D) {
            away = fr.vanillainstincts.compat.Minecraft112Compat.multiply(villager.getPositionVector().subtract(threatPosition), 1.0D, 0.0D, 1.0D);
        }
        if (fr.vanillainstincts.compat.Minecraft112Compat.lengthSq(away) < 1.0E-8D) {
            away = new Vec3d(1.0D, 0.0D, 0.0D);
        }
        ...
        Vec3d center = golem.getPositionVector().add(away.scale(
        ...
        for (Vec3d candidate : candidates) {
            ...
            double defenderDistance = fr.vanillainstincts.compat.Minecraft112Compat.distanceSq(position, golem.getPositionVector());
            if (defenderDistance
    ...
    private static EntityIronGolem resolveDefender(VillagerRuntimeState state,
        ...
        Entity entity = level.getEntityFromUuid(state.defenderGolemId(gameTime));
    ...
    private static EntityLivingBase resolveTrackedThreat(
        ...
        Entity entity = level.getEntityFromUuid(state.reportedAggressorId());
    ...
    private static Optional<EntityLivingBase> nearestImmediateThreat(
        ...
        AxisAlignedBB area = villager.getEntityBoundingBox().grow(
                VillageSocialRules.VILLAGER_DANGER_RADIUS);
        return level.getEntitiesWithinAABB(EntityLivingBase.class, area,
