...
public final class EndermanAdvancedController {
    ...
    public static boolean contributeBlockedAggro(EntityEnderman enderman,
        ...
        Optional<Vec3d> destination = selectPunishmentDestination(enderman,
                player, level);
        if (!destination.isPresent()) return false;
        Vec3d chosen = destination.get();
        plan.offerSpecial(VanillaInstinctsState.ENDERMAN_FORCED_TELEPORT,
                ...
                20, () -> {
                    ...
                    fr.vanillainstincts.compat.Minecraft112Compat.teleport(player, chosen.x, chosen.y, chosen.z);
                    player.fallDistance = 0.0F;
                    emitTeleport(level, before);
                    emitTeleport(level, chosen);
                    enderman.playSound(SoundEvents.ENTITY_ENDERMEN_TELEPORT,
                            1.0F, 1.0F);
    ...
    public static boolean contributeDrowningRescue(EntityEnderman enderman,
        ...
        EntityLivingBase drowning = level.getEntitiesWithinAABB(EntityLivingBase.class,
                        enderman.getEntityBoundingBox().grow(
        ...
        Optional<Vec3d> dry = dryDestination(enderman, level,
                drowning.getPositionVector());
        if (!dry.isPresent()) return false;
        Vec3d destination = dry.get();
        plan.offerSpecial(VanillaInstinctsState.TELEPORT_CARGO,
                ActionOwner.ENDERMAN_TACTICS,
                EndermanRules.PRIORITY_ENDERMAN_GENERIC_DELIVERY + 6,
                16, () -> {
                    Vec3d before = drowning.getPositionVector();
                    fr.vanillainstincts.compat.Minecraft112Compat.teleport(drowning, destination.x, destination.y,
                            destination.z);
    ...
    public static boolean contributeCarriedBlockPresentation(
        ...
        Vec3d look = fr.vanillainstincts.compat.Minecraft112Compat.multiply(player.getLookVec(), 1.0D, 0.0D, 1.0D);
        ...
        Vec3d side = new Vec3d(-look.z, 0.0D, look.x)
                .scale((enderman.getEntityId() & 1) == 0 ? 1.5D : -1.5D);
        Vec3d requested = player.getPositionVector().subtract(look.scale(
    ...
    public static Optional<Vec3d> selectPunishmentDestination(
            EntityEnderman enderman, EntityPlayer player, WorldServer level) {
        List<EntityMob> hostiles = level.getEntitiesWithinAABB(EntityMob.class,
                player.getEntityBoundingBox().grow(
        ...
        EntityMob best = hostiles.stream()
                ...
                .orElse(null);
        if (best != null) {
            Optional<Vec3d> near = safeNearEntity(player, best, enderman,
                    level);
    ...
    public static boolean respectsMinimumDistance(Vec3d destination,
                                                  Vec3d endermanPosition) {
    ...
    private static Optional<Vec3d> safeNearEntity(EntityPlayer player,
                                                 EntityLivingBase entity,
                                                 EntityEnderman enderman,
                                                 WorldServer level) {
        for (int i = 0; i < 8; i++) {
            double angle = Math.PI * 2.0D * i / 8.0D;
            Vec3d candidate =fr.vanillainstincts.compat.Minecraft112Compat.add(entity.getPositionVector(), Math.cos(angle) * 3.0D,
    ...
    private static Optional<Vec3d> findLoadedLava(EntityPlayer player,
                                                 EntityEnderman enderman,
                                                 WorldServer level) {
        ...
        for (BlockPos pos : fr.vanillainstincts.compat.Minecraft112BlockPosCompat.betweenClosed(
            ...
            if (level.getBlockState(pos).getMaterial() == Material.LAVA
                    && !fr.vanillainstincts.compat.Minecraft112Compat.hasCollision(level.getBlockState(pos.up()), level, pos.up())) {
        ...
        return candidates.stream()
                ...
                .map(pos -> new Vec3d(pos.getX() + 0.5D,
                        pos.getY() + 0.15D, pos.getZ() + 0.5D));
    }

    private static boolean safeForPlayer(EntityPlayer player, Vec3d candidate,
                                         EntityEnderman enderman,
                                         WorldServer level) {
        if (!respectsMinimumDistance(candidate, enderman.getPositionVector())) {
            return false;
        }
        BlockPos feet = new BlockPos(candidate);
        if (!level.isBlockLoaded(feet)
                || !!level.getBlockState(feet).getMaterial().isLiquid()) return false;
        AxisAlignedBB moved = fr.vanillainstincts.compat.Minecraft112Compat.moveBox(player.getEntityBoundingBox(),
                candidate.subtract(player.getPositionVector()));
        return fr.vanillainstincts.compat.Minecraft112Compat.noCollision(level, player, moved)
                && fr.vanillainstincts.compat.Minecraft112Compat.hasCollision(level.getBlockState(feet.down()), level, feet.down());
    }

    private static Optional<Vec3d> dryDestination(EntityEnderman enderman,
                                                 WorldServer level,
                                                 Vec3d near) {
        for (int i = 0; i < 12; i++) {
            double angle = Math.PI * 2.0D * i / 12.0D;
            Vec3d requested =fr.vanillainstincts.compat.Minecraft112Compat.add(near, Math.cos(angle) * 5.0D,
                    0.0D, Math.sin(angle) * 5.0D);
            Optional<Vec3d> safe = SafePositionFinder.resolveGroundDestination(
                    enderman, requested);
            if (safe.isPresent() && !level.getBlockState(
                    new BlockPos(safe.get())).getMaterial().isLiquid()) {
                return safe;
            }
        }
        return Optional.empty();
    ...
    private static void emitTeleport(WorldServer level, Vec3d pos) {
        level.spawnParticle(EnumParticleTypes.PORTAL, pos.x, pos.y + 1.0D,
