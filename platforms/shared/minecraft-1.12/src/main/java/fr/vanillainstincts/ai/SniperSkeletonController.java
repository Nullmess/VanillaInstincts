...
public final class SniperSkeletonController {
    ...
    public static void onJoin(AbstractSkeleton skeleton, WorldServer level,
    ...
    public static void tick(AbstractSkeleton skeleton, WorldServer level,
                            long gameTime) {
        if (!isSniper(skeleton) || skeleton instanceof EntityWitherSkeleton) {
            return;
        }
    ...
    private static EntityPlayerMP nearestEligiblePlayer(
            AbstractSkeleton skeleton, WorldServer level) {
    ...
    private static int countLocalSnipers(AbstractSkeleton skeleton,
        ...
        return level.getEntitiesWithinAABB(AbstractSkeleton.class,
                skeleton.getEntityBoundingBox().grow(radius),
    ...
    private static EntityPlayerMP resolveTarget(AbstractSkeleton skeleton,
                                              WorldServer level) {
    ...
    private static boolean validTarget(AbstractSkeleton skeleton,
        ...
        if (!isCombatTarget(player) || player.world != skeleton.world) {
            return false;
        }
        double max = configuredMaxRange();
    ...
    private static void shoot(AbstractSkeleton skeleton, EntityPlayerMP target,
                              WorldServer level) {
        ...
        double distance = skeleton.getDistance(target);
        double lead = leadTicks(distance);
        Vec3d motion = fr.vanillainstincts.compat.Minecraft112Compat.motion(target);
        double aimX = target.posX + motion.x * lead;
        double aimY = (target.posY + target.height * 0.55D) + motion.y * lead;
        double aimZ = target.posZ + motion.z * lead;
    ...
    private static ItemStack weapon(AbstractSkeleton skeleton) {
        ItemStack mainHand = skeleton.getHeldItemMainhand();
        return mainHand.getItem().equals(Items.BOW) ? mainHand
                : skeleton.getHeldItemOffhand();
    }

    private static boolean holdsBow(AbstractSkeleton skeleton) {
        return skeleton.getHeldItemMainhand().getItem().equals(Items.BOW)
                || skeleton.getHeldItemOffhand().getItem().equals(Items.BOW);
    }
