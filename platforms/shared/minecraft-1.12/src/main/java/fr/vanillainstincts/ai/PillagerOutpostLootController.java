...
public final class PillagerOutpostLootController {
    ...
    public static int onPlayerDrops(EntityPlayer player, WorldServer level,
        ...
        for (EntityItem drop : drops) {
            if (drop == null || !drop.isEntityAlive() || drop.getStackInSlot().isEmpty()) {
                continue;
            }
    ...
    private static PillagerEntity selectCarrier(WorldServer level,
        ...
        AxisAlignedBB area = new AxisAlignedBB(death.getX(), death.getY(), death.getZ(),
                ...
                death.getZ() + 1.0D).grow(
                PillagerRules.OUTPOST_LOOT_SCAN_RADIUS,
                PillagerRules.OUTPOST_LOOT_VERTICAL_RADIUS,
                PillagerRules.OUTPOST_LOOT_SCAN_RADIUS);
        return level.getEntitiesWithinAABB(PillagerEntity.class, area,
    ...
    public static boolean availableForMission(PillagerEntity pillager) {
        return pillager != null && pillager.isEntityAlive()
                && !pillager.getEntityData().hasUniqueId(PILLAGER_MISSION)
    ...
    private static void beginMission(PillagerEntity pillager, UUID mission,
        ...
        pillager.getEntityData().setUniqueId(PILLAGER_MISSION, mission);
        ...
        for (int index = 0; index < items.size(); index++) {
            ...
            pillager.getEntityData().setUniqueId(PILLAGER_ITEM_PREFIX + index,
                    item.getUniqueID());
    ...
    public static boolean maintain(PillagerEntity pillager, WorldServer level,
        ...
        if (!pillager.getEntityData().hasUniqueId(PILLAGER_MISSION)) {
            return false;
        }
        ...
        if (!level.getBlockState(depositTarget).getBlock().equals(Blocks.CHEST)) {
            pillager.swingArm(EnumHand.MAIN_HAND);
        }
    ...
    public static boolean isReservedLoot(EntityItem item) {
        return item != null && item.getEntityData().hasUniqueId(LOOT_MISSION);
    }

    private static void reserve(EntityItem item, UUID mission, UUID carrier,
                                BlockPos chest, BlockPos death,
                                long gameTime) {
        item.getEntityData().setUniqueId(LOOT_MISSION, mission);
        ...
        item.getEntityData().setUniqueId(LOOT_CARRIER, carrier);
    ...
    private static java.util.List<EntityItem> resolveMissionItems(
        ...
        UUID mission = pillager.getEntityData().hasUniqueId(PILLAGER_MISSION)
                ? pillager.getEntityData().getUniqueId(PILLAGER_MISSION) : null;
        int count = Math.max(0, pillager.getEntityData()
                .getInteger(PILLAGER_ITEM_COUNT));
        for (int index = 0; index < count; index++) {
            String key = PILLAGER_ITEM_PREFIX + index;
            if (!pillager.getEntityData().hasUniqueId(key)) continue;
            Entity entity = level.getEntityFromUuid(pillager.getEntityData()
                    .getUniqueId(key));
            if (entity instanceof EntityItem && ((EntityItem) (entity)).isEntityAlive()
                    ...
                    .getEntityData().getUniqueId(LOOT_MISSION))
                    && ((EntityItem) (entity)).getEntityData().hasUniqueId(LOOT_CARRIER)
    ...
    private static void releaseMissionItems(PillagerEntity pillager,
                                            WorldServer level) {
        if (!pillager.getEntityData().hasUniqueId(PILLAGER_MISSION)) return;
        UUID mission = pillager.getEntityData().getUniqueId(PILLAGER_MISSION);
        ...
        AxisAlignedBB area = new AxisAlignedBB(death.getX(), death.getY(), death.getZ(),
                death.getX() + 1.0D, death.getY() + 1.0D,
                death.getZ() + 1.0D).grow(radius,
                PillagerRules.OUTPOST_LOOT_VERTICAL_RADIUS, radius);
        for (EntityItem item : level.getEntitiesWithinAABB(EntityItem.class, area,
                candidate -> candidate.isEntityAlive()
                        && candidate.getEntityData().hasUniqueId(LOOT_MISSION)
    ...
    private static void migrateLegacyAssignment(PillagerEntity pillager,
        ...
        if (pillager.getEntityData().getInteger(PILLAGER_ITEM_COUNT) > 0
                || !pillager.getEntityData().hasUniqueId(PILLAGER_ITEM)) {
            return;
        }
        UUID itemId = pillager.getEntityData().getUniqueId(PILLAGER_ITEM);
        Entity entity = level.getEntityFromUuid(itemId);
        ...
        pillager.getEntityData().setUniqueId(PILLAGER_ITEM_PREFIX + 0, itemId);
    ...
    private static ItemStack insertChestAt(WorldServer level,
        ...
        return insert(level.getTileEntity(position), input);
    }
    ...
    private static boolean chestCanAccept(WorldServer level, BlockPos position,
                                          ItemStack input) {
        TileEntity entity = level.getTileEntity(position);
    ...
    private static BlockPos tryExpandChest(WorldServer level, Entity actor,
        ...
        IBlockState baseState = level.getBlockState(base);
        EnumFacing facing = baseState.getValue(BlockChest.FACING);
        ...
        IBlockState partnerState = Blocks.CHEST.getDefaultState()
    ...
    private static BlockPos expansionPosition(WorldServer level, BlockPos base,
                                              EnumFacing preferred) {
        IBlockState state = level.getBlockState(base);
    ...
    private static boolean canPlaceSingleChest(WorldServer level,
        ...
        for (EnumFacing direction : EnumFacing.Plane.HORIZONTAL) {
            if (level.getBlockState(position.offset(direction))
    ...
    private static boolean isSafeChestSpace(WorldServer level,
                                            BlockPos position) {
        return level.isBlockLoaded(position)
                && level.getBlockState(position).isAir()
                && level.getBlockState(position.up()).isAir()
                && level.getBlockState(position.down()).isFaceSturdy(level,
    ...
    public static boolean isOutpostMaterial(IBlockState state) {
