...
public final class SpiderDropAssaultController {
    ...
    public static boolean contribute(SpiderEntity spider, LivingEntity target,
        ...
        Optional<Vector3d> safe = firingPosition(target, spider, level);
        if (!safe.isPresent()) {
            return false;
        }
    ...
    public static Optional<Vector3d> firingPosition(LivingEntity target,
                                                SpiderEntity spider,
                                                ServerWorld level) {
        Vector3d look = target.getLookAngle().multiply(1.0D, 0.0D, 1.0D);
        ...
        for (double range : ranges) {
            Vector3d candidate = target.position().subtract(look.scale(range))
    ...
    public static Vector3d dropPosition(LivingEntity target, SpiderEntity spider) {
        Vector3d look = target == null ? Vector3d.ZERO
        ...
        if (look.lengthSqr() < 1.0E-8D) {
            look = new Vector3d(1.0D, 0.0D, 0.0D);
        } else {
            look = look.normalize();
        }
        Vector3d side = new Vector3d(-look.z, 0.0D, look.x);
        double sign = spider != null && (spider.getId() & 1) == 0
                ? 1.0D : -1.0D;
        Vector3d origin = target == null ? Vector3d.ZERO : target.position();
