...
public final class RecoveredTradeController {
    ...
    public static boolean maintain(WanderingTraderEntity trader, ServerWorld level,
                                   long gameTime) {
    ...
    private static boolean scanNow(AbstractVillagerEntity merchant, long gameTime) {
        return Math.floorMod(gameTime + merchant.getId() * 23L,
                ProfessionRules.RECOVERED_TRADE_SCAN_INTERVAL_TICKS) == 0L;
    }

    private static ItemEntity nearestAccepted(AbstractVillagerEntity merchant,
                                               ServerWorld level,
    ...
    public static boolean recover(AbstractVillagerEntity merchant, ItemEntity entity) {
        ...
        int workmanship = 0;
        if (!playerOrigin && merchant instanceof VillagerEntity) { VillagerEntity villager = (VillagerEntity) (merchant); 
        ...
        MerchantOffer recoveredOffer = new MerchantOffer(
                new ItemStack(Items.EMERALD, price), result, maxUses,
    ...
    public static void markPlayerToss(ItemEntity entity, PlayerEntity player) {
    ...
    public static void markPlayerDeathDrop(ItemEntity entity, PlayerEntity player) {
    ...
    public static boolean isPlayerOrigin(ItemEntity entity) {
        ...
        if (entity.getPersistentData().getBoolean(PLAYER_TOSSED_ITEM)
                || entity.getPersistentData().getBoolean(PLAYER_DEATH_DROP)) {
            return true;
        }
        UUID ownerId = entity.getOwner();
        return ownerId != null && entity.level instanceof ServerWorld
                && ((ServerWorld) (entity.level)).getEntity(ownerId) instanceof PlayerEntity;
    }
    ...
    public static boolean consumePlayerRecoveredOffer(
            AbstractVillagerEntity merchant, MerchantOffer tradedOffer) {
    ...
    private static void markPlayerRecoveredOffer(AbstractVillagerEntity merchant,
                                                  int index) {
    ...
    private static boolean isPlayerRecoveredOffer(AbstractVillagerEntity merchant,
                                                   int index) {
    ...
    private static void shiftPlayerRecoveredIndicesAfterRemoval(
            AbstractVillagerEntity merchant, int removedIndex) {
        int[] previous = merchant.getPersistentData()
                .getIntArray(PLAYER_RECOVERED_OFFER_INDICES);
        int kept = 0;
     ...
     */
    public static boolean addProducedOffer(VillagerEntity merchant,
                                           ItemStack produced) {
    ...
    public static boolean addProducedOffer(VillagerEntity merchant,
        ...
        MerchantOffer offer = new MerchantOffer(
                new ItemStack(Items.EMERALD, price), result, uses,
                ProfessionRules.RECOVERED_TRADE_XP,
                ProfessionRules.RECOVERED_TRADE_PRICE_MULTIPLIER);
        int incidentScore = merchant.getPersistentData()
                .getInt("vanillainstincts_price_incident_score");
     ...
     */
    public static boolean hasAvailableProducedOffer(VillagerEntity merchant,
                                                     ItemStack produced) {
        if (merchant == null || produced == null || produced.isEmpty()) {
            return false;
        }
    ...
    public static boolean accepts(VillagerProfession profession,
        ...
        if (profession == VillagerProfession.FARMER) {
            return isFood(stack) || isCultivation(path)
                    || stack.getItem().equals(Items.EGG) || stack.getItem().equals(Items.TURTLE_EGG);
        }
        ...
        if (profession == VillagerProfession.FLETCHER) {
            return path.contains("bow") || path.contains("arrow")
                    ...
                    || stack.getItem().equals(Items.TRIPWIRE_HOOK);
        }
    ...
    public static boolean usefulToWanderingTrader(ItemStack stack) {
        ...
        return !stack.getItem().equals(Items.AIR) && !stack.getItem().equals(Items.BARRIER)
                && !stack.getItem().equals(Items.STRUCTURE_VOID)
                && !stack.getItem().equals(Items.DEBUG_STICK)
                && !stack.getItem().equals(Items.COMMAND_BLOCK)
                && !stack.getItem().equals(Items.CHAIN_COMMAND_BLOCK)
                && !stack.getItem().equals(Items.REPEATING_COMMAND_BLOCK)
                && !stack.getItem().equals(Items.STRUCTURE_BLOCK)
                && !stack.getItem().equals(Items.JIGSAW);
    }

    public static boolean isVanillaFishingLoot(ItemStack stack) {
        return stack != null && !stack.isEmpty() && (
                stack.getItem().equals(Items.COD) || stack.getItem().equals(Items.SALMON)
                        ...
                        || stack.getItem().equals(Items.NAUTILUS_SHELL)
                        || stack.getItem().equals(Items.SADDLE)
                        || stack.getItem().equals(Items.LILY_PAD)
                        ...
                        || stack.getItem().equals(Items.INK_SAC)
                        || stack.getItem().equals(Items.TRIPWIRE_HOOK));
    }


    private static int pricedFor(AbstractVillagerEntity merchant, ItemStack stack,
                                 long offerSalt, int workmanship) {
        int base = emeraldPrice(merchant == null ? null : merchant.getUUID(),
                stack, offerSalt, workmanship);
        if (merchant != null && merchant.level instanceof ServerWorld) { ServerWorld level = (ServerWorld) (merchant.level); 
    ...
    private static double rarityValue(ItemStack stack) {
        return fr.vanillainstincts.compat.LegacyJava8.eval(() -> { switch ((stack.getRarity())) { case COMMON:  return 0.0D; case UNCOMMON:  return 1.0D; case RARE:  return 3.5D; case EPIC:  return 8.0D;  default: throw new AssertionError("Unexpected switch value"); } });
    }
    ...
    private static long nextOfferSalt(AbstractVillagerEntity merchant) {
