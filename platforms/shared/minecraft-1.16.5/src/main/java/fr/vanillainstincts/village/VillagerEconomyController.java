...
public final class VillagerEconomyController {
    ...
    private static final Map<ServerWorld, Map<IncidentKey, IncidentWindow>>
            INCIDENTS = Collections.synchronizedMap(new WeakHashMap<>());

    private VillagerEconomyController() {
    }

    public static int record(VillagerEntity merchant, ServerPlayerEntity player,
                             Incident incident, long gameTime) {
        if (merchant == null || player == null || incident == null
                || !(merchant.level instanceof ServerWorld)) {
            return 0;
        } ServerWorld level = (ServerWorld) (merchant.level);
        ...
        merchant.getPersistentData().putInt("vanillainstincts_price_incident_score",
                Math.min(ProfessionRules.VILLAGER_PRICE_PENALTY_MAX,
                        merchant.getPersistentData()
                                .getInt("vanillainstincts_price_incident_score")
                                + amount));
        return count;
    }

    public static void maintain(VillagerEntity merchant) {
        if (merchant == null) return;
        int score = Math.min(ProfessionRules.VILLAGER_PRICE_PENALTY_MAX,
                Math.max(0, merchant.getPersistentData()
                        .getInt("vanillainstincts_price_incident_score")));
        if (score <= 0) return;
    ...
    public static void applyPenalty(AbstractVillagerEntity merchant, int amount) {
