...
public final class GenericMobilityController {
    ...
    public static void contribute(MobEntity mob, MobDecisionPlan plan,
        ...
        if (target == null || !target.isAlive() || !mob.isOnGround()
        ...
        safeGapLanding(mob, target, level).ifPresent(landing -> {
            Vector3d horizontal = landing.subtract(mob.position());
            horizontal = new Vector3d(horizontal.x, 0.0D, horizontal.z);
            if (horizontal.lengthSqr() < 1.0E-6D) return;
            horizontal = horizontal.normalize();
            Vector3d impulse = horizontal.scale(MobilityRules.GAP_FORWARD_IMPULSE)
    ...
    public static Optional<Vector3d> safeGapLanding(MobEntity mob, LivingEntity target,
                                                ServerWorld level) {
        if (mob == null || target == null || level == null) {
            return Optional.empty();
        }
        Vector3d direction = target.position().subtract(mob.position());
        direction = new Vector3d(direction.x, 0.0D, direction.z);
        ...
        Vector3d requested = mob.position().add(direction.scale(
        ...
        landingFeet = new BlockPos(landingFeet.getX(), currentFeet.getY(),
                landingFeet.getZ());
        Vector3d landing = Vector3d.atBottomCenterOf(landingFeet);
    ...
    public static Optional<Vector3d> nearestShore(MobEntity mob, ServerWorld level) {
        ...
        return java.util.stream.IntStream.rangeClosed(-radius, radius).boxed()
                ...
                .map(pos -> Vector3d.atBottomCenterOf(pos))
                .filter(candidate -> {
