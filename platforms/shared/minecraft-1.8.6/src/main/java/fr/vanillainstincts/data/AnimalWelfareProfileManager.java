...
/**
 * Species-aware EntityAnimal Welfare 3.0 settings loaded from datapacks.
 *
...
public final class AnimalWelfareProfileManager {
    ...
    private void apply(Map<ResourceLocation, JsonObject> objects) {
        ...
        for (Map.Entry<ResourceLocation, JsonObject> entry : objects.entrySet()) {
            try {
                ...
                VanillaInstincts.LOGGER.warn(
                        "Profil EntityAnimal Welfare ignoré ({}): {}",
        ...
        VanillaInstincts.LOGGER.info(
                "EntityAnimal Welfare 3.0: {} profil(s) datapack chargé(s)",
                profiles.size());
    }
    ...
    public static WelfareProfile profileFor(EntityAnimal animal) {
        if (animal == null) return DEFAULT;
        ResourceLocation type = fr.vanillainstincts.compat.Minecraft110EntityCompat.key(animal);
    ...
    public static class EntityMatcher {
        ...
        static EntityMatcher parse(String selector) {
            ...
            ResourceLocation id = fr.vanillainstincts.compat.LegacyResourceLocation.tryParse(raw);
            if (id == null) {
                throw new IllegalArgumentException(
