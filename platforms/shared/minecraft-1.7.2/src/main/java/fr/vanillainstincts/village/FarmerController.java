...
public final class FarmerController {
    ...
    public static void contribute(EntityVillager villager,
        ...
        if (LegacyVillagerProfession.of(villager)
                ...
                || villager.isTrading()
                || !level.getGameRules().getGameRuleBooleanValue(
                "mobGriefing")) {
            return;
        }
    ...
    public static boolean isActionable(WorldServer level, BlockPos pos,
        ...
        return (fr.vanillainstincts.compat.Minecraft112Compat.isAir(blockState) || blockState.getBlock().getMaterial().isReplaceable())
                && FarmerCropRegistry.hasPotentialPlantingGround(level, pos)
                && findPlantingSlot(fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(villager), level, pos,
                reserve) >= 0;
    }

    public static boolean isMatureCrop(LegacyBlockState state) {
        return FarmerCropRegistry.isMature(state);
    }

    public static boolean isHarvestableFruit(LegacyBlockState state) {
    ...
    public static EntityVillager onPlayerCropBroken(WorldServer level,
        ...
        EntityVillager farmer = fr.vanillainstincts.compat.Minecraft112Compat.entitiesWithinAABB(level, EntityVillager.class,
    ...
    public static boolean isPlayerReplantCandidate(LegacyBlockState state) {
    ...
    public static boolean performAction(WorldServer level, EntityVillager villager,
                                        BlockPos pos,
                                        boolean incidentRepair) {
        if (!level.getGameRules().getGameRuleBooleanValue("mobGriefing")) {
            return false;
        }
        ...
        if (FarmerCropRegistry.isMature(state)) {
            LegacyBlockState replacement = FarmerCropRegistry.harvestedReplacement(
                    state);
            if (replacement == null) {
            ...
            net.minecraft.tileentity.TileEntity harvestedBlockEntity = fr.vanillainstincts.compat.Minecraft17Compat.getTileEntity(level, pos);
            if (!fr.vanillainstincts.compat.Minecraft17Compat.canPlaceBlockAt(replacement.getBlock(), level, pos)
                    || !WorldPermissionService.setBlock(level, villager, pos,
                    replacement, 3,
                    WorldActionType.REPLACE_BLOCK)) {
                return false;
            }
            fr.vanillainstincts.compat.Minecraft17Compat.dropBlockAsItem(state, level, pos, 0);
        ...
        if ((fr.vanillainstincts.compat.Minecraft112Compat.isAir(state) || state.getBlock().getMaterial().isReplaceable())
                && FarmerCropRegistry.hasPotentialPlantingGround(level, pos)) {
            int reserve = incidentRepair
                    ? VillageConstructionRules.FARMER_EMERGENCY_SEED_RESERVE
                    : VillageConstructionRules.FARMER_RESERVED_SEEDS;
            int slot = findPlantingSlot(fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(villager), level, pos,
                    reserve);
            if (slot < 0) return false;
            ItemStack stack = fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(villager).getStackInSlot(slot);
            LegacyBlockState planting = FarmerCropRegistry.plantingState(
                    stack.getItem());
            if (planting == null
    ...
    public static int neighboringCropCount(WorldServer level, BlockPos pos,
        ...
        for (EnumFacing direction : new EnumFacing[]{EnumFacing.NORTH,
            ...
            if (fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, pos.offset(direction)).getBlock() == crop) count++;
        }
        return count;
    }
    ...
    public static boolean shouldLeaveMatureReserve(WorldServer level,
        ...
        for (BlockPos nearby : fr.vanillainstincts.compat.Minecraft112BlockPosCompat.betweenClosed(fr.vanillainstincts.compat.Minecraft112Compat.offset(pos, -2, 0, -2),
            ...
            LegacyBlockState candidate = fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, nearby);
    ...
    private static boolean awaitingPlantingStock(WorldServer level,
                                                  BlockPos pos,
                                                  EntityVillager villager) {
        return pos != null && fr.vanillainstincts.compat.Minecraft112Compat.isAir(fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, pos))
                && FarmerCropRegistry.hasPotentialPlantingGround(level, pos)
                && findPlantingSlot(fr.vanillainstincts.compat.Minecraft17Compat.villagerInventory(villager), level, pos,
