 ...
 *
 * The list is deliberately explicit so a Minecraft update adding a new EntityLiving
 * cannot silently claim complete vanilla coverage. Modded EntityLiving subclasses use
 * the generic runtime profile when enabled by server config.
 */
public final class VanillaMobPossessionCatalog {
    private static final Set<String> VANILLA_MOBS = fr.vanillainstincts.compat.LegacyJava8.setOf(
    ...
    private static final Set<String> FLYING = fr.vanillainstincts.compat.LegacyJava8.setOf(
            "allay", "bat", "bee", "blaze", "ghast", "parrot", "phantom",
            "vex", "wither");

    private static final Set<String> AQUATIC = fr.vanillainstincts.compat.LegacyJava8.setOf(
            "cod", "dolphin", "elder_guardian", "glow_squid",
            "guardian", "pufferfish", "salmon", "squid", "tadpole",
            "tropical_fish");

    private static final Set<String> AMPHIBIOUS = fr.vanillainstincts.compat.LegacyJava8.setOf(
            "axolotl", "drowned", "frog", "turtle");

    private static final Set<String> STATIONARY = fr.vanillainstincts.compat.LegacyJava8.setOf(
            "shulker");

    private static final Set<String> DRAGON = fr.vanillainstincts.compat.LegacyJava8.setOf(
            "ender_dragon");

    private static final Set<String> CLIMBERS = fr.vanillainstincts.compat.LegacyJava8.setOf(
            "cave_spider", "spider");

    private VanillaMobPossessionCatalog() {
    }

    public static boolean coversVanillaMob(EntityLiving mob) {
        ResourceLocation id = LegacyRegistry.ENTITY_TYPE.getKey(mob.getType());
        return id != null && "minecraft".equals(id.getResourceDomain())
                && coversVanillaId(id.getResourcePath());
    }
    ...
    public static boolean isVanilla(EntityLiving mob) {
        ResourceLocation id = LegacyRegistry.ENTITY_TYPE.getKey(mob.getType());
        return id != null && "minecraft".equals(id.getResourceDomain());
    }

    public static String path(EntityLiving mob) {
        ResourceLocation id = LegacyRegistry.ENTITY_TYPE.getKey(mob.getType());
        return id == null ? "unknown" : id.getResourcePath();
    }

    public static Movement movement(EntityLiving mob) {
