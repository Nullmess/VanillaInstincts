...
public final class SpiderSurfaceCache {
    ...
    public static boolean hasCollision(WorldServer level, BlockPos pos,
        ...
        synchronized (LEVELS) {
            Map<BlockPos, Entry> entries = LEVELS.computeIfAbsent(level,
                    ignored -> new HashMap<>());
            boolean loaded = fr.vanillainstincts.compat.Minecraft17Compat.isBlockLoaded(level, pos);
            LegacyBlockState state = loaded ? fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, pos) : null;
    ...
    private static class Entry {
        ...
        public Entry(LegacyBlockState state, boolean collision, long expiresAt) {
            this.state = state;
