...
public final class CreeperTacticsController {
    ...
    public static void contributeVisible(EntityCreeper creeper, EntityPlayer player,
        ...
        species.sampleCreeperMovement(fr.vanillainstincts.compat.Minecraft17Compat.position(creeper), gameTime,
        ...
        if (FeatureGate.enabled(FeatureFlag.CREEPER_PASSAGE_BREAKING,
                ...
                creeper, level)
                && CreeperPassageController.canIgnite(creeper, fr.vanillainstincts.compat.Minecraft17Compat.position(player),
    ...
    private static void advanceWhileSwelling(EntityCreeper creeper,
        ...
        Vec3 direction = fr.vanillainstincts.compat.Minecraft112Compat.multiply(fr.vanillainstincts.compat.Minecraft17Compat.position(target).subtract(fr.vanillainstincts.compat.Minecraft17Compat.position(creeper)), 1.0D, 0.0D, 1.0D);
    ...
    public static Optional<Vec3> findBestAmbushPosition(EntityCreeper creeper,
                                                        EntityPlayer player,
                                                        WorldServer level) {
        Vec3 playerLook = horizontal(fr.vanillainstincts.compat.Minecraft17Compat.look(player));
        if (playerLook == null) {
        ...
        for (int radius = 3; radius <= 6; radius++) {
            for (Vec3 direction : directions) {
                Vec3 raw = fr.vanillainstincts.compat.Minecraft17Compat.position(player).add(fr.vanillainstincts.compat.Minecraft112Compat.scale(direction, radius));
    ...
    public static Optional<Vec3> findBlindSideOrbitPosition(
        ...
        Vec3 look = horizontal(fr.vanillainstincts.compat.Minecraft17Compat.look(player));
        if (look == null) {
            return Optional.empty();
        }
        ...
        List<Vec3> candidates = fr.vanillainstincts.compat.LegacyJava8.listOf(
                fr.vanillainstincts.compat.Minecraft17Compat.position(player).add(fr.vanillainstincts.compat.Minecraft112Compat.scale(behind, 
                ...
                fr.vanillainstincts.compat.Minecraft17Compat.position(player).add(fr.vanillainstincts.compat.Minecraft112Compat.scale(lateral, 
    ...
    public static boolean isPlayerLookingAt(EntityPlayer player, EntityCreeper creeper) {
        ...
        Vec3 toCreeper = fr.vanillainstincts.compat.Minecraft17Compat.eyes(creeper, 1.0F)
                .subtract(fr.vanillainstincts.compat.Minecraft17Compat.eyes(player, 1.0F));
        if (fr.vanillainstincts.compat.Minecraft112Compat.lengthSq(toCreeper) < 1.0E-6D) {
            return true;
        }
        return fr.vanillainstincts.compat.Minecraft112Compat.dot(fr.vanillainstincts.compat.Minecraft17Compat.look(player).normalize(), toCreeper.normalize())
    ...
    private static double scoreCandidate(EntityCreeper creeper, EntityPlayer player,
                                         ...
                                         Vec3 playerLook) {
        Vec3 fromPlayer = candidate.subtract(fr.vanillainstincts.compat.Minecraft17Compat.position(player));
        double rear = rearPreferenceScore(playerLook, fromPlayer);
        boolean visible = hasClearRay(level, fr.vanillainstincts.compat.Minecraft17Compat.eyes(player, 1.0F),fr.vanillainstincts.compat.Minecraft112Compat.add(candidate, 0.0D, creeper.getEyeHeight(), 0.0D), player);
        double travel = Math.sqrt(fr.vanillainstincts.compat.Minecraft112Compat.distanceSq(fr.vanillainstincts.compat.Minecraft17Compat.position(creeper), candidate));
        double playerDistance = Math.sqrt(
                fr.vanillainstincts.compat.Minecraft112Compat.distanceSq(fr.vanillainstincts.compat.Minecraft17Compat.position(player), candidate));
    ...
    private static Vec3 stealthAdvanceDestination(EntityPlayer player,
        ...
        Vec3 look = horizontal(fr.vanillainstincts.compat.Minecraft17Compat.look(player));
        if (look == null) {
            return fr.vanillainstincts.compat.Minecraft17Compat.position(player);
        }
        ...
        return fr.vanillainstincts.compat.Minecraft17Compat.position(player).add(behind).add(lateral);
    }
    ...
    private static boolean tryContextualLeap(EntityCreeper creeper, EntityPlayer player,
        ...
        Vec3 horizontal = horizontal(fr.vanillainstincts.compat.Minecraft17Compat.position(player)
                .subtract(fr.vanillainstincts.compat.Minecraft17Compat.position(creeper)));
        if (horizontal == null) {
            return false;
        }
    ...
    private static boolean hasClearRay(WorldServer level, Vec3 from, Vec3 to,
                                       EntityPlayer source) {
        MovingObjectPosition result = level.rayTraceBlocks(from, to, false);
    ...
    private static boolean isValidTarget(EntityCreeper creeper, EntityLivingBase target) {
        ...
        if (target instanceof EntityPlayer
                && (((EntityPlayer) (target)).capabilities.isCreativeMode || fr.vanillainstincts.compat.Minecraft17Compat.isSpectator((EntityPlayer) (target)))) { EntityPlayer player = (EntityPlayer) (target); 
            return false;
        }
