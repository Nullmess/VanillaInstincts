...
public final class SpiderTacticsController {
    ...
    public static void maintainAttachment(EntitySpider spider,
        ...
        if (species.spiderSurfaceMode() == SpiderSurfaceMode.CEILING) {
            if (ceiling) {
                ...
                Vec3 movement = fr.vanillainstincts.compat.Minecraft112Compat.motion(spider);
                double adhesion = rainAdhesionFactor(
                        fr.vanillainstincts.compat.Minecraft112Compat.isRainingAt(level, entityBlockPos(spider)));
    ...
    public static void contributeVisible(EntitySpider spider, EntityLivingBase target,
        ...
        if (trySurfaceTraversal(spider, fr.vanillainstincts.compat.Minecraft17Compat.position(target), species,
                plan, level, gameTime)) {
            return;
        }
    ...
    public static void contributeFromMemory(EntitySpider spider, Vec3 destination,
                                            SpeciesRuntimeState species,
                                            MobDecisionPlan plan,
                                            WorldServer level, long gameTime) {
    ...
    private static boolean trySurfaceTraversal(EntitySpider spider, Vec3 destination,
                                               SpeciesRuntimeState species,
                                               MobDecisionPlan plan,
                                               WorldServer level,
                                               long gameTime) {
        ...
        Vec3 horizontal = SpiderSurfacePlanner.directionAlongSurface(
                destination, fr.vanillainstincts.compat.Minecraft17Compat.position(spider));
        ...
        if (desired == SpiderSurfaceMode.WALL) {
            double adhesion = rainAdhesionFactor(
                    fr.vanillainstincts.compat.Minecraft112Compat.isRainingAt(level, entityBlockPos(spider)));
            Vec3 forward = fr.vanillainstincts.compat.Minecraft112Compat.scale(horizontal, 
                    SpiderRules.SPIDER_WALL_FORWARD_FORCE * adhesion);
            Vec3 impulse = new Vec3(forward.xCoord,
    ...
    private static boolean tryContextualLeap(EntitySpider spider,
        ...
        Vec3 horizontal = horizontal(fr.vanillainstincts.compat.Minecraft17Compat.position(target)
                .subtract(fr.vanillainstincts.compat.Minecraft17Compat.position(spider)));
        if (horizontal == null) {
            return false;
        }
    ...
    public static boolean hasWallContact(EntitySpider spider, WorldServer level,
        ...
        BlockPos center = new BlockPos(spider.posX,
                fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(spider).minY + spider.height * 0.55D,
                spider.posZ);
    ...
    public static boolean hasCeilingContact(EntitySpider spider, WorldServer level,
        ...
        BlockPos ceiling = new BlockPos(spider.posX,
                fr.vanillainstincts.compat.Minecraft17Compat.boundingBox(spider).maxY + 0.25D, spider.posZ);
