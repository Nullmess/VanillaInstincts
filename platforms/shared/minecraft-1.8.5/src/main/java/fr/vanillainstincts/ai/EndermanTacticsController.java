...
public final class EndermanTacticsController {
    ...
    private static void contributeCarrying(EntityEnderman enderman,
        ...
        if (state.role() == EndermanCargoRole.ALLY_RESCUE
            ...
            Vec3 away = horizontal(enderman.getPositionVector()
                    .subtract(objective.getPositionVector()));
            if (away == null) {
                away = new Vec3(1.0D, 0.0D, 0.0D);
            }
            Vec3 desired = enderman.getPositionVector().add(fr.vanillainstincts.compat.Minecraft112Compat.scale(away, 
        ...
        if (state.role() == EndermanCargoRole.SPIDER_PLATFORM
            ...
            if (spiderTarget != null && spiderTarget.isEntityAlive()) {
                ...
                Vec3 destination = spiderTarget.getPositionVector()
    ...
    private static void offerApproachCargoTeleport(EntityEnderman enderman,
        ...
        Vec3 away = horizontal(enderman.getPositionVector().subtract(cargo.getPositionVector()));
        if (away == null) {
            away = new Vec3(1.0D, 0.0D, 0.0D);
        }
        Optional<Vec3> safe = SafePositionFinder.resolveGroundDestination(
                enderman, cargo.getPositionVector().add(fr.vanillainstincts.compat.Minecraft112Compat.scale(away, 1.4D)));
        if (!safe.isPresent()) {
            return;
        }
        ...
        plan.offerSpecial(VanillaInstinctsState.TELEPORT_CARGO,
                ...
                () -> {
                    Vec3 before = enderman.getPositionVector();
    ...
    private static void offerIdleCarryTeleport(EntityEnderman enderman, Entity cargo,
        ...
        Vec3 desired =fr.vanillainstincts.compat.Minecraft112Compat.add(enderman.getPositionVector(), Math.cos(angle) * distance,
    ...
    public static void positionCargoPassenger(EntityEnderman enderman,
        ...
        Vec3 position = cargoCarryPosition(enderman.getPositionVector(),
    ...
    private static Optional<Vec3> resolveCreeperReleaseDestination(
        ...
        Vec3 requested = objective == null
                ? enderman.getPositionVector().add(fr.vanillainstincts.compat.Minecraft112Compat.scale(enderman.getLookVec(), 1.6D))
        ...
        for (double radius : radii) {
            for (int index = 0; index < 8; index++) {
                ...
                Vec3 candidate =fr.vanillainstincts.compat.Minecraft112Compat.add(objective.getPositionVector(), 
    ...
    private static Optional<Vec3> resolveCreeperRetreatDestination(
        ...
        if (away == null) {
            away = horizontal(fr.vanillainstincts.compat.Minecraft112Compat.scale(enderman.getLookVec(), -1.0D));
        }
        if (away == null) {
            away = new Vec3(1.0D, 0.0D, 0.0D);
        }
    ...
    private static void releaseHostileCargoAndRetreat(
        ...
        Vec3 away = objective == null
                ? horizontal(fr.vanillainstincts.compat.Minecraft112Compat.scale(enderman.getLookVec(), -1.0D))
                : horizontal(enderman.getPositionVector().subtract(objective.getPositionVector()));
        if (away == null) {
            away = new Vec3(1.0D, 0.0D, 0.0D);
        }
        Vec3 requested = enderman.getPositionVector().add(fr.vanillainstincts.compat.Minecraft112Compat.scale(away, 9.0D));
                ...
                Optional<Vec3> safe =
                        SafePositionFinder.resolveGroundDestination(
                                enderman, enderman.getPositionVector().add(
                                        fr.vanillainstincts.compat.Minecraft112Compat.scale(enderman.getLookVec(), 1.4D)));
    ...
    private static Vec3 creeperDeliveryPosition(EntityLivingBase objective,
        ...
        Vec3 away = horizontal(enderman.getPositionVector()
                .subtract(objective.getPositionVector()));
        if (away == null) {
            away = horizontal(fr.vanillainstincts.compat.Minecraft112Compat.scale(objective.getLookVec(), -1.0D));
        }
        if (away == null) {
            away = new Vec3(1.0D, 0.0D, 0.0D);
        }
        ...
        return objective.getPositionVector().add(fr.vanillainstincts.compat.Minecraft112Compat.scale(away, 
    ...
    private static Vec3 creeperRetreatPosition(
            EntityEnderman enderman, @Nullable EntityLivingBase objective,
            Vec3 releasePosition) {
        Vec3 away = objective == null ? horizontal(enderman.getPositionVector()
    ...
    private static Vec3 archerDestination(EntityLivingBase objective,
        ...
        return objective.getPositionVector().add(fr.vanillainstincts.compat.Minecraft112Compat.scale(away, distance)).add(lateral);
    }

    private static Vec3 presentationDestination(EntityLivingBase objective,
                                                EntityEnderman enderman) {
        Vec3 look = horizontal(objective.getLookVec());
        if (look == null) {
        ...
        return objective.getPositionVector().subtract(fr.vanillainstincts.compat.Minecraft112Compat.scale(look, 
    ...
    private static Optional<Vec3> playerExtractionDestination(
        ...
        for (int i = 0; i < 12; i++) {
            ...
            Vec3 requested =fr.vanillainstincts.compat.Minecraft112Compat.add(enderman.getPositionVector(), 
                    Math.cos(angle) * radius, 0.0D,
                    Math.sin(angle) * radius);
            Optional<Vec3> safe = SafePositionFinder.resolveGroundDestination(
                    enderman, requested);
    ...
    private static boolean safePlayerRelease(EntityEnderman enderman,
        ...
        return !enderman.isInWater()
                && !level.getBlockState(entityBlockPos(enderman)).getBlock().getMaterial().isLiquid()
                && enderman.onGround;
    }
