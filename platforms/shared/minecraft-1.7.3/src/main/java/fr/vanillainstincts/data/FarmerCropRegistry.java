...
public final class FarmerCropRegistry {
    ...
    public static CropDefinition definitionForState(LegacyBlockState state) {
    ...
    public static Block cropBlockForItem(Item item) {
        ...
        if (item instanceof ItemBlock
                && fr.vanillainstincts.compat.Minecraft17Compat.itemBlockBlock((ItemBlock) item) instanceof BlockCrops) {
            return fr.vanillainstincts.compat.Minecraft17Compat.itemBlockBlock((ItemBlock) item);
        }
        return null;
    }

    public static LegacyBlockState plantingState(Item item) {
        CropDefinition definition = definitionForItem(item);
        if (definition != null) return definition.plantingState();
        Block block = cropBlockForItem(item);
        return block == null ? null : fr.vanillainstincts.compat.Minecraft17Compat.defaultState(block);
    }

    public static boolean canPlant(Item item, WorldServer level,
                                   fr.vanillainstincts.compat.BlockPos pos) {
        if (item == null || level == null || pos == null) return false;
        LegacyBlockState planting = plantingState(item);
        if (planting == null) return false;
        CropDefinition definition = definitionForItem(item);
        if (definition != null && !definition.plantOn().isEmpty()
                && !definition.matchesGround(fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, pos.down()))) {
            return false;
        }
        return fr.vanillainstincts.compat.Minecraft17Compat.canPlaceBlockAt(planting.getBlock(), level, pos);
    }

    public static boolean hasPotentialPlantingGround(WorldServer level,
                                                     fr.vanillainstincts.compat.BlockPos pos) {
        if (level == null || pos == null) return false;
        LegacyBlockState occupant = fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, pos);
        ...
        if (!fr.vanillainstincts.compat.Minecraft112Compat.isAir(occupant) && !occupant.getBlock().getMaterial().isReplaceable()) return false;
        LegacyBlockState ground = fr.vanillainstincts.compat.Minecraft17Compat.getBlockState(level, pos.down());
        for (CropDefinition definition : byItem.values()) {
            if ((definition.plantOn().isEmpty()
                    || definition.matchesGround(ground))
                    && fr.vanillainstincts.compat.Minecraft17Compat.canPlaceBlockAt(definition.plantingState().getBlock(), level, pos)) {
                return true;
            }
        }
    ...
    public static boolean isMature(LegacyBlockState state) {
        ...
        if (state.getBlock() instanceof BlockCrops) { BlockCrops crop = (BlockCrops) (state.getBlock()); 
            return fr.vanillainstincts.compat.Minecraft17Compat.metaFromState(state) >= 7;
        }
        return false;
    }

    public static boolean shouldReplant(LegacyBlockState state) {
        if (state == null) return false;
        CropDefinition definition = definitionForState(state);
    ...
    public static LegacyBlockState harvestedReplacement(LegacyBlockState harvested) {
        ...
        if (harvested.getBlock() instanceof BlockCrops) { BlockCrops crop = (BlockCrops) (harvested.getBlock()); 
            return fr.vanillainstincts.compat.Minecraft17Compat.defaultState(crop);
        }
        return null;
    }
    ...
    public static class CropDefinition {
        ...
        public boolean matchesGround(LegacyBlockState ground) {
        ...
        public boolean isMature(LegacyBlockState state) {
            if (state == null || state.getBlock() != crop) return false;
            int current = fr.vanillainstincts.compat.Minecraft17Compat.metaFromState(state);
            int target = matureAge == null ? (crop instanceof BlockCrops ? 7 : 15) : matureAge.intValue();
            return current >= Math.max(0, Math.min(15, target));
        }

        public LegacyBlockState plantingState() {
            return fr.vanillainstincts.compat.Minecraft17Compat.defaultState(crop);
        }
    }
