...
public final class Minecraft110Compat {
    ...
    public static Vec3 boxCenter(AxisAlignedBB box) {
        if (box == null) return new Vec3(0.0D, 0.0D, 0.0D);
    ...
    public static boolean isBlock(LegacyBlockState state, String id) {
     ...
     */
    public static LegacyBlockState readBlockState(NBTTagCompound tag) {
        if (tag == null) return Minecraft17Compat.defaultState(Blocks.air);
        String name = tag.getString("Name");
        Block block = block(name);
        if (block == null) return Minecraft17Compat.defaultState(Blocks.air);
        int meta = tag.getInteger("Meta");
        try {
            return Minecraft17Compat.stateFromMeta(block, meta);
        } catch (RuntimeException ignored) {
            return Minecraft17Compat.defaultState(block);
        }
    }
    ...
    public static NBTTagCompound writeBlockState(NBTTagCompound tag,
        ...
        LegacyBlockState safe = state == null ? fr.vanillainstincts.compat.Minecraft17Compat.defaultState(Blocks.air) : state;
        ResourceLocation id = LegacyRegistry.BLOCK.getKey(safe.getBlock());
        out.setString("Name", id == null ? "minecraft:air" : id.toString());
        int meta = 0;
        try {
            meta = Minecraft17Compat.metaFromState(safe);
        } catch (RuntimeException ignored) { }
