...
public final class TargetAcquisitionController {
    ...
    public static AcquisitionResult findBestTarget(EntityLiving mob,
        ...
        AxisAlignedBB area = mob.getEntityBoundingBox().expandXyz(radius);
        List<EntityLivingBase> nearby = level.getEntitiesWithinAABB(
    ...
    public static boolean eligibleCandidate(EntityLiving mob, EntityLivingBase candidate) {
        ...
        if (candidate instanceof EntityPlayer
                && (((EntityPlayer) (candidate)).isCreative() || ((EntityPlayer) (candidate)).isSpectator())) { EntityPlayer player = (EntityPlayer) (candidate); 
            return false;
        }
