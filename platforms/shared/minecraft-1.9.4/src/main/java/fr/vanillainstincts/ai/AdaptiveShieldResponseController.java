...
public final class AdaptiveShieldResponseController {
    ...
    public static void onSkeletonJoin(EntitySkeleton skeleton,
                                      WorldServer level,
                                      boolean loadedFromDisk) {
    ...
    public static void tickSkeleton(EntitySkeleton skeleton,
    ...
    public static void contributeSkeleton(EntitySkeleton skeleton,
    ...
    public static boolean isShieldSkeleton(EntitySkeleton skeleton) {
        return skeleton != null
                && skeleton.getEntityData().getBoolean(SKELETON_SHIELD);
    }

    public static boolean isShieldSkeletonMelee(EntitySkeleton skeleton) {
    ...
    private static void promoteShieldSkeleton(EntitySkeleton skeleton,
                                              UUID targetId) {
    ...
    private static void maintainShieldMelee(EntitySkeleton skeleton,
    ...
    private static void setSkeletonMeleeMode(EntitySkeleton skeleton,
                                             boolean melee) {
        NBTTagCompound data = skeleton.getEntityData();
    ...
    private static void equipSkeletonMelee(EntitySkeleton skeleton) {
    ...
    private static void equipSkeletonRanged(EntitySkeleton skeleton) {
        skeleton.setItemStackToSlot(EntityEquipmentSlot.MAINHAND,
                AdaptiveEquipmentProgressionController.adaptiveBow(skeleton));
        skeleton.setItemStackToSlot(EntityEquipmentSlot.OFFHAND, null);
        fr.vanillainstincts.compat.Minecraft112Compat.reassessWeaponGoal(skeleton);
    }
    ...
    private static int frontLineAllies(EntityZombie flanker, EntityPlayerMP player,
        ...
        return (int) level.getEntitiesWithinAABB(EntityZombie.class,
                player.getEntityBoundingBox().expandXyz(radius), candidate ->
    ...
    private static boolean validTarget(EntityLiving mob, EntityPlayerMP player) {
        double range = AdaptiveProgressionRules.OWNER_SEARCH_RANGE;
        return isCombatTarget(player) && player.worldObj == mob.worldObj
    ...
    private static int countLocalZombieVariant(WorldServer level,
        ...
        return level.getEntitiesWithinAABB(EntityZombie.class,
                zombie.getEntityBoundingBox().expandXyz(radius), candidate ->
    ...
    private static int countLocalSkeletonVariant(WorldServer level,
        ...
        return level.getEntitiesWithinAABB(EntitySkeleton.class,
                skeleton.getEntityBoundingBox().expandXyz(radius), candidate ->
                        candidate.getEntityData().getBoolean(flag)).size();
    }
