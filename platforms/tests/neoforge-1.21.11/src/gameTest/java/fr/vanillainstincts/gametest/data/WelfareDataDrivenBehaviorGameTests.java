...
public final class WelfareDataDrivenBehaviorGameTests {
    ...
    public static void animalWelfareProfilesLoad(GameTestHelper helper) {
        helper.assertTrue(AnimalWelfareProfileManager.profileCount() >= 2,
                net.minecraft.network.chat.Component.literal("Animal Welfare 3.0 doit charger les profils par espèce"));
        helper.succeed();
    }
    ...
    public static void livestockUsesConfiguredWelfareProfile(GameTestHelper helper) {
        ...
        helper.assertValueEqual(profile.id().getPath(), "livestock",
                net.minecraft.network.chat.Component.literal("La vache doit utiliser le profil Animal Welfare livestock"));
        helper.assertTrue(profile.needsHerd() && profile.needsShelter(),
                net.minecraft.network.chat.Component.literal("Le profil livestock doit conserver ses besoins configurés"));
        helper.succeed();
    }
    ...
    public static void chickenWelfareCanDisableWaterNeed(GameTestHelper helper) {
        ...
        helper.assertValueEqual(profile.id().getPath(), "chickens",
                net.minecraft.network.chat.Component.literal("La poule doit utiliser son profil dédié"));
        helper.assertFalse(profile.needsNearbyWater(),
                net.minecraft.network.chat.Component.literal("Les besoins d'une espèce doivent être réellement configurables"));
        helper.succeed();
    }
    ...
    public static void animalWelfareEntersStressState(GameTestHelper helper) {
        ...
        helper.assertValueEqual(snapshot.state(), AnimalWelfareState.STRESSED,
                net.minecraft.network.chat.Component.literal("Une peur persistante et une mauvaise santé doivent créer du stress"));
        helper.assertTrue(snapshot.score() < snapshot.profile().stressThreshold(),
                net.minecraft.network.chat.Component.literal("L'état STRESSED doit être justifié par le score de bien-être"));
        helper.succeed();
    }
    ...
    public static void animalWelfareUsesRecoveryState(GameTestHelper helper) {
        ...
        helper.assertValueEqual(recovery.state(), AnimalWelfareState.RECOVERING,
                net.minecraft.network.chat.Component.literal("Le bien-être doit récupérer progressivement au lieu de passer instantanément à HEALTHY"));
        helper.succeed();
    }
    ...
    public static void stressedAnimalCannotBreed(GameTestHelper helper) {
        ...
        helper.assertFalse(AnimalWelfareController.allowsBreeding(cow),
                net.minecraft.network.chat.Component.literal("Un animal réellement stressé ne doit pas être considéré prêt à se reproduire"));
        helper.succeed();
    }
    ...
    public static void farmerRegistryStillLoadsCrops(GameTestHelper helper) {
        helper.assertTrue(FarmerCropRegistry.definitionCount() >= 4,
                net.minecraft.network.chat.Component.literal("Farmer 3.0 doit conserver le registre de cultures data-driven"));
        helper.succeed();
    }
    ...
    public static void farmerSoilUsesExtensibleTag(GameTestHelper helper) {
        helper.assertTrue(Blocks.FARMLAND.defaultBlockState()
                ...
                net.minecraft.network.chat.Component.literal("Le sol vanilla doit provenir du tag farmer_plantable_on extensible"));
        helper.succeed();
    }
    ...
    public static void farmerHoeImprovesWorkRecovery(GameTestHelper helper) {
        ...
        helper.assertTrue(withTool < withoutTool,
                net.minecraft.network.chat.Component.literal("Une houe reconnue par tag doit améliorer le rythme du fermier"));
        helper.succeed();
    }
    ...
    public static void villagerShareableFoodIsDataDriven(GameTestHelper helper) {
        helper.assertTrue(new ItemStack(Items.BREAD)
                ...
                net.minecraft.network.chat.Component.literal("La nourriture partageable vanilla doit provenir du tag extensible"));
        helper.succeed();
    }
    ...
    public static void villagerAvoidNearPenalizesLava(GameTestHelper helper) {
        ...
        double penalty = VillagePathPreferenceManager.proximityPreference(
                VillagerProfessionCompat.value(VillagerProfession.FARMER), level, candidate);
        helper.assertTrue(penalty < 0.0D,
                net.minecraft.network.chat.Component.literal("avoid_near doit pénaliser une destination proche de lave"));
        helper.succeed();
    }
    ...
    public static void zombieUsesContextualPursuitSprint(GameTestHelper helper) {
        ...
        helper.assertTrue(zombie.isSprinting(),
                net.minecraft.network.chat.Component.literal("Un zombie sain avec cible visible à moyenne distance doit sprinter contextuellement"));
        helper.succeed();
    }
    ...
    public static void spiderRainReducesAdhesion(GameTestHelper helper) {
        helper.assertTrue(SpiderTacticsController.rainAdhesionFactor(true)
                ...
                net.minecraft.network.chat.Component.literal("La pluie doit réduire l'adhérence tactique des araignées"));
        helper.succeed();
    }
    ...
    public static void spiderCanFindNearbyShore(GameTestHelper helper) {
        ...
        helper.assertTrue(shore.isPresent(),
                net.minecraft.network.chat.Component.literal("Generic Mobility doit donner une vraie recherche de rive aux araignées"));
        helper.succeed();
    }
    ...
    public static void creeperTacticsAcceptConfiguredNormalHeight(GameTestHelper helper) {
        helper.assertTrue(VanillaInstinctsServerConfig.creeperTacticsAllowedAtY(64),
                net.minecraft.network.chat.Component.literal("Les limites Y configurables ne doivent pas désactiver le creeper aux hauteurs normales par défaut"));
        helper.succeed();
    }
