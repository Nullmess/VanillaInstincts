...
public final class DataDrivenAiGameTests {
    ...
    public static void villagerPathRulesLoadFromDatapack(GameTestHelper helper) {
        helper.assertTrue(VillagePathPreferenceManager.ruleCount() >= 5,
                net.minecraft.network.chat.Component.literal("Les préférences de chemin intégrées doivent être chargées"));
        helper.succeed();
    }
    ...
    public static void dataDrivenFarmerPrefersFarmland(GameTestHelper helper) {
        ...
        helper.assertTrue(farmerScore > 0.0D,
                net.minecraft.network.chat.Component.literal("Le datapack doit favoriser la farmland pour le fermier"));
        helper.assertTrue(librarianScore < 0.0D,
                net.minecraft.network.chat.Component.literal("Le datapack doit décourager la farmland hors métier agricole"));
        helper.assertTrue(farmerScore > librarianScore,
                net.minecraft.network.chat.Component.literal("Les coûts de métier doivent produire un choix distinct"));
        helper.succeed();
    }
    ...
    public static void fishermanUsesDynamicWaterBonus(GameTestHelper helper) {
        ...
        fisherman.setVillagerData(fisherman.getVillagerData()
                .withProfession(net.minecraft.core.registries.BuiltInRegistries.VILLAGER_PROFESSION.get(VillagerProfession.FISHERMAN).orElseThrow()));

        helper.assertTrue(VillagePathEvaluator.professionPreference(
                        fisherman, level, feet) > 0.0D,
                net.minecraft.network.chat.Component.literal("Le bonus d'eau dynamique du pêcheur doit rester actif"));
        helper.succeed();
    }
    ...
    public static void perceptionProfilesLoadAndDifferentiateMobs(
        ...
        helper.assertTrue(PerceptionProfileManager.profileRuleCount() >= 7,
                net.minecraft.network.chat.Component.literal("Les profils de perception intégrés doivent être chargés"));
        ...
        helper.assertTrue(zombie.visualRange() > creeper.visualRange(),
                net.minecraft.network.chat.Component.literal("Les espèces doivent pouvoir avoir des portées distinctes"));
        helper.assertTrue(zombie.hardDifficultyMultiplier()
                        > zombie.easyDifficultyMultiplier(),
                net.minecraft.network.chat.Component.literal("La difficulté doit pouvoir moduler la perception"));
        helper.assertTrue(zombie.sneakingVisualMultiplier() < 1.0D
                        && zombie.sneakingHearingMultiplier() < 1.0D,
                net.minecraft.network.chat.Component.literal("La furtivité doit réduire vision et audition"));
        helper.succeed();
    }
    ...
    public static void perceptionNeverSeesThroughSolidWall(
        ...
        helper.assertFalse(MobPerceptionMemory.canSee(zombie, target),
                net.minecraft.network.chat.Component.literal("Perception 2.0 ne doit jamais donner de vision à travers un mur"));
        helper.succeed();
    }
    ...
    public static void farmerCropDefinitionsLoadFromDatapack(
        ...
        helper.assertTrue(FarmerCropRegistry.definitionCount() >= 4,
                net.minecraft.network.chat.Component.literal("Les quatre cultures vanilla doivent être enregistrées"));
        helper.assertValueEqual(FarmerCropRegistry.cropBlockForItem(
                        Items.WHEAT_SEEDS), Blocks.WHEAT,
                net.minecraft.network.chat.Component.literal("Le registre doit associer les graines de blé au blé"));
        helper.assertValueEqual(FarmerCropRegistry.cropBlockForItem(
                        Items.CARROT), Blocks.CARROTS,
                net.minecraft.network.chat.Component.literal("Le registre doit associer la carotte à sa culture"));
        helper.succeed();
    }
    ...
    public static void farmerPlantsRegisteredCrop(GameTestHelper helper) {
        runWorldMutationTest(helper, level -> {
            ...
            helper.assertTrue(level.getBlockState(cropPos).isAir(),
                    net.minecraft.network.chat.Component.literal("La case de plantation du fixture doit être vide"));
            Villager farmer = helper.spawn(EntityType.VILLAGER,
                    new BlockPos(1, 1, 1));
            farmer.setVillagerData(farmer.getVillagerData()
                    .withProfession(net.minecraft.core.registries.BuiltInRegistries.VILLAGER_PROFESSION.get(VillagerProfession.FARMER).orElseThrow()));
            ...
            helper.assertValueEqual(FarmerController.totalPlantableCount(
                    ...
                    net.minecraft.network.chat.Component.literal("Le stock de graines du fixture doit être visible"));
            helper.assertTrue(FarmerCropRegistry.canPlant(
                            Items.WHEAT_SEEDS, level, cropPos),
                    net.minecraft.network.chat.Component.literal("Le registre doit accepter la plantation sur le sol taggé"));
            helper.assertTrue(FarmerController.findPlantingSlot(
                            farmer.getInventory(), level, cropPos, 2) >= 0,
                    net.minecraft.network.chat.Component.literal("Le fermier doit trouver un emplacement de graines utilisable"));
            helper.assertValueEqual(WorldPermissionService.check(level, farmer,
                    ...
                    net.minecraft.network.chat.Component.literal("La passerelle de permission doit autoriser cette plantation"));
            helper.assertTrue(FarmerController.performAction(
                            level, farmer, cropPos, true),
                    net.minecraft.network.chat.Component.literal("Le fermier doit planter une culture enregistrée"));
            helper.assertTrue(level.getBlockState(cropPos).is(Blocks.WHEAT),
                    net.minecraft.network.chat.Component.literal("La plantation doit créer le bloc de culture enregistré"));
        });
    }
    ...
    public static void farmerHarvestsAndReplantsRegisteredCrop(
        ...
        runWorldMutationTest(helper, level -> {
            ...
            helper.assertTrue(FarmerController.performAction(
                            level, farmer, cropPos, true),
                    net.minecraft.network.chat.Component.literal("Le fermier doit récolter une culture mûre enregistrée"));
            helper.assertTrue(level.getBlockState(cropPos).is(Blocks.WHEAT),
                    net.minecraft.network.chat.Component.literal("Une culture replantable doit rester plantée après récolte"));
            helper.assertTrue(level.getBlockState(cropPos)
                            .equals(wheat.getStateForAge(0)),
                    net.minecraft.network.chat.Component.literal("La culture replantée doit revenir à son âge minimal"));
        });
    }
