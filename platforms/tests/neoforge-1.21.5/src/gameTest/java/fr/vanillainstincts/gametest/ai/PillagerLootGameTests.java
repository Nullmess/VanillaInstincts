...
public final class PillagerLootGameTests {
    ...
    @GameTest(batch = "pillager_loot", templateNamespace = "vanillainstincts", template = "empty")
    public static void everyPlayerRecoveredOfferHasOneUse(GameTestHelper helper) {
        ...
        helper.assertTrue(RecoveredTradeController.recover(merchant, deathDrop),
                net.minecraft.network.chat.Component.literal("Le marchand doit récupérer la pile du joueur mort"));
        MerchantOffer offer = merchant.getOffers().getFirst();
        helper.assertValueEqual(offer.getResult().getCount(), 32,
                net.minecraft.network.chat.Component.literal("La pile de mort doit être rendue entière, sans apprentissage unitaire"));
        helper.assertValueEqual(offer.getMaxUses(), 1,
                net.minecraft.network.chat.Component.literal("Le stuff récupéré ne doit être payable qu'une fois"));
        helper.assertTrue(RecoveredTradeController.consumePlayerRecoveredOffer(
                        merchant, offer) && merchant.getOffers().isEmpty(),
                net.minecraft.network.chat.Component.literal("Après paiement, l'offre doit disparaître immédiatement"));
        helper.assertFalse(RecoveredTradeController.consumePlayerRecoveredOffer(
                        merchant, offer),
                net.minecraft.network.chat.Component.literal("La même récupération ne doit jamais être payable deux fois"));
        ...
        helper.assertTrue(RecoveredTradeController.recover(merchant, manualDrop),
                net.minecraft.network.chat.Component.literal("Le marchand doit aussi récupérer une pile jetée manuellement"));
        MerchantOffer tossedOffer = merchant.getOffers().getFirst();
        helper.assertValueEqual(tossedOffer.getResult().getCount(), 16,
                net.minecraft.network.chat.Component.literal("Le drop manuel doit rester une pile exacte, sans offre apprise"));
        helper.assertValueEqual(tossedOffer.getMaxUses(), 1,
                net.minecraft.network.chat.Component.literal("Un drop manuel du joueur ne doit être payable qu'une fois"));
        helper.assertTrue(RecoveredTradeController.consumePlayerRecoveredOffer(
                        merchant, tossedOffer) && merchant.getOffers().isEmpty(),
                net.minecraft.network.chat.Component.literal("L'offre du drop manuel doit disparaître après le paiement"));
        helper.assertFalse(RecoveredTradeController.consumePlayerRecoveredOffer(
                        merchant, tossedOffer),
                net.minecraft.network.chat.Component.literal("Le drop manuel ne doit jamais être payable une seconde fois"));
        helper.succeed();
    }
    ...
    public static void naturalRecoveredOfferKeepsStock(GameTestHelper helper) {
        helper.assertValueEqual(RecoveredTradeController
                ...
                net.minecraft.network.chat.Component.literal("Une trouvaille naturelle ne doit pas être confondue avec le bien du joueur"));
        helper.succeed();
    }
    ...
    public static void everyPlayerStackMayBeLooted(GameTestHelper helper) {
        helper.assertTrue(PillagerOutpostLootController
                ...
                net.minecraft.network.chat.Component.literal("Les pillards doivent accepter tout équipement du joueur"));
        helper.succeed();
    }
    ...
    public static void emptyStackIsNotLoot(GameTestHelper helper) {
        helper.assertFalse(PillagerOutpostLootController
                ...
                net.minecraft.network.chat.Component.literal("Une pile vide ne doit pas créer une mission de pillage"));
        helper.succeed();
    }
    ...
    public static void darkOakCountsAsOutpostMaterial(GameTestHelper helper) {
        helper.assertTrue(PillagerOutpostLootController.isOutpostMaterial(
                ...
                net.minecraft.network.chat.Component.literal("Le bois sombre doit participer à la signature de l'avant-poste"));
        helper.succeed();
    }
    ...
    public static void cobblestoneCountsAsOutpostMaterial(GameTestHelper helper) {
        helper.assertTrue(PillagerOutpostLootController.isOutpostMaterial(
                ...
                net.minecraft.network.chat.Component.literal("La pierre de l'avant-poste doit être reconnue"));
        helper.succeed();
    }
    ...
    public static void unrelatedBlockIsRejected(GameTestHelper helper) {
        helper.assertFalse(PillagerOutpostLootController.isOutpostMaterial(
                ...
                net.minecraft.network.chat.Component.literal("Un coffre isolé dans le désert ne doit pas devenir un avant-poste"));
        helper.succeed();
    }
    ...
    public static void outpostScoreUsesThreshold(GameTestHelper helper) {
        helper.assertFalse(PillagerOutpostLootController.isOutpostChestScore(
                ...
                net.minecraft.network.chat.Component.literal("Une structure trop faible doit être refusée"));
        helper.assertTrue(PillagerOutpostLootController.isOutpostChestScore(
                        PillagerRules.OUTPOST_CHEST_MATERIAL_THRESHOLD),
                net.minecraft.network.chat.Component.literal("Le seuil exact doit identifier le coffre d'avant-poste"));
        helper.succeed();
    }
    ...
    public static void activeClaimDoesNotExpireEarly(GameTestHelper helper) {
        helper.assertFalse(PillagerOutpostLootController.assignmentExpired(
                ...
                net.minecraft.network.chat.Component.literal("Un pillard actif ne doit pas perdre sa pile prématurément"));
        helper.succeed();
    }
    ...
    public static void abandonedClaimCanBeRecovered(GameTestHelper helper) {
        helper.assertTrue(PillagerOutpostLootController.assignmentExpired(
                ...
                net.minecraft.network.chat.Component.literal("Une pile abandonnée doit pouvoir être reprise par un autre pillard"));
        helper.succeed();
    }
    ...
    public static void chestReceivesExactStack(GameTestHelper helper) {
        ...
        helper.assertTrue(remainder.isEmpty(),
                net.minecraft.network.chat.Component.literal("Le coffre vide doit recevoir toute la pile"));
        helper.assertValueEqual(chest.getItem(0).getCount(), 17,
                net.minecraft.network.chat.Component.literal("La quantité déposée doit rester exacte"));
        helper.succeed();
    }
    ...
    public static void fullChestReturnsOnlyOverflow(GameTestHelper helper) {
        ...
        helper.assertValueEqual(chest.getItem(0).getCount(), 64,
                net.minecraft.network.chat.Component.literal("Le coffre doit être rempli sans dépasser la limite"));
        helper.assertValueEqual(remainder.getCount(), 3,
                net.minecraft.network.chat.Component.literal("Seul le surplus réel doit rester au-dessus du coffre"));
        helper.succeed();
    }
}
