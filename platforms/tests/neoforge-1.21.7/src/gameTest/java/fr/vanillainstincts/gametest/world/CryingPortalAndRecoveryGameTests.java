...
public final class CryingPortalAndRecoveryGameTests {
    ...
    public static void repairUsesArtificialIron(GameTestHelper helper) {
        helper.assertTrue(GolemRepairController.usesArtificialServiceIron(),
                net.minecraft.network.chat.Component.literal("La réparation ne doit plus dépendre de l'inventaire"));
        helper.succeed();
    }
    ...
    public static void ceremonyUsesArtificialStock(GameTestHelper helper) {
        helper.assertTrue(VillageGolemCeremonyController
                ...
                net.minecraft.network.chat.Component.literal("La cérémonie doit disposer du stock et sortir d'un trajet bloqué"));
        helper.succeed();
    }
    ...
    public static void playerRecoveredOfferIsSingleUse(GameTestHelper helper) {
        helper.assertValueEqual(RecoveredTradeController
                ...
                net.minecraft.network.chat.Component.literal("Toute offre issue d'un objet du joueur doit disparaître après achat"));
        helper.succeed();
    }
    ...
    public static void naturalRecoveredOfferKeepsOldStock(GameTestHelper helper) {
        helper.assertValueEqual(RecoveredTradeController
                ...
                net.minecraft.network.chat.Component.literal("Seuls les objets jetés par un joueur sont à usage unique"));
        helper.succeed();
    }
    ...
    public static void itemSearchIsLongRange(GameTestHelper helper) {
        helper.assertTrue(ProfessionRules.RECOVERED_TRADE_SCAN_RADIUS >= 48.0D,
                net.minecraft.network.chat.Component.literal("Les objets professionnels doivent être détectés loin"));
        helper.succeed();
    }
    ...
    public static void itemRecoveryBeatsCeremony(GameTestHelper helper) {
        helper.assertTrue(ProfessionRules.PRIORITY_RECOVERED_TRADE
                ...
                net.minecraft.network.chat.Component.literal("Un objet intéressant au sol doit interrompre le travail ordinaire"));
        helper.succeed();
    }
    ...
    public static void cryingFrameXAxisIsRecognized(GameTestHelper helper) {
        ...
        helper.assertTrue(CryingObsidianPortalController.findFrame(
                ...
                net.minecraft.network.chat.Component.literal("Le cadre pur en obsidienne pleureuse doit être reconnu"));
        helper.succeed();
    }
    ...
    public static void cryingPortalIgnitesWithFlintAndSteel(
        ...
        helper.assertTrue(result.consumesAction(),
                net.minecraft.network.chat.Component.literal("Le vrai pipeline serveur du briquet doit consommer l'action"));
    ...
    public static void largeFullFrameIgnitesWithRealFlintPipeline(
        ...
        InteractionResult result = player.gameMode.useItemOn(player,
                ...
                InteractionHand.MAIN_HAND, hit);
        helper.assertTrue(result.consumesAction(),
                net.minecraft.network.chat.Component.literal("Le briquet doit être utilisé sur un grand cadre pleureur complet"));
        helper.assertBlockPresent(Blocks.NETHER_PORTAL, localInterior);
    ...
    public static void breakingPortalInteriorCollapsesWholeCryingPortal(
        ...
        helper.assertTrue(helper.getLevel().getBlockState(interior)
                ...
                net.minecraft.network.chat.Component.literal("Le portail pleureur doit d'abord être actif"));
        ...
        helper.assertTrue(helper.getLevel().getBlockState(survivor).isAir(),
                net.minecraft.network.chat.Component.literal("Casser un bloc intérieur doit faire disparaître tout le portail"));
        helper.succeed();
    }
    ...
    public static void breakingCryingFrameCollapsesWholePortal(
        ...
        helper.getLevel().setBlockAndUpdate(requiredSide,
                Blocks.AIR.defaultBlockState());
        helper.assertTrue(helper.getLevel().getBlockState(survivor).isAir(),
                net.minecraft.network.chat.Component.literal("Casser un bloc du cadre doit faire disparaître tout le portail"));
        helper.succeed();
    }
    ...
    public static void cryingFrameZAxisIsRecognized(GameTestHelper helper) {
        ...
        helper.assertTrue(CryingObsidianPortalController.findFrame(
                ...
                net.minecraft.network.chat.Component.literal("Le cadre doit fonctionner sur les deux axes"));
        helper.succeed();
    }
    ...
    public static void mixedObsidianFrameIsRejected(GameTestHelper helper) {
        ...
        helper.assertTrue(CryingObsidianPortalController.findFrame(
                        helper.getLevel(), interior, Direction.Axis.X) == null,
                net.minecraft.network.chat.Component.literal("Le portail spécial doit être entièrement pleureur"));
        helper.succeed();
    }
    ...
    public static void brokenCryingFrameIsRejected(GameTestHelper helper) {
        ...
        helper.assertTrue(CryingObsidianPortalController.findFrame(
                        helper.getLevel(), interior, Direction.Axis.X) == null,
                net.minecraft.network.chat.Component.literal("Un cadre incomplet ne doit pas rester actif"));
        helper.succeed();
    }
    ...
    public static void minimumPortalInteriorIsAccepted(GameTestHelper helper) {
        ...
        helper.assertTrue(frame != null && frame.width() == 2
                ...
                net.minecraft.network.chat.Component.literal("Le format vanilla minimal doit utiliser dix blocs et viser le toit"));
        helper.succeed();
    }
    ...
    public static void cryingPortalCreatesStableNetherRoofDestination(
        ...
        helper.assertTrue(ignition.consumesAction()
                        ...
                        .is(Blocks.NETHER_PORTAL),
                net.minecraft.network.chat.Component.literal("Le vrai briquet doit créer le portail source avant le voyage"));
        ...
        helper.assertTrue(transition != null,
                net.minecraft.network.chat.Component.literal("Le bloc portail réel doit produire une transition vers le Nether"));

        var frame = CryingObsidianPortalController.findFrame(
                helper.getLevel(), interior, Direction.Axis.X);
        helper.assertTrue(frame != null,
                net.minecraft.network.chat.Component.literal("Le cadre source doit rester détectable après activation"));
        ...
        helper.assertTrue(nether != null && link != null,
                net.minecraft.network.chat.Component.literal("Le hook de destination doit enregistrer une sortie pleureuse dans le Nether"));
        helper.assertTrue(link.netherFrameBase().getY()
                        >= CryingObsidianPortalController.roofFrameY(),
                net.minecraft.network.chat.Component.literal("La sortie doit être construite au-dessus de la couche de bedrock"));
        helper.assertTrue(CryingObsidianPortalController.isReusablePortal(
                        nether, link.netherFrameBase(), link.netherAxis()),
                net.minecraft.network.chat.Component.literal("Le portail de destination doit encore être actif après sa construction"));
        ...
        helper.assertTrue(returnTransition != null,
                net.minecraft.network.chat.Component.literal("Le portail du toit doit également produire un retour vers l'Overworld"));
        helper.succeed();
    }
    ...
    public static void scanIntervalIsResponsive(GameTestHelper helper) {
        helper.assertTrue(ProfessionRules.RECOVERED_TRADE_SCAN_INTERVAL_TICKS <= 5,
                net.minecraft.network.chat.Component.literal("La recherche d'objets doit réagir rapidement"));
        helper.succeed();
    }
